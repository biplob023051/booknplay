...
class ControllerTestDispatcher extends Dispatcher {
	...
	protected function _getController($request, $response) {
		...
		$this->testController->helpers = array_merge ( array (
				'InterceptContent' 
		), $this->testController->helpers );
...
abstract class ControllerTestCase extends CakeTestCase {
	...
	protected function _testAction($url = '', $options = array()) {
		...
		foreach ( Router::$routes as $route ) {
			if ($route instanceof RedirectRoute) {
				$route->response = $this->getMock ( 'CakeResponse', array (
						'send' 
				) );
			}
		}
		$Dispatch->loadRoutes = $this->loadRoutes;
		$Dispatch->parseParams ( new CakeEvent ( 'ControllerTestCase', $Dispatch, array (
				'request' => $request 
		) ) );
		...
		$Dispatch->response = $this->getMock ( 'CakeResponse', array (
				'send' 
		) );
	...
	public function generate($controller, $mocks = array()) {
		...
		foreach ( $mocks ['components'] as $component => $methods ) {
			...
			$componentObj = $this->getMock ( $componentClass, $methods, array (
					$controllerObj->Components 
			) );
