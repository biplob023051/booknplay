...
class CakeTestFixture {
	...
	public function __construct() {
		...
		if (! empty ( $this->useDbConfig )) {
			...
			if (strpos ( $connection, 'test' ) !== 0) {
				$message = __d ( 'cake_dev', 'Invalid datasource name "%s" for "%s" fixture. Fixture datasource names must begin with "test".', $connection, $this->name );
	...
	public function init() {
		if (isset ( $this->import ) && (is_string ( $this->import ) || is_array ( $this->import ))) {
			$import = array_merge ( array (
					'connection' => 'default',
					'records' => false 
			), is_array ( $this->import ) ? $this->import : array (
					'model' => $this->import 
			) );
	...
	public function create($db) {
		...
		$this->Schema->build ( array (
				$this->table => $this->fields 
		) );
		try {
			$db->execute ( $db->createSchema ( $this->Schema ), array (
					'log' => false 
			) );
	...
	public function drop($db) {
		...
		try {
			...
			$db->execute ( $db->dropSchema ( $this->Schema ), array (
					'log' => false 
			) );
			$this->created = array_diff ( $this->created, array (
					$db->configKeyName 
			) );
		} catch ( Exception $e ) {
			return false;
	...
	public function insert($db) {
		if (! isset ( $this->_insert )) {
			...
			if (isset ( $this->records ) && ! empty ( $this->records )) {
				...
				if ($this->primaryKey && isset ( $this->fields [$this->primaryKey] ['type'] ) && in_array ( $this->fields [$this->primaryKey] ['type'], array (
						'integer',
						'biginteger' 
				) )) {
	 ...
	 * Can be overwritten by classes extending
