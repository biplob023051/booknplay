...
class CakeEventManagerTest extends CakeTestCase {
	...
	public function testAttachListeners() {
		...
		$manager->attach ( 'inQ5', 'fake.event', array (
				'priority' => 5 
		) );
		$manager->attach ( 'inQ1', 'fake.event', array (
				'priority' => 1 
		) );
		$manager->attach ( 'otherInQ5', 'fake.event', array (
				'priority' => 5 
		) );
	...
	public function testAttachMultipleEventKeys() {
		...
		$manager->attach ( 'fakeFunction3', 'another.event', array (
				'priority' => 1,
				'passParams' => true 
		) );
		$expected = array (
	...
	public function testDetach() {
		...
		$manager->attach ( 'fakeFunction', 'another.event', array (
				'priority' => 1 
		) );
		
		$manager->detach ( array (
				'AClass',
				'aMethod' 
		), 'fake.event' );
	...
	public function testDispatchReturnValue() {
		$this->skipIf ( version_compare ( PHPUnit_Runner_Version::id (), '3.7', '<' ), 'These tests fail in PHPUnit 3.6' );
		$manager = new CakeEventManager ();
		$listener = $this->getMock ( 'CakeEventTestListener' );
		$anotherListener = $this->getMock ( 'CakeEventTestListener' );
		$manager->attach ( array (
		...
		$listener->expects ( $this->at ( 0 ) )->method ( 'listenerFunction' )->with ( $event )->will ( $this->returnValue ( 'something special' ) );
	...
	public function testDispatchFalseStopsEvent() {
		...
		$listener->expects ( $this->at ( 0 ) )->method ( 'listenerFunction' )->with ( $event )->will ( $this->returnValue ( false ) );
	...
	public function testDispatchPassingParams() {
		...
		$event = new CakeEvent ( 'fake.event', $this, array (
				'some' => 'data' 
		) );
	...
	public function testAttachSubscriber() {
		$manager = new CakeEventManager ();
		$listener = $this->getMock ( 'CustomTestEventListerner', array (
				'secondListenerFunction' 
		) );
		...
		$event = new CakeEvent ( 'another.event', $this, array (
				'some' => 'data' 
		) );
		$manager->dispatch ( $event );
	...
	public function testDispatchWithGlobal() {
		$generalManager = $this->getMock ( 'CakeEventManager', array (
				'dispatch' 
		) );
		$manager = new CakeEventManager ();
		$event = new CakeEvent ( 'fake.event' );
