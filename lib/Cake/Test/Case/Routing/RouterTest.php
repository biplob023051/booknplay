...
class RouterTest extends CakeTestCase {
	...
	public function testFullBaseURL() {
		...
		$this->assertRegExp ( '/^http(s)?:\/\//', Router::url ( array (
				'full_base' => true 
		) ) );
		$this->assertSame ( FULL_BASE_URL . '/', Router::url ( array (
				'full_base' => true 
		) ) );
	}
	...
	public function testRouteDefaultParams() {
		...
		$this->assertEquals ( Router::url ( array (
				'action' => 'index' 
		) ), '/' );
	}
	...
	public function testMapResources() {
		...
		$this->assertEquals ( array (
				'pass' => array (
						'475acc39-a328-44d3-95fb-015000000000' 
				),
				'named' => array (),
				'plugin' => '',
				'controller' => 'posts',
	...
	public function testPluginMapResourcesWithPrefix() {
		...
		$resources = Router::mapResources ( 'TestPlugin.TestPlugin', array (
				'prefix' => '/api/' 
		) );
	...
	public function testUrlGenerationBasic() {
		...
		Router::reload ();
		Router::connect ( '/:plugin/:id/*', array (
				'controller' => 'posts',
				'action' => 'view' 
		), array (
				'id' => $ID 
		...
		Router::reload ();
		Router::connect ( '/:controller/:action/:id', array (), array (
				'id' => $ID 
		) );
		Router::parse ( '/' );
		...
		Router::connect ( 'short_controller_name/:action/*', array (
				'controller' => 'real_controller_name' 
		) );
		Router::parse ( '/' );
		...
		Router::reload ();
		Router::connect ( '/contact/:action', array (
				'plugin' => 'contact',
				'controller' => 'contact' 
		) );
		Router::parse ( '/' );
	...
	public function testUrlGenerationWithRegexQualifiedParams() {
		Router::connect ( ':language/galleries', array (
		...
		Router::connect ( '/:language/:admin/:controller/:action/*', array (
				'admin' => 'admin' 
		), array (
				'language' => '[a-z]{3}',
				'admin' => 'admin' 
		...
		Router::reload ();
		Router::connect ( '/:language/pages', array (
				'controller' => 'pages',
				'action' => 'index' 
		), array (
				'language' => '[a-z]{3}' 
		...
		Router::reload ();
		Router::connect ( '/forestillinger/:month/:year/*', array (
				'plugin' => 'shows',
				'controller' => 'shows',
				'action' => 'calendar' 
		), array (
		...
		Router::reload ();
		Router::connect ( '/kalender/:month/:year/*', array (
				'plugin' => 'shows',
				'controller' => 'shows',
				'action' => 'calendar' 
		), array (
	...
	public function testUrlGenerationWithAdminPrefix() {
		...
		Router::connect ( '/pages/contact_us', array (
				'controller' => 'pages',
				'action' => 'contact_us' 
		) );
		Router::connect ( '/pages/*', array (
		...
		$request->addParams ( array (
				...
				'admin' => true,
				'url' => array (
						'url' => 'admin/subscriptions/index/page:2' 
				) 
		) );
		$request->base = '/magazine';
		...
		Router::reload ();
		Router::connect ( '/admin/:controller/:action/:id', array (
				'admin' => true 
		), array (
				'id' => '[0-9]+' 
		) );
		Router::parse ( '/' );
		$request = new CakeRequest ();
		Router::setRequestInfo ( $request->addParams ( array (
				'plugin' => null,
				'controller' => 'pages',
				'action' => 'admin_edit',
				'pass' => array (
	...
	public function testUrlGenerationPlugins() {
		...
		Router::reload ();
		Router::connect ( '/:lang/:plugin/:controller/*', array (
				'action' => 'index' 
		) );
	...
	public function testCanLeavePlugin() {
		Router::reload ();
		Router::connect ( '/admin/other/:controller/:action/*', array (
				'admin' => 1,
				'plugin' => 'aliased',
				'prefix' => 'admin' 
		) );
	...
	public function testUrlParsing() {
		...
		$expected = array (
				'pass' => array (
						'47fc97a9-019c-41d1-a058-1fa3cbdd56cb',
						'sample-post-title' 
				),
				'named' => array (),
	...
	public function testPersistentParameters() {
		Router::reload ();
		Router::connect ( '/:lang/:color/posts/view/*', array (
				'controller' => 'posts',
				'action' => 'view' 
		), array (
				'persist' => array (
		...
		Router::connect ( '/:lang/:color/posts/index', array (
				'controller' => 'posts',
				'action' => 'index' 
		), array (
				'persist' => array (
		...
		Router::connect ( '/:lang/:color/posts/edit/*', array (
				'controller' => 'posts',
				'action' => 'edit' 
		) );
	...
	public function testUuidRoutes() {
		Router::connect ( '/subjects/add/:category_id', array (
				'controller' => 'subjects',
				'action' => 'add' 
		), array (
	...
	public function testRouteSymmetry() {
		Router::connect ( "/:extra/page/:slug/*", array (
				'controller' => 'pages',
				'action' => 'view',
				'extra' => null 
		), array (
	...
	public function testPrefixRoutingAndPlugins() {
		...
		Router::setRequestInfo ( $request->addParams ( array (
				...
				'form' => array (),
				'url' => array (
						'url' => 'admin/shows/show_tickets/edit/6' 
				) 
		) )->addPaths ( array (
				'base' => '/',
	...
	public function testSetExtensions() {
		...
		$result = Router::setExtensions ( array (
				'pdf' 
		), false );
		$this->assertEquals ( array (
				'pdf' 
	...
	public function testConnectNamed() {
		$named = Router::connectNamed ( false, array (
				'default' => true 
		) );
		...
		Router::reload ();
		Router::connect ( '/foo/*', array (
				'controller' => 'bar',
				'action' => 'fubar' 
		) );
		Router::connectNamed ( array (), array (
				'separator' => '=' 
		) );
	...
	public function testParsingWithPatternOnAction() {
		Router::reload ();
		Router::connect ( '/blog/:action/*', array (
	...
	public function testUrlWritingWithPrefixes() {
		Router::connect ( '/company/:controller/:action/*', array (
				'prefix' => 'company',
				'company' => true 
		) );
	...
	public function testPassedArgsOrder() {
		Router::connect ( '/test-passed/*', array (
				'controller' => 'pages',
				'action' => 'display',
				'home' 
		) );
		Router::connect ( '/test2/*', array (
				'controller' => 'pages',
				'action' => 'display',
				2 
		) );
	...
	public function testCurrentRoute() {
		...
		$route = Router::currentRoute ();
		$this->assertEquals ( array_merge ( $url, array (
				'plugin' => null 
		) ), $route->defaults );
	}
	...
	public function testUsingCustomRouteClass() {
		...
		$routes = Router::connect ( '/:slug', array (
				'controller' => 'posts',
				'action' => 'view' 
		), array (
				'routeClass' => 'MockConnectedRoute',
				'slug' => '[a-z_-]+' 
		) );
		...
		$routes [0]->expects ( $this->any () )->method ( 'parse' )->will ( $this->returnValue ( $expected ) );
	...
	public function testRouterReverse() {
		...
		Router::connect ( '/:lang/:controller/:action/*', array (), array (
				'lang' => '[a-z]{3}' 
		) );
		$params = array (
				'lang' => 'eng',
	...
	public function testUrlFullUrlReturnFromRoute() {
		...
		$this->getMock ( 'CakeRoute', array (), array (
				'/' 
		), 'MockReturnRoute' );
		$routes = Router::connect ( '/:controller/:action', array (), array (
				'routeClass' => 'MockReturnRoute' 
		) );
		$routes [0]->expects ( $this->any () )->method ( 'match' )->will ( $this->returnValue ( $url ) );
	...
	public function testPatternOnAction() {
		$route = new CakeRoute ( '/blog/:action/*', array (
				'controller' => 'blog_posts' 
		), array (
				'action' => 'other|actions' 
		) );
	...
	public function testRouteRedirection() {
		Router::redirect ( '/blog', array (
				'controller' => 'posts' 
		), array (
				'status' => 302 
		) );
		$this->assertEquals ( 1, count ( Router::$routes ) );
		Router::$routes [0]->response = $this->getMock ( 'CakeResponse', array (
				'_sendHeader' 
		) );
	...
	public function testDefaultRouteClass() {
		$this->getMock ( 'CakeRoute', array (), array (
				'/test' 
		), 'TestDefaultRouteClass' );
		Router::defaultRouteClass ( 'TestDefaultRouteClass' );
		
		$result = Router::connect ( '/', array (
				'controller' => 'pages',
				'action' => 'display',
				'home' 
		) );
	...
	public function testQueryString() {
		$result = Router::queryString ( array (
				'var' => 'foo bar' 
		) );
		$expected = '?var=foo+bar';
		$this->assertEquals ( $expected, $result );
		
		$result = Router::queryString ( false, array (
				'some' => 'param',
				'foo' => 'bar' 
		) );
		...
		$result = Router::queryString ( $existing, array (
				'some' => 'param',
				'foo' => 'bar' 
		) );
		...
		$result = Router::queryString ( 'foo=bar', array (
				'php' => 'nut',
				'jose' => 'zap' 
		), true );
