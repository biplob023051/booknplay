...
class DispatcherTest extends CakeTestCase {
	...
	public function testParseParamsWithoutZerosAndEmptyPost() {
		...
		$event = new CakeEvent ( 'DispatcherTest', $Dispatcher, array (
				'request' => $request 
		) );
	...
	public function testMissingController() {
		...
		$Dispatcher->dispatch ( $url, $response, array (
				'return' => 1 
		) );
	}
	...
	public function testDispatchActionSendsFile() {
		...
		$response->expects ( $this->never () )->method ( 'body' );
	...
	public function testPluginDispatch() {
		...
		Router::reload ();
		$Dispatcher = new TestDispatcher ();
		Router::connect ( '/my_plugin/:controller/*', array (
				'plugin' => 'my_plugin',
				'controller' => 'pages',
				'action' => 'display' 
		) );
	...
	public function testAutomaticPluginDispatch() {
		...
		Router::reload ();
		$Dispatcher = new TestDispatcher ();
		Router::connect ( '/my_plugin/:controller/:action/*', array (
				'plugin' => 'my_plugin',
				'controller' => 'pages',
				'action' => 'display' 
		) );
	...
	public function testAutomaticPluginControllerDispatch() {
		...
		CakePlugin::load ( 'MyPlugin', array (
				'path' => '/fake/path' 
		) );
		...
		$this->assertEquals ( array (
				5 
		), $Dispatcher->controller->params ['pass'] );
		$this->assertEquals ( array (
				'param' => 'value',
				'param2' => 'value2' 
		), $Dispatcher->controller->params ['named'] );
		...
		CakePlugin::load ( 'ArticlesTest', array (
				'path' => '/fake/path' 
		) );
		Router::reload ();
		require CAKE . 'Config' . DS . 'routes.php';
		...
		$Dispatcher->dispatch ( new CakeRequest ( 'admin/articles_test' ), $response, array (
				'return' => 1 
		) );
