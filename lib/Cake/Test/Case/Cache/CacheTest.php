...
class CacheTest extends CakeTestCase {
	...
	public function testConfigWithLibAndPluginEngines() {
		...
		$settings = array (
				'engine' => 'TestPlugin.TestPluginCache',
				'path' => TMP,
				'prefix' => 'cake_test_' 
		);
	...
	public function testConfigChange() {
		...
		$result = Cache::config ( 'sessions', array (
				'engine' => 'File',
				'path' => TMP . 'sessions' 
		) );
		$this->assertEquals ( Cache::settings ( 'sessions' ), $result ['settings'] );
		
		$result = Cache::config ( 'tests', array (
				'engine' => 'File',
				'path' => TMP . 'tests' 
		) );
	...
	public function testWriteTriggerError() {
		...
		Cache::config ( 'test_trigger', array (
				'engine' => 'TestAppCache',
				'prefix' => '' 
		) );
		try {
	...
	public function testCacheDisable() {
		Configure::write ( 'Cache.disable', false );
		Cache::config ( 'test_cache_disable_1', array (
				'engine' => 'File',
				'path' => TMP . 'tests' 
		) );
		...
		Configure::write ( 'Cache.disable', true );
		Cache::config ( 'test_cache_disable_2', array (
				'engine' => 'File',
				'path' => TMP . 'tests' 
		) );
	...
	public function testSetOnAlternateConfigs() {
		Cache::config ( 'file_config', array (
				'engine' => 'File',
				'prefix' => 'test_file_' 
		) );
		Cache::set ( array (
