...
class ModelWriteTest extends BaseModelTest {
	...
	public function testSaveDateAsFirstEntry() {
		...
		$testResult = $Article->find ( 'first', array (
				'conditions' => array (
	...
	/**
	 * Test counter cache with models that use a non-standard (i.e.
	 * not using 'id')
	...
	public function testCounterCacheWithNonstandardPrimaryKey() {
		$this->loadFixtures ( 'CounterCacheUserNonstandardPrimaryKey', 'CounterCachePostNonstandardPrimaryKey' );
	...
	public function testCounterCacheWithSelfJoin() {
		...
		$column .= $this->db->buildColumn ( array (
				'name' => 'child_count',
				'type' => 'integer' 
		) );
		...
		$result = $Category->updateAll ( array (
				'CategoryThread.name' => "'updated'" 
		), array (
				'CategoryThread.parent_id' => 5 
		) );
		$this->assertFalse ( empty ( $result ) );
		
		$Category = new CategoryThread ();
		$Category->belongsTo ['ParentCategory'] ['counterCache'] = 'child_count';
		$Category->updateCounterCache ( array (
				'parent_id' => 5 
		) );
		$result = Hash::extract ( $Category->find ( 'all', array (
				'conditions' => array (
						'CategoryThread.id' => 5 
				) 
		) ), '{n}.CategoryThread.child_count' );
		$expected = array (
				1 
		);
		$this->assertEquals ( $expected, $result );
	...
	public function testSaveWithCounterCacheScope() {
		...
		$TestModel2->belongsTo ['Syfile'] ['counterScope'] = array (
				'published' => true 
		);
	...
	public function testSaveField() {
		...
		$this->assertEquals ( array (
				'Second' 
		), Hash::extract ( $result, 'ParentNode.{n}.name' ) );
	...
	public function testSaveWithCreate() {
		...
		$result = $TestModel->create () && $TestModel->save ( $data, true, array (
				'id',
				'title',
				'published' 
		) );
	...
	public function testSaveFromXml() {
		...
		$this->assertFalse ( empty ( $result ) );
		$results = $Article->find ( 'first', array (
				'conditions' => array (
						'Article.title' => 'test xml' 
				) 
		) );
		...
		$result = $Article->save ( Xml::build ( '<article><title>testing with DOMDocument</title><user_id>7</user_id></article>', array (
				'return' => 'domdocument' 
		) ) );
		$this->assertFalse ( empty ( $result ) );
		$results = $Article->find ( 'first', array (
				'conditions' => array (
						'Article.title' => 'testing with DOMDocument' 
				) 
		) );
		$this->assertFalse ( empty ( $results ) );
	}
	...
	public function testSaveHabtm() {
		...
		$TestModel = new JoinA ();
		$TestModel->hasBelongsToMany = array (
				'JoinC' => array (
						'unique' => true 
				) 
		);
	...
	public function testCreationWithMultipleDataSameModel() {
		...
		$result = $Article->field ( 'title', array (
				'id' => 1 
		) );
	...
	public function testCreationWithMultipleDataSameModelManualInstances() {
		...
		$result = $Primary->field ( 'primary_name', array (
				'id' => 1 
		) );
	...
	public function testUpdateMultiple() {
		...
		$result = $TestModel->updateAll ( array (
				'Comment.comment' => "'Updated today'" 
		), array (
				'Comment.user_id' => 5 
		) );
		$this->assertFalse ( empty ( $result ) );
		$result = Hash::extract ( $TestModel->find ( 'all', array (
				'conditions' => array (
						'Comment.user_id' => 5 
				) 
		) ), '{n}.Comment.comment' );
	...
	public function testHabtmUuidWithUuidId() {
		...
		$data ['Uuiditem'] ['Uuiditem'] = array (
				'483798c8-c7cc-430e-8cf9-4fcc40cf8569' 
		);
	...
	public function testHabtmUuidWithNumericId() {
		...
		$data ['Uuidportfolio'] ['Uuidportfolio'] = array (
				'480af662-eb8c-47d3-886b-230540cf8569' 
		);
		$TestModel->create ( $data );
		$TestModel->save ();
		$id = $TestModel->id;
	...
	public function testSaveAllHasOne() {
		...
		$model->Attachment->bindModel ( array (
				'belongsTo' => array (
						'Comment' 
				) 
		), false );
		...
		$this->assertTrue ( $model->saveAll ( $data, array (
				'validate' => 'first' 
		) ) );
	}
	...
	public function testSaveAllHasOneValidation() {
		...
		$model->Attachment->validate = array (
				'attachment' => 'notEmpty' 
		);
		$model->Attachment->bindModel ( array (
				'belongsTo' => array (
	...
	public function testSaveAllDeepAssociated() {
		...
		$TestModel = new Article ();
		$TestModel->hasMany ['Comment'] ['order'] = array (
				'Comment.created' => 'ASC' 
		);
		...
		$result = $TestModel->Comment->User->field ( 'id', array (
				'user' => 'newuser',
				'password' => 'newuserpass' 
		) );
		...
		$result = $TestModel->Comment->Attachment->field ( 'id', array (
				'attachment' => 'deepsaved' 
		) );
		$this->assertEquals ( 2, $result );
		$data = array (
		...
		unset ( $result ['Article'] ['created'], $result ['Article'] ['updated'], $result ['User'] ['created'], $result ['User'] ['updated'], $result ['Comment'] [0] ['created'], $result ['Comment'] [0] ['updated'] );
		$this->assertEquals ( $expected, $result );
	}
		...
		$data = array (
				array (
						...
						'Comment' => array (
								...
								array (
										'comment' => 'Second comment deepsaved article 1',
										'published' => 'Y',
										'user_id' => 2 
								) 
						) 
				...
				array (
						...
						'Comment' => array (
								...
								array (
										'comment' => 'Second comment deepsaved article 2',
										'published' => 'Y',
										'user_id' => 2 
								) 
						) 
	...
	public function testSaveAllNotDeepValidateOnly() {
		...
		$result = $TestModel->saveAll ( array (
				'Article' => array (
						'id' => 2,
						'body' => 'Ignore invalid attachment data' 
				),
				'Comment' => array (
						array (
								'comment' => 'Third new comment',
	...
	public function testSaveAllManyRowsTransactionNoRollback() {
		...
		$this->getMock ( 'DboSource', array (
				...
				'rollback',
				'describe' 
		), array (), 'MockTransactionDboSource' );
	...
	public function testSaveAllAssociatedTransactionNoRollback() {
		...
		$this->getMock ( 'DboSource', array (
				...
				'update',
				'begin' 
		), array (), 'MockTransactionAssociatedDboSource' );
		...
		$db->expects ( $this->once () )->method ( 'rollback' );
		$db->expects ( $this->any () )->method ( 'describe' )->will ( $this->returnValue ( array (
				'id' => array (
						'type' => 'integer',
						'length' => 11 
				),
	...
	public function testSaveAllNestedSaveAll() {
		...
		$this->assertTrue ( $TransactionTestModel->saveAll ( $data, array (
				'atomic' => true 
		) ) );
	}
	...
	public function testSaveAllValidation() {
		...
		$this->assertEquals ( self::date (), $result [3] ['Post'] ['updated'] );
		$this->assertEquals ( self::date (), $result [3] ['Post'] ['created'] );
		unset ( $result [0] ['Post'] ['updated'], $result [1] ['Post'] ['updated'], $result [3] ['Post'] ['updated'], $result [3] ['Post'] ['created'] );
		$this->assertEquals ( $expected, $result );
		$this->assertEquals ( $errors, $TestModel->validationErrors );
		...
		$this->assertFalse ( $TestModel->saveAll ( $data, array (
				'validate' => 'first' 
		) ) );
	...
	public function testSaveAllValidationOnly() {
		...
		$TestModel = new Comment ();
		$TestModel->Attachment->validate = array (
				'attachment' => 'notEmpty' 
		);
	...
	public function testSaveAllValidateFirst() {
		...
		$result = $model->Comment->saveAll ( $data, array (
				'validate' => 'first' 
		) );
		$this->assertFalse ( empty ( $result ) );
		
		$result = $model->find ( 'all' );
		$this->assertEquals ( $result [0] ['Article'] ['title'], 'Post with Author saveAlled from comment' );
	...
	public function testSaveAllEmptyData() {
		...
		$model = new Article ();
		$result = $model->saveAll ( array (), array (
				'validate' => 'first' 
		) );
		$this->assertFalse ( empty ( $result ) );
	...
	public function testSaveAssociated() {
		...
		$this->assertEquals ( self::date (), $result [3] ['Author'] ['created'] );
		$this->assertEquals ( self::date (), $result [3] ['Author'] ['updated'] );
		unset ( $result [3] ['Post'] ['updated'], $result [3] ['Post'] ['created'], $result [3] ['Author'] ['updated'], $result [3] ['Author'] ['created'] );
		$this->assertEquals ( $expected, $result [3] );
		$this->assertEquals ( 4, count ( $result ) );
	...
	public function testSaveAssociatedAtomicFalseValidateFirstWithErrors() {
		...
		$Article->saveAssociated ( $data, array (
				'validate' => 'first',
				'atomic' => false 
		) );
	...
	public function testSaveAssociatedHasOne() {
		...
		$this->assertTrue ( $model->saveAssociated ( $data, array (
				'validate' => 'first' 
		) ) );
	}
	...
	public function testSaveAssociatedTransactionNoRollback() {
		...
		$this->getMock ( 'DboSource', array (
				...
				'create',
				'begin' 
		), array (), 'MockAssociatedTransactionDboSource', false );
		...
		$Post->saveAssociated ( $data, array (
				'validate' => true,
				'atomic' => true 
		) );
	}
	...
	public function testSaveManyNestedSaveMany() {
		...
		$this->assertTrue ( $TransactionManyTestModel->saveMany ( $data, array (
				'atomic' => true 
		) ) );
	}
	...
	public function testSaveManyValidation() {
		...
		$result = $TestModel->saveMany ( $data, array (
				'validate' => true,
				'atomic' => false 
		) );
		$this->assertEquals ( array (
		...
		$this->assertFalse ( $TestModel->saveMany ( $data, array (
				'validate' => 'first' 
		) ) );
	...
	public function testSaveAssociatedValidateFirst() {
		...
		$result = $model->Comment->saveAssociated ( $data, array (
				'validate' => 'first' 
		) );
		$this->assertFalse ( empty ( $result ) );
	...
	public function testSaveManyEmptyData() {
		...
		$this->loadFixtures ( 'Article', 'ProductUpdateAll', 'Comment', 'Attachment' );
		$model = new Article ();
		$result = $model->saveMany ( array (), array (
				'validate' => true 
		) );
		$this->assertFalse ( empty ( $result ) );
	...
	public function testSaveAssociatedEmptyData() {
		...
		$this->loadFixtures ( 'Article', 'ProductUpdateAll', 'Comment', 'Attachment' );
		$model = new Article ();
		$result = $model->saveAssociated ( array (), array (
				'validate' => true 
		) );
		$this->assertFalse ( empty ( $result ) );
	...
	public function testUpdateWithCalculation() {
		...
		$result = Hash::extract ( $model->find ( 'all', array (
				'fields' => 'count' 
		) ), '{n}.DataTest.count' );
		$this->assertEquals ( array (
				5,
		...
		$this->assertTrue ( $model->updateAll ( array (
				'count' => 'count + 2' 
		) ) );
		$result = Hash::extract ( $model->find ( 'all', array (
				'fields' => 'count' 
		...
		$this->assertTrue ( $model->updateAll ( array (
				'DataTest.count' => 'DataTest.count - 1' 
		) ) );
		$result = Hash::extract ( $model->find ( 'all', array (
				'fields' => 'count' 
		) ), '{n}.DataTest.count' );
	...
	public function testToggleBoolFields() {
		...
		$this->assertTrue ( $Post->updateAll ( array (
	...
	public function testUpdateAllWithJoins() {
		...
		$ProductUpdateAll->updateAll ( array (
				'name' => "'new product'" 
		), $conditions );
		$results = $ProductUpdateAll->find ( 'all', array (
				'conditions' => array (
						'ProductUpdateAll.name' => 'new product' 
				) 
		) );
		$expected = array (
				'0' => array (
	...
	public function testUpdateAllWithoutForeignKey() {
		...
		$resultsFkFalse = $ProductUpdateAll->find ( 'all', array (
				'conditions' => array (
						'ProductUpdateAll.name' => 'new product' 
				) 
		) );
	...
	public function testSaveAllFieldListHasMany() {
		...
		$TestModel->whitelist = array (
				'id' 
		);
		$TestModel->Comment->whitelist = array (
				'user_id' 
		);
	...
	public function testSaveAllFieldListHasOneAddFkToWhitelist() {
		...
		$Article->Featured->validate = array (
				'end_date' => 'notEmpty' 
		);
	...
	public function testSaveAllDeepFieldListValidateBelongsTo() {
		$this->loadFixtures ( 'Post', 'Author', 'Comment', 'Attachment', 'Article', 'User' );
		$TestModel = new Post ();
		$TestModel->Author->bindModel ( array (
				'hasMany' => array (
						'Comment' => array (
								'foreignKey' => 'user_id' 
						) 
	...
	public function testSaveAllDeepHasManyHasMany() {
		...
		$TestModel->Comment->unbindModel ( array (
				'hasOne' => array (
						'Attachment' 
				) 
		), false );
		$TestModel->Comment->bindModel ( array (
				'hasMany' => array (
						'Attachment' 
				) 
		), false );
	...
	public function testSaveAllDeepOrderHasManyHasMany() {
		...
		$result = $TestModel->saveAll ( array (
				'Article' => array (
						'id' => 2,
						'title' => 'Comment has its data after Attachment' 
				),
				'Comment' => array (
						array (
								'Attachment' => array (
