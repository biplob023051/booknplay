...
class SqlserverTest extends CakeTestCase {
	...
	public function testFields() {
		...
		$this->db->clearFieldMappings ();
		$result = $this->db->fields ( $this->model, null, array (
				'*',
				'AnotherModel.id',
				'AnotherModel.name' 
		) );
	...
	public function testDistinctFields() {
		$result = $this->db->fields ( $this->model, null, array (
				'DISTINCT Car.country_code' 
		) );
		$expected = array (
				'DISTINCT [Car].[country_code] AS [Car__country_code]' 
		);
		$this->assertEquals ( $expected, $result );
	...
	public function testDistinctWithLimit() {
		$this->db->read ( $this->model, array (
				'fields' => array (
						'DISTINCT SqlserverTestModel.city',
						'SqlserverTestModel.country' 
				),
				'limit' => 5 
		) );
	...
	public function testDescribe() {
		...
		$this->db->executeResultsStack = array (
	...
	public function testBuildIndex() {
		...
		$result = $this->db->buildIndex ( $indexes, 'items' );
		$expected = array (
				'ALTER TABLE items ADD CONSTRAINT client_id UNIQUE([client_id], [period_id]);' 
		);
		$this->assertEquals ( $expected, $result );
	}
	...
	public function testUpdateAllSyntax() {
		$fields = array (
				'SqlserverTestModel.client_id' => '[SqlserverTestModel].[client_id] + 1' 
		);
		$conditions = array (
				'SqlserverTestModel.updated <' => date ( '2009-01-01 00:00:00' ) 
		);
