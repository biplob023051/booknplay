...
class PostgresTest extends CakeTestCase {
	...
	public function testLastInsertIdMultipleInsert() {
		...
		$db1->execute ( "INSERT INTO {$table} (\"user\", password) VALUES ('mariano', '{$password}')" );
	...
	public function testRegexpOperatorConditionsParsing() {
		$this->assertSame ( ' WHERE "name" ~ \'[a-z_]+\'', $this->Dbo->conditions ( array (
				'name ~' => '[a-z_]+' 
		) ) );
		$this->assertSame ( ' WHERE "name" ~* \'[a-z_]+\'', $this->Dbo->conditions ( array (
				'name ~*' => '[a-z_]+' 
	...
	public function testCakeSchema() {
		...
		$schema->tables = array (
				'datatype_tests' => $result2 ['tables'] ['missing'] ['datatype_tests'] 
		);
	...
	public function testUpdateAllWithNonQualifiedConditions() {
		$this->loadFixtures ( 'Article' );
		$Article = new Article ();
		$result = $Article->updateAll ( array (
				'title' => "'Awesome'" 
		), array (
				'title' => 'Third Article' 
		) );
	...
	public function testTruncateStatements() {
		...
		$this->Dbo = $this->getMock ( 'Postgres', array (
				'execute' 
		), array (
				$db->config 
		) );
