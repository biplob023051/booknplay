...
class MysqlTest extends CakeTestCase {
	...
	public function testTinyintCasting() {
		...
		$this->assertTrue ( ( bool ) $this->model->save ( array (
				'bool' => 5,
				'small_int' => 5 
		) ) );
	...
	public function testIndexOnMySQL4Output() {
		...
		$mockDbo = $this->getMock ( 'Mysql', array (
				'connect',
				'_execute',
				'getVersion' 
		) );
		...
		$resultMock = $this->getMock ( 'PDOStatement', array (
				'fetch' 
		) );
		$mockDbo->expects ( $this->once () )->method ( '_execute' )->with ( 'SHOW INDEX FROM ' . $name )->will ( $this->returnValue ( $resultMock ) );
	...
	public function testGetCharsetNameCaching() {
		...
		$db->expects ( $this->exactly ( 1 ) )->method ( '_execute' )->with ( 'SELECT CHARACTER_SET_NAME FROM INFORMATION_SCHEMA.COLLATIONS WHERE COLLATION_NAME = ?', array (
				'utf8_unicode_ci' 
		) )->will ( $this->returnValue ( $queryResult ) );
		
		$queryResult->expects ( $this->once () )->method ( 'fetch' )->with ( PDO::FETCH_ASSOC )->will ( $this->returnValue ( array (
	...
	public function testVirtualFieldSeparators() {
		...
		$result = $this->Dbo->fields ( $model, null, array (
				'data',
				'other__field' 
		) );
		...
		$expected = array (
				'`BinaryTest`.`data`',
				'(SUM(id)) AS  `BinaryTest_$_other__field`' 
		);
		$this->assertEquals ( $expected, $result );
	}
	...
	public function testListSources() {
		...
		$queryResult = $this->getMock ( 'PDOStatement' );
		$db->expects ( $this->once () )->method ( '_execute' )->with ( 'SHOW TABLES FROM `cake`' )->will ( $this->returnValue ( $queryResult ) );
		$queryResult->expects ( $this->at ( 0 ) )->method ( 'fetch' )->will ( $this->returnValue ( array (
				'cake_table' 
		) ) );
		$queryResult->expects ( $this->at ( 1 ) )->method ( 'fetch' )->will ( $this->returnValue ( array (
				'another_table' 
		) ) );
		$queryResult->expects ( $this->at ( 2 ) )->method ( 'fetch' )->will ( $this->returnValue ( null ) );
	...
	public function testGetEncoding() {
		...
		$db->expects ( $this->once () )->method ( '_execute' )->with ( 'SHOW VARIABLES LIKE ?', array (
				'character_set_client' 
		) )->will ( $this->returnValue ( $queryResult ) );
		$result = new StdClass ();
		$result->Value = 'utf-8';
		$queryResult->expects ( $this->once () )->method ( 'fetchObject' )->will ( $this->returnValue ( $result ) );
	...
	public function testFieldDoubleEscaping() {
		...
		$this->Model = $this->getMock ( 'Article2', array (
				'getDataSource' 
		) );
		$this->Model->alias = 'Article';
		$this->Model->expects ( $this->any () )->method ( 'getDataSource' )->will ( $this->returnValue ( $test ) );
		...
		$test->expects ( $this->at ( 0 ) )->method ( 'execute' )->with ( 'SELECT `Article`.`id` FROM ' . $test->fullTableName ( 'articles' ) . ' AS `Article`   WHERE 1 = 1' );
	...
	public function testGenerateAssociationQuerySelfJoin() {
		$this->Dbo = $this->getMock ( 'Mysql', array (
				'connect',
				'_execute',
				'execute' 
		) );
	...
	protected function &_prepareAssociationQuery(Model $model, &$queryData, $binding) {
		...
		$result = array_merge ( array (
				'linkModel' => &$linkModel 
		), compact ( 'type', 'assoc', 'assocData', 'external' ) );
		return $result;
	}
	...
	public function testGenerateInnerJoinAssociationQuery() {
		...
		$this->Model = $this->getMock ( 'TestModel9', array (
				'getDataSource' 
		) );
		$this->Model->expects ( $this->any () )->method ( 'getDataSource' )->will ( $this->returnValue ( $test ) );
		
		$this->Model->TestModel8 = $this->getMock ( 'TestModel8', array (
				'getDataSource' 
		) );
		$this->Model->TestModel8->expects ( $this->any () )->method ( 'getDataSource' )->will ( $this->returnValue ( $test ) );
		
		$testModel8Table = $this->Model->TestModel8->getDataSource ()->fullTableName ( $this->Model->TestModel8 );
		
		$test->expects ( $this->at ( 0 ) )->method ( 'execute' )->with ( $this->stringContains ( '`TestModel9` LEFT JOIN ' . $testModel8Table ) );
		
		$test->expects ( $this->at ( 1 ) )->method ( 'execute' )->with ( $this->stringContains ( 'TestModel9` INNER JOIN ' . $testModel8Table ) );
	...
	public function testGenerateAssociationQuerySelfJoinWithConditions() {
		...
		$this->assertRegExp ( '/^SELECT\s+`Featured2`\.`id`, `Featured2`\.`article_id`, `Featured2`\.`category_id`, `Featured2`\.`name`,\s+' . '`ArticleFeatured2`\.`id`, `ArticleFeatured2`\.`title`, `ArticleFeatured2`\.`user_id`, `ArticleFeatured2`\.`published`\s+' . 'FROM\s+\S+`featured2` AS `Featured2`\s+LEFT JOIN\s+\S+`article_featured` AS `ArticleFeatured2`' . '\s+ON\s+\(`ArticleFeatured2`.`published` = \'Y\'\s+AND\s+`Featured2`\.`article_featured2_id` = `ArticleFeatured2`\.`id`\)' . '\s+WHERE\s+1\s+=\s+1\s*$/', $result );
	}
	...
	public function testGenerateAssociationQueryHasManyWithLimit() {
		...
		$result = $this->Dbo->generateAssociationQuery ( $this->Model, $params ['linkModel'], $params ['type'], $params ['assoc'], $params ['assocData'], $queryData, $params ['external'], $resultSet );
		$this->assertRegExp ( '/^SELECT\s+' . '`TestModel6`\.`id`, `TestModel6`\.`test_model5_id`, `TestModel6`\.`name`, `TestModel6`\.`created`, `TestModel6`\.`updated`\s+' . 'FROM\s+\S+`test_model6` AS `TestModel6`\s+WHERE\s+' . '`TestModel6`.`test_model5_id`\s+=\s+\({\$__cakeID__\$}\)\s*' . 'LIMIT \d*' . '\s*$/', $result );
		
		$result = $this->Dbo->generateAssociationQuery ( $this->Model, $null, null, null, null, $queryData, false, $null );
		$this->assertRegExp ( '/^SELECT\s+' . '`TestModel5`\.`id`, `TestModel5`\.`test_model4_id`, `TestModel5`\.`name`, `TestModel5`\.`created`, `TestModel5`\.`updated`\s+' . 'FROM\s+\S+`test_model5` AS `TestModel5`\s+WHERE\s+' . '(?:\()?\s*1 = 1\s*(?:\))?' . '\s*$/', $result );
	}
	...
	public function testGenerateAssociationQueryHasManyWithFields() {
		...
		$this->Model->hasMany ['TestModel6'] ['fields'] = array (
				'name' 
		);
	...
	public function testStringConditionsParsing() {
		...
		$conditions = array (
				'Company.name similar to ' => 'a word' 
		);
		$result = $this->Dbo->conditions ( $conditions );
	...
	public function testParenthesisInArrayConditions() {
		...
		$result = $this->Dbo->conditions ( array (
				'Member.name' => '(mariano word) iglesias' 
		) );
		$this->assertRegExp ( '/^\s+WHERE\s+`Member`.`name`\s+=\s+\'\(mariano word\) iglesias\'$/', $result );
		...
		$result = $this->Dbo->conditions ( array (
				'Member.name' => 'Mariano Iglesias (mariano.iglesias)' 
		) );
		$this->assertRegExp ( '/^\s+WHERE\s+`Member`.`name`\s+=\s+\'Mariano Iglesias \(mariano.iglesias\)\'$/', $result );
		
		$result = $this->Dbo->conditions ( array (
				'Member.name' => 'Mariano Iglesias (mariano.iglesias) CakePHP' 
		) );
		$this->assertRegExp ( '/^\s+WHERE\s+`Member`.`name`\s+=\s+\'Mariano Iglesias \(mariano.iglesias\) CakePHP\'$/', $result );
		
		$result = $this->Dbo->conditions ( array (
				'Member.name' => '(mariano.iglesias) CakePHP' 
		) );
		$this->assertRegExp ( '/^\s+WHERE\s+`Member`.`name`\s+=\s+\'\(mariano.iglesias\) CakePHP\'$/', $result );
	}
	...
	public function testArrayConditionsParsing() {
		...
		$result = $this->Dbo->conditions ( array (
				'Post.modified >=' => 'DATE_SUB(NOW(), INTERVAL 7 DAY)' 
		) );
		...
		$result = $this->Dbo->conditions ( array (
				'TestModel.field =' => 'gribe$@()lu' 
		) );
		$expected = " WHERE `TestModel`.`field` = 'gribe$@()lu'";
		$this->assertEquals ( $expected, $result );
		
		$conditions ['NOT'] = array (
				'Listing.expiration BETWEEN ? AND ?' => array (
						"1",
						"100" 
				) 
		);
		...
		$result = $this->Dbo->conditions ( array (
				'MD5(CONCAT(Reg.email,Reg.id))' => 'blah' 
		) );
		$expected = " WHERE MD5(CONCAT(`Reg`.`email`,`Reg`.`id`)) = 'blah'";
		$this->assertEquals ( $expected, $result );
		
		$result = $this->Dbo->conditions ( array (
				'MD5(CONCAT(Reg.email,Reg.id))' => array (
						'blah',
						'blahblah' 
				) 
		) );
		...
		$conditions = array (
				'`Correction`.`source` collate utf8_bin' => array (
						'kiwi',
						'pear' 
				) 
		);
		...
		$result = $this->Dbo->conditions ( array (
				'ASCII(SUBSTRING(keyword, 1, 1)) BETWEEN ? AND ?' => array (
						65,
						90 
				) 
		) );
	...
	public function testFieldParsing() {
		...
		$expected = array (
				'SUM(`Thread`.`unread_buyer`) AS `sum_unread_buyer`' 
		);
		$this->assertEquals ( $expected, $result );
	...
	public function testCalculations() {
		...
		$result = $this->Dbo->calculate ( $this->Model, 'count', array (
				'id' 
		) );
		...
		$result = $this->Dbo->calculate ( $this->Model, 'max', array (
				'id' 
		) );
		...
		$result = $this->Dbo->calculate ( $this->Model, 'min', array (
				'`Model`.`id`',
				'id' 
		) );
	...
	public function testBuildIndex() {
		...
		$result = $this->Dbo->buildIndex ( $data );
		$expected = array (
				'FULLTEXT KEY `MyFtIndex` (`name`, `description`)' 
		);
		$this->assertEquals ( $expected, $result );
		...
		$result = $this->Dbo->buildIndex ( $data );
		$expected = array (
				'KEY `MyMultiTextIndex` (`text_field1`(20), `text_field2`(20))' 
		);
		$this->assertEquals ( $expected, $result );
	}
	...
	public function testHasAny() {
		...
		$this->Model = $this->getMock ( 'TestModel', array (
				'getDataSource' 
		) );
		...
		$this->Dbo->expects ( $this->at ( 1 ) )->method ( 'execute' )->with ( 'SELECT COUNT(`TestModel`.`id`) AS count FROM ' . $modelTable . ' AS `TestModel` WHERE `TestModel`.`name` = \'harry\'' );
		$this->Dbo->expects ( $this->at ( 2 ) )->method ( 'execute' )->with ( 'SELECT COUNT(`TestModel`.`id`) AS count FROM ' . $modelTable . ' AS `TestModel` WHERE 1 = 1' );
		
		$this->Dbo->hasAny ( $this->Model, array (
				'TestModel.name' => 'harry' 
		) );
		$this->Dbo->hasAny ( $this->Model, array () );
	}
	...
	public function testVirtualFields() {
		...
		$Article->virtualFields = array (
				'this_moment' => 'NOW()',
				'two' => '1 + 1',
				'comment_count' => 'SELECT COUNT(*) FROM ' . $commentsTable . ' WHERE Article.id = ' . $commentsTable . '.article_id' 
		);
	...
	public function testVirtualFieldsInCalculate() {
		...
		$result = $this->Dbo->calculate ( $Article, 'count', array (
				'this_moment' 
		) );
		$expected = 'COUNT(NOW()) AS `count`';
		$this->assertEquals ( $expected, $result );
		
		$result = $this->Dbo->calculate ( $Article, 'max', array (
				'comment_count' 
		) );
	...
	public function testExecute() {
		...
		$result = $this->Dbo->execute ( $query, array (
				'log' => false 
		) );
	...
	public function testVirtualFieldsFetch() {
		...
		$Article = ClassRegistry::init ( 'Article' );
		$Article->virtualFields = array (
				'comment_count' => 'SELECT COUNT(*) FROM ' . $this->Dbo->fullTableName ( 'comments' ) . ' WHERE Article.id = ' . $this->Dbo->fullTableName ( 'comments' ) . '.article_id' 
		);
		
		$conditions = array (
				'comment_count >' => 2 
		);
		$query = 'SELECT ' . implode ( ',', $this->Dbo->fields ( $Article, null, array (
				'id',
				'comment_count' 
	...
	public function testVirtualFieldsComplexRead() {
		...
		$Article = ClassRegistry::init ( 'Article' );
		$Article->virtualFields = array (
				'comment_count' => 'SELECT COUNT(*) FROM ' . $commentTable . ' AS Comment WHERE Article.id = Comment.article_id' 
		);
	...
	public function testUpdateStatements() {
		...
		$this->Dbo->expects ( $this->at ( 0 ) )->method ( 'execute' )->with ( "UPDATE `$db`.`articles` SET `field1` = 'value1'  WHERE 1 = 1" );
		
		$this->Dbo->expects ( $this->at ( 1 ) )->method ( 'execute' )->with ( "UPDATE `$db`.`articles` AS `Article` LEFT JOIN `$db`.`users` AS `User` ON " . "(`Article`.`user_id` = `User`.`id`)" . " SET `Article`.`field1` = 2  WHERE 2=2" );
		
		$this->Dbo->expects ( $this->at ( 2 ) )->method ( 'execute' )->with ( "UPDATE `$db`.`articles` AS `Article` LEFT JOIN `$db`.`users` AS `User` ON " . "(`Article`.`user_id` = `User`.`id`)" . " SET `Article`.`field1` = 'value'  WHERE `index` = 'val'" );
		
		$Article = new Article ();
		
		$this->Dbo->update ( $Article, array (
				'field1' 
		), array (
				'value1' 
		) );
	...
	public function testDeleteStatements() {
		...
		$this->Dbo->expects ( $this->at ( 0 ) )->method ( 'execute' )->with ( "DELETE  FROM `$db`.`articles`  WHERE 1 = 1" );
		
		$this->Dbo->expects ( $this->at ( 1 ) )->method ( 'execute' )->with ( "DELETE `Article` FROM `$db`.`articles` AS `Article` LEFT JOIN `$db`.`users` AS `User` " . "ON (`Article`.`user_id` = `User`.`id`)" . "  WHERE 1 = 1" );
		
		$this->Dbo->expects ( $this->at ( 2 ) )->method ( 'execute' )->with ( "DELETE `Article` FROM `$db`.`articles` AS `Article` LEFT JOIN `$db`.`users` AS `User` " . "ON (`Article`.`user_id` = `User`.`id`)" . "  WHERE 2=2" );
		$Article = new Article ();
