...
class DboSourceTest extends CakeTestCase {
	...
	public function testMagicMethodQuerying() {
		$result = $this->db->query ( 'findByFieldName', array (
				'value' 
		), $this->Model );
		$expected = array (
				'first',
		...
		$result = $this->db->query ( 'findByFindBy', array (
				'value' 
		), $this->Model );
		$expected = array (
				'first',
		...
		$result = $this->db->query ( 'findAllByFieldName', array (
				'value' 
		), $this->Model );
		$expected = array (
				'all',
		...
		$result = $this->db->query ( 'findAllById', array (
				'a' 
		), $this->Model );
		$expected = array (
				'all',
	...
	public function testGetLogParams() {
		$this->testDb->logQuery ( 'Query 1', array (
				1,
				2,
				'abc' 
		) );
		$this->testDb->logQuery ( 'Query 2', array (
				'field1' => 1,
				'field2' => 'abc' 
		) );
	...
	public function testQueryAssociationUnneededQueries() {
		...
		$Comment->find ( 'all', array (
				'recursive' => 2 
		) ); // ensure Model descriptions are saved
		$this->db->getLog ();
	...
	public function testFieldsUsingMethodCache() {
		...
		$Article = ClassRegistry::init ( 'Article' );
		$this->testDb->fields ( $Article, null, array (
				'title',
				'body',
				'published' 
		) );
	...
	public function testFieldsCacheKeyWithDatasourceChange() {
		...
		$first = $ds->fields ( $Article, null, array (
				'title',
				'body',
				'published' 
		) );
		...
		$ds = $Article->getDataSource ();
		$ds->cacheMethods = true;
		$second = $ds->fields ( $Article, null, array (
				'title',
				'body',
				'published' 
		) );
	...
	public function testLastError() {
		...
		$stmt->expects ( $this->any () )->method ( 'errorInfo' )->will ( $this->returnValue ( array (
				'',
				'something',
				'bad' 
		) ) );
	...
	public function testBuildStatementDefaults() {
		$conn = $this->getMock ( 'MockPDO', array (
				'quote' 
		) );
	...
	public function testBuildJoinStatement($join, $expected) {
		$db = $this->getMock ( 'DboTestSource', array (
				'getSchemaName' 
		) );
		$db->expects ( $this->any () )->method ( 'getSchemaName' )->will ( $this->returnValue ( 'cakephp' ) );
