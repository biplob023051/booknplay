...
class ModelIntegrationTest extends BaseModelTest {
	...
	public function testDynamicBehaviorAttachment() {
		...
		$TestModel->Behaviors->attach ( 'Tree', array (
				'left' => 'left_field',
				'right' => 'right_field' 
		) );
	...
	/**
	 * Tests cross database joins.
	 * Requires $test and $test2 to both be set in DATABASE_CONFIG
	...
	public function testHABTMKeepExistingWithThreeDbs() {
		...
		$larry ['Guild'] ['Guild'] = array (
				1,
				3 
		); // larry joins another guild
		$larry ['Armor'] ['Armor'] = array (
				2,
				3 
		); // purchases chainmail
		$Player->save ( $larry );
	...
	public function testTablePrefixSwitching() {
		ConnectionManager::create ( 'database1', array_merge ( $this->db->config, array (
				'prefix' => 'aaa_' 
		) ) );
		ConnectionManager::create ( 'database2', array_merge ( $this->db->config, array (
				'prefix' => 'bbb_' 
		) ) );
	...
	public function testAutoConstructAssociations() {
		...
		$TestFakeModel = new ArticleFeatured ( array (
				'table' => false 
		) );
	...
	/**
	 * test creating associations with plugins.
	 * Ensure a double alias isn't created
	 *
	 * @return void
	 */
	...
	public function testHasMethod() {
		...
		$Article->Behaviors->expects ( $this->at ( 0 ) )->method ( 'hasMethod' )->will ( $this->returnValue ( true ) );
		
		$Article->Behaviors->expects ( $this->at ( 1 ) )->method ( 'hasMethod' )->will ( $this->returnValue ( false ) );
	...
	public function testMultischemaFixture() {
		$config = ConnectionManager::enumConnectionObjects ();
		$this->skipIf ( $this->db instanceof Sqlite, 'This test is not compatible with Sqlite.' );
		$this->skipIf ( ! isset ( $config ['test'] ) || ! isset ( $config ['test2'] ), 'Primary and secondary test databases not configured, skipping cross-database join tests. To run these tests define $test and $test2 in your database configuration.' );
		...
		$guilds = $Player->Guild->find ( 'all', array (
				'recursive' => - 1 
		) );
		$guildsPlayers = $Player->GuildsPlayer->find ( 'all', array (
				'recursive' => - 1 
		) );
	...
	public function testMultischemaFixtureWithThreeDatabases() {
		...
		$armorsPlayers = $Player->ArmorsPlayer->find ( 'all', array (
				'recursive' => - 1 
		) );
