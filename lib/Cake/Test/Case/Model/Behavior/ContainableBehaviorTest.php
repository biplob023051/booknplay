...
class ContainableBehaviorTest extends CakeTestCase {
	...
	public function testContainments() {
		...
		$this->assertEquals ( array (
				'id' 
		), Hash::extract ( $r, 'ArticleFeatured.keep.Featured.fields' ) );
		...
		$this->assertEquals ( array (
				'comment',
				'published' 
		), Hash::extract ( $r, 'Article.keep.Comment.fields' ) );
		$this->assertEquals ( array (
				'user' 
		), Hash::extract ( $r, 'Article.keep.User.fields' ) );
		$this->assertTrue ( Set::matches ( '/Comment/keep/Attachment', $r ) );
		$this->assertEquals ( array (
				'attachment' 
		), Hash::extract ( $r, 'Comment.keep.Attachment.fields' ) );
		...
		$this->assertEquals ( array (
				'Attachment.id >' => 1 
		), $r ['Comment'] ['keep'] ['Attachment'] ['conditions'] );
	...
	public function testInvalidContainments() {
		$this->_containments ( $this->Article, array (
				'Comment',
				'InvalidBinding' 
		) );
	}
	...
	public function testInvalidContainmentsNoNotices() {
		$this->Article->Behaviors->attach ( 'Containable', array (
				'notices' => false 
		) );
		$this->_containments ( $this->Article, array (
				'Comment',
	...
	public function testFindEmbeddedThirdLevelNonReset() {
		...
		$this->_assertBindings ( $this->User, array (
				'hasMany' => array (
						'ArticleFeatured' 
				) 
		) );
		...
		if (! empty ( $result )) {
			foreach ( $result as $i => $article ) {
				foreach ( $article ['Comment'] as $j => $comment ) {
					$result [$i] ['Comment'] [$j] = array_diff_key ( $comment, array (
							'id' => true 
					) );
				}
			}
	...
	public function testResetAddedAssociation() {
		...
		$this->JoinA->JoinB->find ( 'all', array (
				'contain' => array (
						'JoinA' 
				) 
		) );
		$this->JoinA->bindModel ( array (
				'hasOne' => array (
						'JoinAsJoinC' => array (
								'joinTable' => 'as_cs' 
						) 
	...
	public function testAutoFieldsWithRecursiveNegativeOne() {
		...
		$result = $this->Article->field ( 'title', array (
	...
	protected function _containments($Model, $contain = array()) {
		...
		foreach ( $result as $i => $containment ) {
			$result [$i] = array_diff_key ( $containment, array (
					'instance' => true 
			) );
		}
		return $result;
