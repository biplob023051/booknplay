...
class TranslateBehaviorTest extends CakeTestCase {
	...
	public function testTranslateModel() {
		...
		$TestModel->Behaviors->attach ( 'Translate', array (
				'title' 
		) );
	...
	public function testLocaleFalseAssociations() {
		...
		$TestModel->hasMany ['Title'] ['fields'] = $TestModel->hasMany ['Content'] ['fields'] = array (
				'content' 
		);
	...
	public function testLocaleSingleWithConditions() {
		...
		$result = $TestModel->find ( 'all', array (
				'conditions' => "TranslatedItem.slug = 'first_translated'" 
		) );
		$expected = array (
				array (
						'TranslatedItem' => array (
	...
	public function testSaveDeleteIgnoreAlias() {
		...
		$TestModel = new TranslatedItem ( array (
				'alias' => 'SomethingElse' 
		) );
		$TestModel->locale = 'spa';
		$data = array (
		...
		$result = $TestModel->read ();
		$expected = array (
				$TestModel->alias => array_merge ( $data, array (
						'id' => $id,
						'locale' => 'spa' 
				) ) 
		);
	...
	public function testSaveUpdate() {
		...
		$expected = array (
				'TranslatedItem' => array_merge ( $oldData, $newData, array (
						'locale' => 'spa' 
				) ) 
		);
		$this->assertEquals ( $expected, $result );
	...
	public function testMultipleUpdate() {
		...
		$TestModel->bindTranslation ( array (
				'title',
				'content' 
		), false );
	}
	...
	public function testSaveAllTranslatedAssociations() {
		...
		$result = $Model->TranslatedItem->find ( 'all', array (
				'conditions' => array (
						'translated_article_id' => $Model->id 
				) 
		) );
	...
	public function testValidation() {
		...
		), $TestModel->validationErrors ['title'] );
