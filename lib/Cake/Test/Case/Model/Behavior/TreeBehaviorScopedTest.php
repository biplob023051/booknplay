...
class TreeBehaviorScopedTest extends CakeTestCase {
	...
	public function testStringScope() {
		...
		$this->Tree->Behaviors->attach ( 'Tree', array (
				'scope' => 'FlagTree.flag = 1' 
		) );
	...
	public function testMoveUpWithScope() {
		$this->Ad = new Ad ();
		$this->Ad->Behaviors->attach ( 'Tree', array (
				'scope' => 'Campaign' 
		) );
		$this->Ad->moveUp ( 6 );
		...
		$this->assertEquals ( array (
				6,
				5 
		), Hash::extract ( $result, '{n}.Ad.id' ) );
		$this->assertEquals ( array (
				2,
				2 
		), Hash::extract ( $result, '{n}.Campaign.id' ) );
	}
	...
	public function testTranslatingTree() {
		...
		$this->Tree->Behaviors->attach ( 'Translate', array (
				'title' 
		) );
