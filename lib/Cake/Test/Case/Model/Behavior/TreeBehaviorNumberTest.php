...
class TreeBehaviorNumberTest extends CakeTestCase {
	...
	public function testDetectEqualLftsRghts() {
		...
		$this->Tree->updateAll ( array (
				$leftField => $this->Tree->escapeField ( $leftField ) . ' -1' 
		), array (
				$leftField . ' >' => $result [$modelClass] [$leftField] 
		) );
		$this->Tree->updateAll ( array (
				$rightField => $this->Tree->escapeField ( $rightField ) . ' -1' 
		), array (
				$rightField . ' >' => $result [$modelClass] [$leftField] 
		) );
	...
	public function testAddMiddle() {
		...
		$children = $this->Tree->children ( $data [$modelClass] ['id'], true, array (
				'name' 
		) );
		$expects = array (
				array (
	...
	public function testMovePromote() {
		...
		$parent = $this->Tree->find ( 'first', array (
				'conditions' => array (
	...
	public function testInsertWithWhitelist() {
		...
		$result = $this->Tree->findByName ( 'testAddOrphan', array (
				'name',
				$parentField,
				$leftField,
				$rightField 
	...
	public function testMoveUpSuccess() {
		...
		$parent = $this->Tree->findByName ( '1. Root', array (
				'id' 
		) );
		$this->Tree->id = $parent [$modelClass] ['id'];
		$result = $this->Tree->children ( null, true, array (
				'name' 
		) );
		$expected = array (
				array (
	...
	public function testRemove() {
		...
		$children = $this->Tree->children ( $result [$modelClass] [$parentField], true, array (
				'name' 
		) );
		$expects = array (
				array (
		...
		$topNodes = $this->Tree->children ( false, true, array (
				'name' 
		) );
		$expects = array (
				array (
	...
	public function testRemoveNoChildren() {
		...
		$nodes = $this->Tree->find ( 'list', array (
				'order' => $leftField 
		) );
		$expects = array (
				1 => '1. Root',
	...
	public function testChildren() {
		...
		$direct = $this->Tree->children ( null, true, array (
				'id',
				'name',
				$parentField,
				$leftField,
		...
		$total = $this->Tree->children ( null, null, array (
				'id',
				'name',
				$parentField,
				$leftField,
	...
	public function testGetParentNode() {
		...
		$result = $this->Tree->getParentNode ( null, array (
				'name' 
		) );
		$expects = array (
				$modelClass => array (
	...
	public function testGetPath() {
		...
		$result = $this->Tree->getPath ( null, array (
				'name' 
		) );
		$expects = array (
				array (
	...
	public function testReorderTree() {
		...
		$data = $this->Tree->find ( 'first', array (
				...
				'conditions' => array (
						$modelClass . '.name' => '1.3.2.2' 
				) 
		) );
		$this->Tree->moveDown ( $data [$modelClass] ['id'] );
		
		$unsortedNodes = $this->Tree->find ( 'list', array (
				'order' => $leftField 
		) );
		...
		$this->Tree->reorder ();
		$sortedNodes = $this->Tree->find ( 'list', array (
				'order' => $leftField 
		) );
	...
	public function testGenerateTreeListFormatting() {
		...
		$result = $this->Tree->generateTreeList ( null, "{n}.$modelClass.id", array (
				'%s - %s',
				"{n}.$modelClass.id",
				"{n}.$modelClass.name" 
		) );
	...
	public function testArraySyntax() {
		...
		$this->Tree = new $modelClass ();
		$this->Tree->initialize ( 3, 3 );
		$this->assertSame ( $this->Tree->childCount ( 2 ), $this->Tree->childCount ( array (
				'id' => 2 
		) ) );
		$this->assertSame ( $this->Tree->getParentNode ( 2 ), $this->Tree->getParentNode ( array (
				'id' => 2 
		) ) );
		$this->assertSame ( $this->Tree->getPath ( 4 ), $this->Tree->getPath ( array (
				'id' => 4 
		) ) );
	}
