...
class ModelDeleteTest extends BaseModelTest {
	...
	public function testDeleteHabtmReferenceWithConditions() {
		...
		$Portfolio->hasAndBelongsToMany ['Item'] ['conditions'] = array (
				'ItemsPortfolio.item_id >' => 1 
		);
	...
	public function testDeleteAll() {
		...
		$result = $TestModel->deleteAll ( array (
				'Article.published' => 'N' 
		) );
		$this->assertTrue ( $result );
	...
	public function testDeleteAllUnknownColumn() {
		...
		$TestModel = new Article ();
		$result = $TestModel->deleteAll ( array (
				'Article.non_existent_field' => 999 
		) );
	...
	public function testDeleteAllFailedFind() {
		$this->loadFixtures ( 'Article' );
		$this->getMock ( 'Article', array (
				'find' 
		), array (), 'ArticleDeleteAll' );
		
		$TestModel = new ArticleDeleteAll ();
		$TestModel->expects ( $this->once () )->method ( 'find' )->will ( $this->returnValue ( null ) );
	...
	public function testDeleteDependent() {
		...
		$result = $Bidding->BiddingMessage->find ( 'all', array (
				'order' => array (
						'BiddingMessage.name' => 'ASC' 
				) 
		) );
		$expected = array (
				array (
