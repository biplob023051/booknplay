...
class ModelValidationTest extends BaseModelTest {
	...
	public function testValidationParams() {
		...
		$expected = array (
				'title' => array (
						'This field will *never* validate! Muhahaha!' 
				) 
		);
	...
	/**
	 * Test that invalidFields() integrates well with save().
	 * And that fieldList can be an empty type.
	 *
	 * @return void
	 */
	...
	public function testValidates() {
		...
		$TestModel->validate ['modified'] = array (
				'allowEmpty' => true,
				'rule' => 'date' 
		);
	...
	public function testValidatesWithAssociations() {
		...
		$count = $Something->find ( 'count', array (
				'conditions' => array (
						'Something.id' => $data ['Something'] ['id'] 
				) 
		) );
		...
		$joinRecords = $JoinThing->find ( 'count', array (
				'conditions' => array (
						'JoinThing.something_id' => $data ['Something'] ['id'] 
				) 
		) );
	...
	public function testValidatesWithModelsAndSaveAll() {
		...
		$Something->create ();
		$result = $Something->saveAll ( $data, array (
				'validate' => 'only' 
		) );
		$this->assertFalse ( $result );
	...
	public function testValidatesWithModelsAndSaveAllWithoutId() {
		...
		$Author->create ();
		$result = $Author->saveAll ( $data, array (
				'validate' => 'only' 
		) );
		$this->assertTrue ( $result );
		...
		$TestModel->validate = array (
				'title' => array (
						...
						'between' => array (
								...
								'message' => array (
										'You may enter up to %s chars (minimum is %s chars)',
										14,
										6 
								) 
						) 
	...
	public function testValidationMessageTranslation() {
		...
		App::build ( array (
				'Locale' => array (
						CAKE . 'Test' . DS . 'test_app' . DS . 'Locale' . DS 
				) 
		), App::RESET );
	...
	public function testValidateAssociated() {
		...
		$result = $TestModel->validateAssociated ( $record, array (
				'fieldList' => $fieldList 
		) );
		$this->assertTrue ( $result );
		$this->assertEmpty ( $TestModel->validationErrors );
		...
		$result = $model->saveAll ( $data, array (
				'validate' => 'only' 
		) );
		$this->assertFalse ( $result );
	...
	public function testAddRule() {
		...
		$Validator->add ( 'other', 'numeric', array (
				'rule' => 'numeric',
				'allowEmpty' => false 
		) );
		$Validator->add ( 'other', 'range', array (
				'rule' => array (
						'between',
						1,
						5 
	...
	public function testValidateCallbacks() {
		$TestModel = $this->getMock ( 'Article', array (
				'beforeValidate',
				'afterValidate' 
		) );
	...
	public function testValidateFirstWithBeforeValidate() {
		...
		$this->assertEquals ( 'foo', $model->field ( 'title', array (
				'body' => 'foo2' 
		) ) );
		$this->assertEquals ( 'foo', $model->field ( 'title', array (
				'body' => 'foo3' 
	...
	public function testValidateFirstAssociatedWithBeforeValidate() {
		...
		$result = $user->saveAll ( $data, array (
				'validate' => 'first' 
		) );
		$this->assertTrue ( $result );
	...
	public function testCustomMethodWithEmptyValue() {
		...
		$model = $this->getMock ( 'Article', array (
				'isLegit' 
		) );
		$model->validate = array (
				'title' => array (
		...
		$model->expects ( $this->once () )->method ( 'isLegit' )->will ( $this->returnValue ( false ) );
	...
	public function testValidateAssociatedAtomicFalseDeepTrueWithErrors() {
		...
		$result = $Attachment->validateAssociated ( $data, array (
				'atomic' => false,
				'deep' => true 
		) );
	...
	public function testValidateManyAtomicFalseDeepTrueWithErrors() {
		...
		$Article->validateMany ( $data, array (
				'atomic' => false,
				'deep' => true 
		) );
