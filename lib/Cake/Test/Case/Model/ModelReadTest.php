...
class ModelReadTest extends BaseModelTest {
	...
	public function testFetchingNonUniqueFKJoinTableRecords() {
		...
		$result = $Something->JoinThing->find ( 'all', array (
				'conditions' => array (
						'something_else_id' => 2 
				) 
		) );
	...
	public function testPreparedQuery() {
		...
		$this->assertTrue ( isset ( $result [0] [$this->db->fullTableName ( 'articles', false, false )] ) || isset ( $result [0] [0] ) );
		$result = $this->db->getQueryCache ( $query, $params );
		...
		$result = $Article->query ( $query, $params );
		$this->assertTrue ( is_array ( $result ) );
		$this->assertTrue ( isset ( $result [0] [$this->db->fullTableName ( 'articles', false, false )] ['title'] ) || isset ( $result [0] [0] ['title'] ) );
	...
	public function testRecursiveUnbind() {
		...
		$result = $TestModel->Parent->unbindModel ( array (
				'hasOne' => array (
						'Sample' 
				) 
		) );
		...
		$result = $TestModel->Sample->unbindModel ( array (
				'belongsTo' => array (
						'Apple' 
				) 
		) );
	...
	public function testAfterFindUnset() {
		...
		$this->assertTrue ( empty ( $result [0] ['ModifiedComment'] ), 'Zeroith row should be removed by afterFind' );
	}
	...
	public function testFindThreaded() {
		...
		$result = $Model->find ( 'threaded', array (
				'parent' => 'mother_id' 
		) );
		$expected = array (
				array (
	...
	public function testFindAllThreaded() {
		...
		$result = $TestModel->find ( 'threaded', array (
				'conditions' => array (
						'Category.name LIKE' => 'Category 1.1%' 
				) 
		) );
		$expected = array (
				array (
	...
	public function testBindUnbind() {
		...
		$result = $TestModel->bindModel ( array (
				'hasMany' => array (
						'Comment' => array (
								'className' => 'Comment',
								'conditions' => 'Comment.published = \'Y\'' 
						) 
				) 
		) );
		$this->assertTrue ( $result );
	...
	public function testCallbackSourceChangeUnknownDatasource() {
		...
		$TestModel = new Post ();
		$this->assertFalse ( $TestModel->find ( 'all', array (
				'connection' => 'foo' 
		) ) );
	}
	...
	public function testHabtmFinderQuery() {
		...
		$sql = $this->db->buildStatement ( array (
	...
	public function testGenerateFindList() {
		...
		$result = Hash::combine ( $TestModel->find ( 'all', array (
				'order' => 'Article.title ASC',
				'fields' => array (
						'id',
						'title' 
	...
	public function testVirtualFields() {
		...
		if (! ($this->db instanceof Sqlserver)) {
			$Post->Author->virtualFields = array (
					'false' => '1 = 2' 
			);
			$result = $Post->find ( 'first' );
			$this->assertEquals ( 2, $result ['Post'] ['two'] );
		...
		ClassRegistry::flush ();
		$Writing = ClassRegistry::init ( array (
				'class' => 'Post',
				'alias' => 'Writing' 
		) );
		$Writing->virtualFields = array (
				'two' => "1 + 1" 
		);
	...
	public function testVirtualFieldsOrder() {
		...
		$Post = ClassRegistry::init ( 'Post' );
		$Post->virtualFields = array (
				'other_field' => '10 - Post.id' 
		);
		$result = $Post->find ( 'list', array (
				'order' => array (
						'Post.other_field' => 'ASC' 
				) 
		) );
		$expected = array (
				'3' => 'Third Post',
	...
	public function testNotInArrayWithOneValue() {
		...
		$result = $Article->find ( 'all', array (
				'conditions' => array (
						'Article.id NOT' => array (
								1 
						) 
				) 
		) );
