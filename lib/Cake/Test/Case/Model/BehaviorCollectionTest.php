	...
	public function beforeValidate(Model $model) {
		...
		switch ($settings ['validate']) {
			...
			case 'test' :
				return null;
			case 'whitelist' :
				$this->_addToWhitelist ( $model, array (
						'name' 
				) );
				return true;
			case 'stop' :
...
class BehaviorCollectionTest extends CakeTestCase {
	...
	public function testLoadAlias() {
		...
		$Apple->Behaviors->load ( 'Test', array (
				'className' => 'TestAlias',
				'somesetting' => true 
		) );
		$this->assertSame ( array (
				'Test' 
		), $Apple->Behaviors->loaded () );
		...
		$this->assertTrue ( $Apple->Behaviors->load ( 'SomeOther', array (
				'className' => 'TestPlugin.TestPluginPersisterOne' 
		) ) );
	...
	public function testBehaviorBinding() {
		...
		$Apple->Behaviors->attach ( 'Test', array (
				'key' => 'value' 
		) );
		$this->assertSame ( array (
				'Test' 
		...
		$this->assertEquals ( array (
				'priority',
				'Apple' 
		), array_keys ( $Apple->Behaviors->Test->settings ) );
		
		$this->assertSame ( $Apple->Sample->Behaviors->loaded (), array () );
		$Apple->Sample->Behaviors->attach ( 'Test', array (
				'key2' => 'value2' 
		) );
		$this->assertSame ( $Apple->Sample->Behaviors->loaded (), array (
				'Test' 
		...
		$this->assertSame ( $Apple->Sample->Behaviors->Test->settings, $Apple->Behaviors->Test->settings );
		...
		$Apple->Child->Behaviors->attach ( 'Test', array (
				'key' => 'value',
				'key2' => 'value2',
				'key3' => 'value3',
				'beforeFind' => 'off' 
		...
		$Apple->Parent->Behaviors->attach ( 'Test', array (
				'key' => 'value',
				'key2' => 'value2',
				'key3' => 'value3',
				'beforeFind' => 'off' 
		...
		$Apple->Behaviors->attach ( 'Plugin.Test', array (
				'key' => 'new value' 
		) );
		$expected = array (
				'beforeFind' => 'off',
		...
		$expected = array_merge ( $current, array (
				'mangle' => 'trigger mangled' 
		) );
		$Apple->Behaviors->attach ( 'Test', array (
				'mangle' => 'trigger' 
		) );
		...
		$expected = array_merge ( $current, array (
				'mangle' => 'trigger mangled mangled' 
		) );
	...
	public function testBehaviorToggling() {
		...
		$Apple->Behaviors->init ( 'Apple', array (
				'Test' => array (
						'key' => 'value' 
				) 
		) );
	...
	public function testBehaviorSaveCallbacks() {
		...
		$Sample->Behaviors->attach ( 'Test', array (
				'beforeSave' => 'on' 
		) );
		$Sample->create ();
		...
		$expected = Hash::merge ( $record, array (
				'Sample' => array (
						'aftersave' => 'modified after on create' 
				) 
		) );
		$Sample->create ();
		$result = $Sample->save ( $record );
		...
		$expected = Hash::merge ( $record, array (
				'Sample' => array (
						'name' => 'sample99 modified before modified after on create' 
				) 
		) );
		$Sample->create ();
		$result = $Sample->save ( $record );
		...
		$expected = Hash::merge ( $record2, array (
				'Sample' => array (
						'aftersave' => 'modified after' 
				) 
		) );
		$Sample->create ();
	...
	public function testBehaviorValidateMethods() {
		...
		$result = $Apple->save ( array (
				'name' => 'Genetically Modified Apple',
				'color' => 'Orange' 
		) );
		$this->assertEquals ( array (
				'name',
	...
	public function testBehaviorAttachAndDetach() {
		...
		$Sample->Behaviors->trigger ( 'beforeTest', array (
				&$Sample 
		) );
	}
