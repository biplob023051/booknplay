...
class ComponentCollectionTest extends CakeTestCase {
	...
	public function testLoadWithAlias() {
		$result = $this->Components->load ( 'Cookie', array (
				'className' => 'CookieAlias',
				'somesetting' => true 
		) );
		...
		$result = $this->Components->load ( 'SomeOther', array (
				'className' => 'TestPlugin.Other' 
		) );
	...
	public function testUnload() {
		...
		$this->assertEquals ( array (
				'Cookie',
				'Security' 
		), $result, 'loaded components is wrong' );
		...
		$this->assertEquals ( array (
				'Security' 
		), $result, 'enabled components is wrong' );
	}
