...
class ActionsAuthorizeTest extends CakeTestCase {
	...
	protected function _mockAcl() {
		$this->Collection->expects ( $this->any () )->method ( 'load' )->with ( 'Acl' )->will ( $this->returnValue ( $this->Acl ) );
	}
	...
	public function testAuthorizeFailure() {
		...
		$this->Acl->expects ( $this->once () )->method ( 'check' )->with ( $user, 'controllers/Posts/index' )->will ( $this->returnValue ( false ) );
	...
	public function testAuthorizeSuccess() {
		...
		$this->Acl->expects ( $this->once () )->method ( 'check' )->with ( $user, 'controllers/Posts/index' )->will ( $this->returnValue ( true ) );
	...
	public function testAuthorizeSettings() {
		...
		$expected = array (
				'TestPlugin.TestPluginAuthUser' => array (
						'id' => 1,
						'user' => 'mariano' 
				) 
		);
