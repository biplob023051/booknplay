...
class SecurityComponentTest extends CakeTestCase {
	...
	public function testRequirePostFail() {
		...
		$this->Controller->Security->requirePost ( array (
				'posted' 
		) );
		$this->Controller->Security->startup ( $this->Controller );
	...
	public function testRequireSecureFail() {
		...
		$_SERVER ['REQUEST_METHOD'] = 'POST';
		$this->Controller->request ['action'] = 'posted';
		$this->Controller->Security->requireSecure ( array (
				'posted' 
		) );
		$this->Controller->Security->startup ( $this->Controller );
		$this->assertTrue ( $this->Controller->failed );
	...
	public function testRequireAuthFail() {
		...
		$this->Controller->Security->requireAuth ( array (
				'posted' 
		) );
		$this->Controller->Security->startup ( $this->Controller );
		$this->assertTrue ( $this->Controller->failed );
	...
	public function testRequireGetFail() {
		$_SERVER ['REQUEST_METHOD'] = 'POST';
		$this->Controller->request ['action'] = 'getted';
		$this->Controller->Security->requireGet ( array (
				'getted' 
		) );
		$this->Controller->Security->startup ( $this->Controller );
		$this->assertTrue ( $this->Controller->failed );
	...
	public function testRequirePutFail() {
		...
		$this->Controller->Security->requirePut ( array (
				'putted' 
		) );
		$this->Controller->Security->startup ( $this->Controller );
		$this->assertTrue ( $this->Controller->failed );
	...
	public function testRequireDeleteFail() {
		...
		$this->Controller->Security->requireDelete ( array (
				'deleted',
				'other_method' 
		) );
		$this->Controller->Security->startup ( $this->Controller );
	...
	/**
	 * Test that objects can't be passed into the serialized string.
	 * This was a vector for RFI and LFI
	...
	public function testValidatePostWithDisabledFields() {
		$this->Controller->Security->disabledFields = array (
				'Model.username',
				'Model.password' 
		);
		$this->Controller->Security->startup ( $this->Controller );
	...
	public function testCsrfNonceConsumption() {
		...
		$this->Security->Session->write ( '_Token.csrfTokens', array (
				'nonce1' => strtotime ( '+10 minutes' ) 
		) );
		
		$this->Controller->request = $this->getMock ( 'CakeRequest', array (
				'is' 
		) );
		$this->Controller->request->expects ( $this->once () )->method ( 'is' )->with ( 'post' )->will ( $this->returnValue ( true ) );
	...
	public function testCsrfNotUseOnce() {
		...
		$this->assertEquals ( array (
				$key 
		), array_keys ( $token ), '_Token.key and csrfToken do not match request will blackhole.' );
	}
