...
class PhpAclTest extends CakeTestCase {
	...
	public function testAddRole() {
		...
		$this->Acl->Aro->addRole ( array (
				'User/foobar' => 'Role/accounting' 
		) );
		$this->assertEquals ( array (
				array (
						'Role/accounting' 
	...
	public function testAroResolve() {
		...
		$this->assertEquals ( 'User/hardy', $this->Acl->Aro->resolve ( array (
				'FooModel' => array (
						'nickname' => 'hardy' 
				) 
		) ) );
		$this->assertEquals ( 'Role/admin', $this->Acl->Aro->resolve ( array (
				'FooModel' => array (
						'role' => 'admin' 
				) 
		) ) );
		...
		$this->assertEquals ( PhpAro::DEFAULT_ROLE, $this->Acl->Aro->resolve ( array (
				'FooModel' => array (
						'role' => 'hardy' 
				) 
		) ) );
	...
	public function testAroAliases() {
		...
		$this->Acl->Aro->addRole ( array (
				'User/foo' => 'Role/IT, Role/accounting' 
		) );
	...
	public function testCheck() {
		...
		$this->assertTrue ( $this->Acl->check ( array (
				'User' => array (
						'username' => 'jan' 
				) 
		), '/controllers/bar/bll' ) );
	...
	public function testInvalidConfigWithAroMissing() {
		$this->setExpectedException ( 'AclException', '"roles" section not found in configuration' );
		$config = array (
				'aco' => array (
						'allow' => array (
								'foo' => '' 
	...
	public function testInvalidConfigWithAcosMissing() {
		$this->setExpectedException ( 'AclException', 'Neither "allow" nor "deny" rules were provided in configuration.' );
	...
	public function testAcoResolve() {
		...
		$this->assertEquals ( array (
				...
				'bar',
				'baz' 
		), $this->Acl->Aco->resolve ( 'foo/bar/baz' ) );
		$this->assertEquals ( array (
				'foo',
				'*-bar',
				'?-baz' 
