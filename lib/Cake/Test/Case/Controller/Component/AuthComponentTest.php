...
class AuthComponentTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$User->updateAll ( array (
				'password' => $User->getDataSource ()->value ( Security::hash ( 'cake', null, true ) ) 
		) );
	}
	...
	public function testLogin() {
		...
		$this->Auth->Session = $this->getMock ( 'SessionComponent', array (
				'renew' 
		), array (), '', false );
		...
		$mocks [0]->expects ( $this->once () )->method ( 'authenticate' )->with ( $this->Auth->request )->will ( $this->returnValue ( $user ) );
		
		$this->Auth->Session->expects ( $this->once () )->method ( 'renew' );
	...
	public function testIsAuthorizedMissingFile() {
		...
		$this->Controller->Auth->isAuthorized ( array (
				'User' => array (
						'id' => 1 
				) 
		) );
	...
	public function testIsAuthorizedDelegation() {
		$this->getMock ( 'BaseAuthorize', array (
				'authorize' 
		), array (), 'AuthMockOneAuthorize', false );
		$this->getMock ( 'BaseAuthorize', array (
				'authorize' 
		), array (), 'AuthMockTwoAuthorize', false );
		$this->getMock ( 'BaseAuthorize', array (
				'authorize' 
		), array (), 'AuthMockThreeAuthorize', false );
		...
		$mocks [0]->expects ( $this->once () )->method ( 'authorize' )->with ( array (
				'User' 
		), $request )->will ( $this->returnValue ( false ) );
		
		$mocks [1]->expects ( $this->once () )->method ( 'authorize' )->with ( array (
				'User' 
		), $request )->will ( $this->returnValue ( true ) );
		
		$mocks [2]->expects ( $this->never () )->method ( 'authorize' );
		
		$this->assertTrue ( $this->Auth->isAuthorized ( array (
				'User' 
		), $request ) );
	}
	...
	public function testIsAuthorizedUsingUserInSession() {
		$this->getMock ( 'BaseAuthorize', array (
				'authorize' 
		), array (), 'AuthMockFourAuthorize', false );
		$this->Auth->authorize = array (
				'AuthMockFour' 
		);
		...
		$mocks [0]->expects ( $this->once () )->method ( 'authorize' )->with ( $user, $request )->will ( $this->returnValue ( true ) );
	...
	public function testAllowDenyAll() {
		...
		$this->Controller->Auth->allow ();
		$this->Controller->Auth->deny ( array (
				'add',
				'camelCase' 
		) );
	...
	public function testAllowedActionsWithCamelCaseMethods() {
		...
		$this->Controller->Auth->allow ( array (
				'delete',
				'add' 
		) );
		$result = $this->Controller->Auth->startup ( $this->Controller );
	...
	public function testAllowedActionsSetWithAllowMethod() {
		...
		$this->assertEquals ( array (
				'action_name',
				'anotherAction' 
		), $this->Controller->Auth->allowedActions );
	}
	...
	public function testAjaxLogin() {
		...
		$Dispatcher->dispatch ( new CakeRequest ( '/ajax_auth/add' ), new CakeResponse (), array (
				'return' => 1 
		) );
		$result = ob_get_clean ();
	...
	public function testComponentSettings() {
		...
		$this->Controller->Components->init ( $this->Controller );
		$this->Controller->Components->trigger ( 'initialize', array (
				&$this->Controller 
		) );
		Router::reload ();
	...
	/**
	 * test that logout deletes the session variables.
	...
	public function testLogoutTrigger() {
		...
		$mock [0]->expects ( $this->once () )->method ( 'logout' );
	...
	public function testMapActionsDelegation() {
		$this->getMock ( 'BaseAuthorize', array (
				'authorize' 
		), array (), 'MapActionMockAuthorize', false );
		$this->Auth->authorize = array (
				'MapActionMock' 
		);
		$mock = $this->Auth->constructAuthorize ();
		$mock [0]->expects ( $this->once () )->method ( 'mapActions' )->with ( array (
				'create' => array (
						'my_action' 
				) 
		) );
	...
	public function testLoginWithRequestData() {
		...
		$mock = $this->Auth->constructAuthenticate ();
		$mock [0]->expects ( $this->once () )->method ( 'authenticate' )->with ( $request )->will ( $this->returnValue ( $user ) );
	...
	public function testFlashSettings() {
		...
		$this->Auth->Session->expects ( $this->once () )->method ( 'setFlash' )->with ( 'Auth failure', 'custom', array (
				1 
		), 'auth-key' );
	...
	public function testRedirectSessionReadEqualToLoginAction() {
		...
		$this->Auth->Session->write ( 'Auth.redirect', array (
				'controller' => 'users',
				'action' => 'login' 
		) );
