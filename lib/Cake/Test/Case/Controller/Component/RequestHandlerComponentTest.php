...
class RequestHandlerComponentTest extends CakeTestCase {
	...
	public function testConstructorSettings() {
		...
		$this->assertEquals ( array (
				'json' => 'MyPlugin.MyJson' 
		), $RequestHandler->settings ['viewClassMap'] );
	}
	...
	public function testInitializeContentTypeAndExtensionMismatch() {
		...
		$this->Controller->request->expects ( $this->any () )->method ( 'accepts' )->will ( $this->returnValue ( array (
				'application/json' 
		) ) );
	...
	public function testViewClassMap() {
		$this->RequestHandler->settings = array (
				'viewClassMap' => array (
						'json' => 'CustomJson' 
				) 
		);
	...
	public function testStartupCallback() {
		...
		$this->Controller->request = $this->getMock ( 'CakeRequest', array (
				'_readInput' 
		) );
		$this->RequestHandler->startup ( $this->Controller );
	...
	public function testStartupCustomTypeProcess() {
		...
		$this->Controller->request->expects ( $this->once () )->method ( '_readInput' )->will ( $this->returnValue ( '"A","csv","string"' ) );
		$this->RequestHandler->addInputType ( 'csv', array (
				'str_getcsv' 
		) );
		$this->RequestHandler->startup ( $this->Controller );
		$expected = array (
	...
	public function testRenderAsWithAttachment() {
		...
		$this->RequestHandler->request->expects ( $this->any () )->method ( 'parseAccept' )->will ( $this->returnValue ( array (
				'1.0' => array (
						'application/xml' 
				) 
		) ) );
		
		$this->RequestHandler->response = $this->getMock ( 'CakeResponse', array (
				'type',
				'download',
				'charset' 
		) );
		$this->RequestHandler->response->expects ( $this->at ( 0 ) )->method ( 'type' )->with ( 'application/xml' );
		$this->RequestHandler->response->expects ( $this->at ( 1 ) )->method ( 'charset' )->with ( 'UTF-8' );
		$this->RequestHandler->response->expects ( $this->at ( 2 ) )->method ( 'download' )->with ( 'myfile.xml' );
		
		$this->RequestHandler->renderAs ( $this->Controller, 'xml', array (
	...
	public function testRespondAs() {
		...
		$this->RequestHandler->response->expects ( $this->at ( 0 ) )->method ( 'type' )->with ( 'application/json' );
		$this->RequestHandler->response->expects ( $this->at ( 1 ) )->method ( 'type' )->with ( 'text/xml' );
	...
	public function testRespondAsWithAttachment() {
		$this->RequestHandler = $this->getMock ( 'RequestHandlerComponent', array (
				'_header' 
		), array (
		...
		$this->RequestHandler->request->expects ( $this->once () )->method ( 'parseAccept' )->will ( $this->returnValue ( array (
				'1.0' => array (
						'application/xml' 
				) 
		) ) );
		
		$this->RequestHandler->response->expects ( $this->once () )->method ( 'download' )->with ( 'myfile.xml' );
		$this->RequestHandler->response->expects ( $this->once () )->method ( 'type' )->with ( 'application/xml' );
		
		$result = $this->RequestHandler->respondAs ( 'xml', array (
				'attachment' => 'myfile.xml' 
		) );
		$this->assertTrue ( $result );
	}
	...
	public function testRequestContentTypes() {
		...
		$result = $this->RequestHandler->requestedWith ( array (
				'json',
				'xml' 
		) );
	...
	public function testRequestProperties() {
		$request = $this->getMock ( 'CakeRequest' );
		$request->expects ( $this->once () )->method ( 'is' )->with ( 'ssl' )->will ( $this->returnValue ( true ) );
	...
	public function testMapAlias() {
		...
		$result = $this->RequestHandler->mapAlias ( array (
				'xml',
				'js',
				'json' 
		) );
	...
	public function testAccepts() {
		...
		$this->assertTrue ( $this->RequestHandler->accepts ( array (
				'js',
				'xml',
				'html' 
		) ) );
		$this->assertFalse ( $this->RequestHandler->accepts ( array (
				'gif',
				'jpeg',
				'foo' 
		) ) );
	...
	public function testPrefers() {
		...
		$this->assertEquals ( 'xml', $this->RequestHandler->prefers ( array (
				'js',
				'xml',
				'xhtml' 
		) ) );
		$this->assertFalse ( $this->RequestHandler->prefers ( array (
				'red',
				'blue' 
		) ) );
		$this->assertEquals ( 'xhtml', $this->RequestHandler->prefers ( array (
				'js',
				'json',
				'xhtml' 
		) ) );
		$this->assertTrue ( $this->RequestHandler->prefers ( array (
				'rss' 
		) ), 'Should return true if input matches ext.' );
		$this->assertFalse ( $this->RequestHandler->prefers ( array (
				'html' 
		) ), 'No match with ext, return false.' );
	...
	public function testAjaxRedirectAsRequestAction() {
		...
		$this->Controller->RequestHandler = $this->getMock ( 'RequestHandlerComponent', array (
				'_stop' 
		), array (
				&$this->Controller->Components 
		) );
		$this->Controller->request = $this->getMock ( 'CakeRequest' );
		$this->Controller->response = $this->getMock ( 'CakeResponse', array (
				'_sendHeader' 
		) );
		...
		ob_start ();
		$this->Controller->RequestHandler->beforeRedirect ( $this->Controller, array (
				'controller' => 'request_handler_test',
				'action' => 'destination' 
		) );
		$result = ob_get_clean ();
	...
	public function testBeforeRedirectCallbackWithArrayUrl() {
		...
		$RequestHandler = $this->getMock ( 'RequestHandlerComponent', array (
				'_stop' 
		), array (
				&$this->Controller->Components 
		) );
		$RequestHandler->response = $this->getMock ( 'CakeResponse', array (
				'_sendHeader' 
		) );
		...
		ob_start ();
		$RequestHandler->beforeRedirect ( $this->Controller, array (
				'controller' => 'request_handler_test',
				'action' => 'param_method',
				'first',
				'second' 
	...
	public function testBeforeRedirectCallingHeader() {
		...
		$RequestHandler->request->expects ( $this->once () )->method ( 'is' )->with ( 'ajax' )->will ( $this->returnValue ( true ) );
