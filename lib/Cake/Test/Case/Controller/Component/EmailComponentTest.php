...
class DebugCompTransport extends AbstractTransport {
	 ...
	 *
	 * @param
	 *        	s object $email CakeEmail
	 * @return boolean
	 */
	public function send(CakeEmail $email) {
		$email->addHeaders ( array (
				'Date' => EmailComponentTest::$sentDate 
		) );
...
HTMLBLOC;
		...
		$this->assertTrue ( $this->Controller->EmailTest->send ( 'This is the body of the message' ) );
		$this->assertTextEquals ( $expect, preg_replace ( '/[a-z0-9]{32}/i', '{boundary}', DebugCompTransport::$lastEmail ) );
...
HTMLBLOC;
	...
	public function testReset() {
		...
		$this->Controller->EmailTest->cc = array (
				'cc1@example.com',
				'cc2@example.com' 
		);
		$this->Controller->EmailTest->bcc = array (
				'bcc1@example.com',
				'bcc2@example.com' 
		);
