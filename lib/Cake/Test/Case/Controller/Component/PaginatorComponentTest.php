...
class PaginatorComponentTest extends CakeTestCase {
	...
	public function testPaginate() {
		...
		$Controller->request->params ['pass'] = array (
				'1' 
		);
		...
		$Controller->Paginator->settings = array (
				'order' => array (
						'PaginatorControllerComment.id' => 'ASC' 
				) 
		);
		...
		$Controller->request->params ['named'] = array (
				'sort' => 'PaginatorControllerPost.id',
				'direction' => 'asc' 
		);
		$results = Hash::extract ( $Controller->Paginator->paginate ( 'PaginatorControllerPost' ), '{n}.PaginatorControllerPost.id' );
		$this->assertEquals ( 1, $Controller->params ['paging'] ['PaginatorControllerPost'] ['page'] );
		...
		$results = Hash::extract ( $Controller->Paginator->paginate ( 'PaginatorControllerPost' ), '{n}.PaginatorControllerPost.id' );
		$this->assertEquals ( array (
				'PaginatorControllerPost.author_id' => 'asc' 
		), $Controller->PaginatorControllerPost->lastQueries [1] ['order'] [0] );
		$this->assertEquals ( array (
				1,
				3,
				2 
	...
	public function testPageParamCasting() {
		$this->Controller->Post->expects ( $this->at ( 0 ) )->method ( 'hasMethod' )->with ( 'paginate' )->will ( $this->returnValue ( false ) );
		
		$this->Controller->Post->expects ( $this->at ( 1 ) )->method ( 'find' )->will ( $this->returnValue ( array (
				'stuff' 
		) ) );
		
		$this->Controller->Post->expects ( $this->at ( 2 ) )->method ( 'hasMethod' )->with ( 'paginateCount' )->will ( $this->returnValue ( false ) );
		
		$this->Controller->Post->expects ( $this->at ( 3 ) )->method ( 'find' )->will ( $this->returnValue ( 2 ) );
	...
	public function testPaginateExtraParams() {
		...
		$this->assertEquals ( array (
				...
				2,
				3 
		), Hash::extract ( $result, '{n}.PaginatorControllerPost.id' ) );
	...
	public function testPaginateSpecialType() {
		...
		$this->assertEquals ( $Controller->PaginatorControllerPost->lastQueries [1] ['conditions'], array (
				'PaginatorControllerPost.id > ' => '1' 
		) );
	...
	public function testPaginateOrderVirtualField() {
		...
		$this->assertEquals ( array (
				...
				3,
				2 
		), Hash::extract ( $result, '{n}.PaginatorControllerPost.offset_test' ) );
	...
	public function testPaginateOrderVirtualFieldJoinedModel() {
		...
		$Controller->Paginator->settings = array (
				'order' => array (
						'PaginatorAuthor.joined_offset' => 'DESC' 
				),
				'maxLimit' => 10,
				'paramType' => 'named' 
		);
		...
		$this->assertEquals ( array (
				...
				2,
				2 
		), Hash::extract ( $result, '{n}.PaginatorAuthor.joined_offset' ) );
	...
	public function testOutOfRangePageNumberAndPageCountZero() {
		...
		$Controller->paginate = array (
				'conditions' => array (
						'PaginatorControllerPost.id >' => 100 
				) 
		);
		$Controller->Paginator->paginate ( 'PaginatorControllerPost' );
	}
	...
	public function testValidateSortWhitelistFailure() {
		...
		$result = $this->Paginator->validateSort ( $model, $options, array (
				'title',
				'id' 
		) );
	...
	public function testValidateSortVirtualField() {
		...
		$model->expects ( $this->at ( 0 ) )->method ( 'hasField' )->with ( 'something' )->will ( $this->returnValue ( false ) );
		
		$model->expects ( $this->at ( 1 ) )->method ( 'hasField' )->with ( 'something', true )->will ( $this->returnValue ( true ) );
	...
	public function testValidateSortSharedFields() {
		...
		$model->Child->expects ( $this->at ( 0 ) )->method ( 'hasField' )->with ( 'something' )->will ( $this->returnValue ( true ) );
		
		$options = array (
				'sort' => 'Child.something',
				'direction' => 'desc' 
		);
		$result = $this->Paginator->validateSort ( $model, $options );
	...
	public function testCheckLimit() {
		$result = $this->Paginator->checkLimit ( array (
				'limit' => 1000000,
				'maxLimit' => 100 
		) );
	...
	public function testPaginateOrderVirtualFieldSharedWithRealField() {
		...
		$result = Hash::extract ( $Controller->paginate ( 'PaginatorControllerComment' ), '{n}.PaginatorControllerComment.id' );
	...
	public function testPaginateCustomFind() {
		...
		$this->assertEquals ( array (
				...
				3,
				4 
		), Hash::extract ( $result, '{n}.PaginatorCustomPost.id' ) );
	...
	public function testPaginateCustomFindFieldsArray() {
		...
		$Controller->paginate = array (
				'list',
				'conditions' => array (
						'PaginatorCustomPost.published' => 'Y' 
				),
				'limit' => 2 
		);
		$result = $Controller->paginate ();
