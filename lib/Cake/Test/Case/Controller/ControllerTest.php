...
class ControllerTest extends CakeTestCase {
	...
	public function testLoadModelInPlugins() {
		App::build ( array (
				...
				'Controller' => array (
						CAKE . 'Test' . DS . 'test_app' . DS . 'Controller' . DS 
				),
				'Model' => array (
	...
	public function testFlash() {
		...
		$Controller = new Controller ( $request, $this->getMock ( 'CakeResponse', array (
				'_sendHeader' 
		) ) );
		...
		$Controller = new Controller ( $request );
		$Controller->response = $this->getMock ( 'CakeResponse', array (
				'_sendHeader' 
		) );
	...
	public function testRender() {
		...
		$expectedModels = array (
				'ControllerAlias' => array (
						'plugin' => null,
						'className' => 'ControllerAlias' 
				),
				'ControllerComment' => array (
						'plugin' => 'TestPlugin',
						'className' => 'ControllerComment' 
				),
	...
	public function testRedirectByCode($code, $msg) {
		...
		$Controller->Components = $this->getMock ( 'ComponentCollection', array (
				'trigger' 
		) );
		
		$Controller->response->expects ( $this->once () )->method ( 'statusCode' )->with ( $code );
		$Controller->response->expects ( $this->once () )->method ( 'header' )->with ( 'Location', 'http://cakephp.org' );
	...
	public function testRedirectTriggeringComponentsReturnNull() {
		...
		$Controller->Components->expects ( $this->once () )->method ( 'trigger' )->will ( $this->returnValue ( null ) );
		
		$Controller->response->expects ( $this->once () )->method ( 'statusCode' )->with ( 301 );
	...
	public function testRedirectBeforeRedirectModifyingParams() {
		...
		$Controller->Components->expects ( $this->once () )->method ( 'trigger' )->will ( $this->returnValue ( array (
				'http://book.cakephp.org' 
		) ) );
		
		$Controller->response->expects ( $this->once () )->method ( 'statusCode' )->with ( 301 );
		
		$Controller->response->expects ( $this->once () )->method ( 'header' )->with ( 'Location', 'http://book.cakephp.org' );
	...
	public function testRedirectBeforeRedirectModifyingParamsArrayReturn() {
		...
		$Controller->Components->expects ( $this->once () )->method ( 'trigger' )->will ( $this->returnValue ( $return ) );
		
		$Controller->response->expects ( $this->once () )->method ( 'header' )->with ( 'Location', 'http://example.com/test/2' );
		
		$Controller->response->expects ( $this->at ( 1 ) )->method ( 'statusCode' )->with ( 302 );
	...
	public function testRedirectBeforeRedirectInController() {
		...
		$Controller->expects ( $this->once () )->method ( 'beforeRedirect' )->with ( 'http://cakephp.org' )->will ( $this->returnValue ( false ) );
	...
	public function testRedirectBeforeRedirectInControllerWithArray() {
		...
		$Controller->expects ( $this->once () )->method ( 'beforeRedirect' )->with ( 'http://cakephp.org', null, true )->will ( $this->returnValue ( array (
				'url' => 'http://example.org',
				'status' => 302,
				'exit' => true 
		) ) );
		
		$Controller->response->expects ( $this->at ( 0 ) )->method ( 'header' )->with ( 'Location', 'http://example.org' );
	...
	public function testChildComponentOptionsSupercedeParents() {
		...
		$TestController->components = array (
				'Cookie' => $expected 
		);
		$TestController->constructClasses ();
	...
	public function testReferer() {
		...
		$Controller = new Controller ( $request );
		$request->setReturnValue ( 'referer', '/', array (
				true 
		) );
		$result = $Controller->referer ( array (
				'controller' => 'posts',
				'action' => 'index' 
		), true );
		$this->assertEquals ( '/posts/index', $result );
	...
	public function testStartupProcess() {
		...
		$eventManager->expects ( $this->at ( 0 ) )->method ( 'dispatch' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'Controller.initialize' ), $this->attributeEqualTo ( '_subject', $Controller ) ) );
		$eventManager->expects ( $this->at ( 1 ) )->method ( 'dispatch' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'Controller.startup' ), $this->attributeEqualTo ( '_subject', $Controller ) ) );
		$Controller->expects ( $this->exactly ( 2 ) )->method ( 'getEventManager' )->will ( $this->returnValue ( $eventManager ) );
	...
	public function testShutdownProcess() {
		...
		$eventManager = $this->getMock ( 'CakeEventManager' );
		$eventManager->expects ( $this->once () )->method ( 'dispatch' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'Controller.shutdown' ), $this->attributeEqualTo ( '_subject', $Controller ) ) );
		$Controller->expects ( $this->once () )->method ( 'getEventManager' )->will ( $this->returnValue ( $eventManager ) );
