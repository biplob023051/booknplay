...
class TextHelperTest extends CakeTestCase {
	...
	public function testTextHelperProxyMethodCalls() {
		...
		$Text = new TextHelperTestObject ( $this->View, array (
				'engine' => 'StringMock' 
		) );
		$Text->attach ( $String );
	...
	public function testEngineOverride() {
		App::build ( array (
				'Utility' => array (
						CAKE . 'Test' . DS . 'test_app' . DS . 'Utility' . DS 
				) 
		), App::REGISTER );
		$Text = new TextHelperTestObject ( $this->View, array (
				'engine' => 'TestAppEngine' 
		) );
		...
		$Text = new TextHelperTestObject ( $this->View, array (
				'engine' => 'TestPlugin.TestPluginEngine' 
		) );
	...
	public function testAutoLink() {
		...
		$result = $this->Text->autoLink ( $text, array (
				'class' => 'link' 
		) );
		$this->assertEquals ( $expected, $result );
	...
	public function testAutoLinkUrlsOptions() {
		...
		$result = $this->Text->autoLinkUrls ( $text, array (
				'class' => 'link' 
		) );
		$this->assertRegExp ( '#^' . $expected . '$#', $result );
