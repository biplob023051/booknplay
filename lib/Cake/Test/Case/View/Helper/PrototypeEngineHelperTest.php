...
class PrototypeEngineHelperTest extends CakeTestCase {
	...
	public function testEvent() {
		...
		$result = $this->Proto->event ( 'click', 'doClick', array (
				'wrap' => false 
		) );
		$expected = '$("myLink").observe("click", doClick);';
		$this->assertEquals ( $expected, $result );
		
		$result = $this->Proto->event ( 'click', 'Element.hide(this);', array (
				'stop' => false 
		) );
	...
	public function testEffect() {
		...
		$result = $this->Proto->effect ( 'fadeIn', array (
				'speed' => 'fast' 
		) );
		...
		$result = $this->Proto->effect ( 'fadeOut', array (
				'speed' => 'fast' 
		) );
		...
		$result = $this->Proto->effect ( 'slideOut', array (
				'speed' => 'fast' 
		) );
	...
	public function testRequest() {
		$result = $this->Proto->request ( array (
				'controller' => 'posts',
				'action' => 'view',
				1 
		) );
		...
		$result = $this->Proto->request ( '/users/login', array (
				...
				'update' => 'auth',
				'data' => $this->Proto->serializeForm ( array (
						'isForm' => false,
						'inline' => true 
				) ),
				'dataExpression' => true 
	...
	/**
	 * test drag() method.
	 * Scriptaculous lacks the ability to take an Array of Elements
	...
	public function testSerializeForm() {
		$this->Proto->get ( '#element' );
		$result = $this->Proto->serializeForm ( array (
				'isForm' => true 
		) );
