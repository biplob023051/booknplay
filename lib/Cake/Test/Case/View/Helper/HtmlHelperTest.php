...
class HtmlHelperTest extends CakeTestCase {
	...
	public function setUp() {
		parent::setUp ();
		$this->View = $this->getMock ( 'View', array (
				'append' 
		), array (
				new TheHtmlTestController () 
		) );
	...
	public function testLink() {
		...
		$result = $this->Html->link ( array (
				'action' => 'login',
				'<[You]>' 
		) );
		$expected = array (
				'a' => array (
						'href' => '/login/%3C%5BYou%5D%3E' 
				),
				'preg:/\/login\/&lt;\[You\]&gt;/',
				'/a' 
		);
		...
		$expected = array (
				'a' => array (
						'href' => '/home',
						'onclick' => 'someFunction(); event.returnValue = false; return false;' 
				),
				'Home',
				'/a' 
		);
		...
		$expected = array (
				'a' => array (
						'href' => '/images/view/3?height=100&amp;width=200' 
				),
				'Original size',
				'/a' 
		);
		...
		$result = $this->Html->image ( 'test.gif', array (
				'url' => '#' 
		) );
		$expected = array (
				'a' => array (
		...
		$result = $this->Html->link ( $this->Html->image ( '../favicon.ico' ), '#', array (
				'escape' => false 
		) );
		$expected = array (
				'a' => array (
		...
		$result = $this->Html->image ( '../favicon.ico', array (
				'url' => '#' 
		) );
		$expected = array (
				'a' => array (
		...
		$expected = array (
				'a' => array (
						'href' => 'http://www.example.org?param1=value1&amp;param2=value2' 
				),
		...
		$expected = array (
				'a' => array (
						'href' => 'mailto:example@cakephp.org' 
				),
				'write me',
				'/a' 
		);
		...
		$expected = array (
				'a' => array (
						'href' => 'tel:0123465-798' 
				),
				'call me on 0123465-798',
				'/a' 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'a' => array (
						'href' => 'sms:0123465-798' 
				),
				'text me on 0123465-798',
				'/a' 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'a' => array (
						'href' => 'sms:0123465-798?body=hello there' 
				),
		...
		$expected = array (
				'a' => array (
						'href' => 'sms:0123465-798?body=hello &quot;cakephp&quot;' 
				),
				'say hello to 0123465-798',
				'/a' 
		);
	...
	public function testImageWithFullBase() {
		...
		$this->assertTags ( $result, array (
				'img' => array (
						'src' => $here . 'img/test.gif',
						'alt' => '' 
				) 
		) );
		
		$result = $this->Html->image ( 'sub/test.gif', array (
				'fullBase' => true 
		) );
		$here = $this->Html->url ( '/', true );
		$this->assertTags ( $result, array (
				'img' => array (
						'src' => $here . 'img/sub/test.gif',
						'alt' => '' 
				) 
		) );
	...
	public function testImageWithTimestampping() {
		...
		$expected = array (
				'img' => array (
						'src' => 'preg:/\/testing\/longer\/img\/cake\.icon\.png\?[0-9]+/',
						'alt' => '' 
				) 
		);
		$this->assertTags ( $result, $expected );
	...
	public function testCssLink() {
		...
		$result = $this->Html->css ( 'TestPlugin.style', null, array (
				'plugin' => false 
		) );
		...
		$this->View->expects ( $this->at ( 0 ) )->method ( 'append' )->with ( 'css', $this->matchesRegularExpression ( '/css_in_head.css/' ) );
		
		$this->View->expects ( $this->at ( 1 ) )->method ( 'append' )->with ( 'css', $this->matchesRegularExpression ( '/more_css_in_head.css/' ) );
		
		$result = $this->Html->css ( 'css_in_head', null, array (
				'inline' => false 
		) );
		$this->assertNull ( $result );
		
		$result = $this->Html->css ( 'more_css_in_head', null, array (
				'inline' => false 
		) );
		$this->assertNull ( $result );
	}
	...
	public function testCssWithFullBase() {
		...
		$result = $this->Html->css ( 'screen', null, array (
				'fullBase' => true 
		) );
		$expected = array (
				'link' => array (
	...
	public function testScriptTimestamping() {
		...
		$result = $this->Html->script ( '__cake_js_test', array (
				'inline' => true,
				'once' => false 
		) );
	...
	public function testPluginScriptTimestamping() {
		...
		$result = $this->Html->script ( 'TestPlugin.__cake_js_test', array (
				'inline' => true,
				'once' => false 
		) );
	...
	public function testScript() {
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => 'http://example.com/test.json' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => '/plugin/js/jquery-1.3.2.js?someparam=foo' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => 'js/test.json.js?foo=bar&amp;other=test' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$result = $this->Html->script ( 'jquery-1.3.2', array (
				'defer' => true,
				'encoding' => 'utf-8' 
		) );
		$expected = array (
	...
	public function testPluginScript() {
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => 'test_plugin/js/jquery-1.3.js' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => 'test_plugin/js/test.json.js' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => 'test_plugin/jquery-1.3.2.js?someparam=foo' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => 'test_plugin/js/test.json.js?foo=bar' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$result = $this->Html->script ( 'TestPlugin.foo', array (
				'inline' => true,
				'once' => false 
		) );
		$this->assertNotNull ( $result );
		
		$result = $this->Html->script ( 'TestPlugin.jquery-1.3.2', array (
				'defer' => true,
				'encoding' => 'utf-8' 
		) );
		$expected = array (
	...
	public function testScriptWithBlocks() {
		$this->View->expects ( $this->at ( 0 ) )->method ( 'append' )->with ( 'script', $this->matchesRegularExpression ( '/script_in_head.js/' ) );
		
		$this->View->expects ( $this->at ( 1 ) )->method ( 'append' )->with ( 'script', $this->matchesRegularExpression ( '/bool_false.js/' ) );
		
		$this->View->expects ( $this->at ( 2 ) )->method ( 'append' )->with ( 'headScripts', $this->matchesRegularExpression ( '/second_script.js/' ) );
		
		$result = $this->Html->script ( 'script_in_head', array (
				'inline' => false 
		) );
		$this->assertNull ( $result );
		...
		$result = $this->Html->script ( 'second_script', array (
				'block' => 'headScripts' 
		) );
		$this->assertNull ( $result );
	}
	...
	public function testScriptAssetFilter() {
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => '//example.com/js/jquery-1.3.js' 
				) 
		);
		$this->assertTags ( $result, $expected );
	}
	...
	public function testScriptInTheme() {
		...
		$expected = array (
				'script' => array (
						'src' => '/theme/test_theme/js/__test_js.js',
						'type' => 'text/javascript' 
				) 
		);
		$this->assertTags ( $result, $expected );
	...
	public function testScriptBlock() {
		...
		$result = $this->Html->scriptBlock ( 'window.foo = 2;', array (
		...
		$this->View->expects ( $this->at ( 0 ) )->method ( 'append' )->with ( 'script', $this->matchesRegularExpression ( '/window\.foo\s\=\s2;/' ) );
		
		$this->View->expects ( $this->at ( 1 ) )->method ( 'append' )->with ( 'scriptTop', $this->stringContains ( 'alert(' ) );
		...
		$result = $this->Html->scriptBlock ( 'alert("hi")', array (
				'block' => 'scriptTop' 
		) );
		$this->assertNull ( $result );
	...
	public function testScriptStartAndScriptEnd() {
		$result = $this->Html->scriptStart ( array (
				'safe' => true 
		) );
		$this->assertNull ( $result );
		...
		$this->View->expects ( $this->once () )->method ( 'append' );
		$result = $this->Html->scriptStart ( array (
				'safe' => false,
				'inline' => false 
		) );
	...
	public function testNestedList() {
		...
		$result = $this->Html->nestedList ( $list, array (
				'class' => 'list' 
		) );
		$expected = array (
				array (
		...
		$result = $this->Html->nestedList ( $list, array (), array (
				'class' => 'item' 
		) );
		$expected = array (
				'<ul',
	...
	public function testMeta() {
		$result = $this->Html->meta ( 'this is an rss feed', array (
				'controller' => 'posts',
				'ext' => 'rss' 
		) );
		$this->assertTags ( $result, array (
		...
		$result = $this->Html->meta ( 'non-existing', '/posts.xpp', array (
				'type' => 'atom' 
		) );
		$this->assertTags ( $result, array (
				'link' => array (
	...
	public function testMetaWithBlocks() {
		$this->View->expects ( $this->at ( 0 ) )->method ( 'append' )->with ( 'meta', $this->stringContains ( 'ROBOTS' ) );
		
		$this->View->expects ( $this->at ( 1 ) )->method ( 'append' )->with ( 'metaTags', $this->stringContains ( 'favicon.ico' ) );
		...
		$result = $this->Html->meta ( 'icon', 'favicon.ico', array (
				'block' => 'metaTags' 
		) );
		$this->assertNull ( $result );
	}
	...
	public function testTableCells() {
		...
		$result = $this->Html->tableCells ( $tr, array (
				'class' => 'odd' 
		), array (
				'class' => 'even' 
		) );
	...
	public function testTag() {
		...
		$result = $this->Html->tag ( 'div', '<text>', array (
				'class' => 'class-name',
				'escape' => true 
		) );
		$this->assertTags ( $result, array (
	...
	public function testUseTag() {
		...
		$result = $this->Html->useTag ( 'form', 'example.com', array (
				'test' => 'ok' 
		) );
		$this->assertTags ( $result, array (
				'form' => array (
	...
	public function testDiv() {
		...
		$result = $this->Html->div ( 'class-name', '<text>', array (
				'escape' => true 
		) );
		$this->assertTags ( $result, array (
				'div' => array (
	...
	public function testPara() {
		...
		$result = $this->Html->para ( 'class-name', '<text>', array (
				'escape' => true 
		) );
		$this->assertTags ( $result, array (
				'p' => array (
	...
	public function testMedia() {
		...
		$result = $this->Html->media ( 'video.ogv', array (
				'tag' => 'video' 
		) );
		$expected = array (
				'video' => array (
		...
		$result = $this->Html->media ( null, array (
				'src' => 'video.webm' 
		) );
		$expected = array (
				'video' => array (
	...
	public function testLoadConfig() {
		...
		$result = $this->Html->loadConfig ( array (
	 ...
	 *
	 * @return void @expectedException ConfigureException
	 */
	...
	public function testParseAttributeCompact() {
		...
		$this->assertEquals ( ' compact="compact"', $helper->parseAttributes ( array (
				'compact' 
		) ) );
		...
		$expected = ' require';
		$this->assertEquals ( $expected, $helper->parseAttributes ( array (
				'require' 
		) ) );
		$this->assertEquals ( $expected, $helper->parseAttributes ( array (
				'require' => true 
		) ) );
		$this->assertEquals ( '', $helper->parseAttributes ( array (
				'require' => false 
		) ) );
	}
}
