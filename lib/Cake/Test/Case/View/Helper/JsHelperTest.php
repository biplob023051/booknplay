...
class JsHelperTest extends CakeTestCase {
	...
	protected function _useMock() {
		...
		if (! class_exists ( 'TestJsEngineHelper', false )) {
			$this->getMock ( 'JsBaseEngineHelper', array (), array (
					$this->View 
			), 'TestJsEngineHelper' );
		}
		
		$this->Js = new JsHelper ( $this->View, array (
				'TestJs' 
		) );
	...
	public function testConstruction() {
		...
		$js = new JsHelper ( $this->View, array (
				'mootools' 
		) );
		$this->assertEquals ( array (
				'Html',
	...
	public function testMethodDispatching() {
		...
		$this->Js->TestJsEngine->expects ( $this->once () )->method ( 'event' )->with ( 'click', 'callback' );
	...
	public function testEventDispatchWithBuffering() {
		...
		$this->Js->TestJsEngine->bufferedMethods = array (
				'event',
				'sortables' 
		);
		$this->Js->TestJsEngine->expects ( $this->exactly ( 3 ) )->method ( 'event' )->will ( $this->returnValue ( 'This is an event call' ) );
		...
		$result = $this->Js->event ( 'click', 'foo', array (
				'buffer' => false 
		) );
	...
	public function testEffectDispatchWithBuffering() {
		$this->_useMock ();
		$this->Js->TestJsEngine->expects ( $this->exactly ( 4 ) )->method ( 'effect' )->will ( $this->returnValue ( 'I am not buffered.' ) );
		...
		$result = $this->Js->effect ( 'slideIn', array (
				'speed' => 'slow' 
		), true );
		$buffer = $this->Js->getBuffer ();
		$this->assertNull ( $result );
	...
	public function testWriteScriptsNoFile() {
		...
		$result = $this->Js->writeBuffer ( array (
				'onDomReady' => false,
				'cache' => false,
				'clear' => false 
		) );
		...
		$this->View->expects ( $this->once () )->method ( 'append' )->with ( 'script', $this->matchesRegularExpression ( '/one\s\=\s1;\ntwo\s\=\s2;/' ) );
		$result = $this->Js->writeBuffer ( array (
				'onDomReady' => false,
				'inline' => false,
				'cache' => false 
	...
	public function testWriteBufferNotInline() {
		...
		$this->View->expects ( $this->once () )->method ( 'append' )->with ( 'script', $this->matchesRegularExpression ( '#<script type="text\/javascript">window.app \= \{"foo"\:1\}\;<\/script>#' ) );
	...
	public function testLinkWithMock() {
		...
		$this->Js->TestJsEngine->expects ( $this->at ( 0 ) )->method ( 'get' );
		
		$this->Js->TestJsEngine->expects ( $this->at ( 1 ) )->method ( 'request' )->with ( '/posts/view/1', $options )->will ( $this->returnValue ( '--ajax code--' ) );
		
		$this->Js->TestJsEngine->expects ( $this->at ( 2 ) )->method ( 'event' )->with ( 'click', '--ajax code--', $options + array (
				'buffer' => null 
		) );
	...
	public function testLinkWithMockAndConfirm() {
		...
		$this->Js->TestJsEngine->expects ( $this->once () )->method ( 'confirmReturn' )->with ( $options ['confirm'] )->will ( $this->returnValue ( '--confirm script--' ) );
		
		$this->Js->TestJsEngine->expects ( $this->once () )->method ( 'request' )->with ( '/posts/view/1' );
		
		$this->Js->TestJsEngine->expects ( $this->once () )->method ( 'event' )->with ( 'click', '--confirm script--' );
	...
	public function testLinkWithNoBuffering() {
		...
		$this->Js->TestJsEngine->expects ( $this->at ( 1 ) )->method ( 'request' )->with ( '/posts/view/1', array (
				'update' => '#content' 
		) )->will ( $this->returnValue ( 'ajax code' ) );
		
		$this->Js->TestJsEngine->expects ( $this->at ( 2 ) )->method ( 'event' )->will ( $this->returnValue ( '-event handler-' ) );
	...
	public function testSubmitWithMock() {
		...
		$this->Js->TestJsEngine->expects ( $this->at ( 1 ) )->method ( 'serializeForm' )->will ( $this->returnValue ( 'serialize-code' ) );
		
		$this->Js->TestJsEngine->expects ( $this->at ( 2 ) )->method ( 'request' )->will ( $this->returnValue ( 'ajax-code' ) );
		...
		$this->Js->TestJsEngine->expects ( $this->at ( 3 ) )->method ( 'event' )->with ( 'click', "ajax-code", $params );
	...
	public function testSubmitWithMockRequestParams() {
		...
		$this->Js->TestJsEngine->expects ( $this->at ( 2 ) )->method ( 'request' )->with ( '/custom/url', $requestParams )->will ( $this->returnValue ( 'ajax-code' ) );
	...
	public function testSubmitWithNoBuffer() {
		...
		$this->Js->TestJsEngine->expects ( $this->at ( 3 ) )->method ( 'event' )->will ( $this->returnValue ( 'event-handler' ) );
	...
	public function testSetVarsAtTopOfBufferedScripts() {
		...
		$this->Js->alert ( 'hey you!', array (
				'buffer' => true 
		) );
		$this->Js->confirm ( 'Are you sure?', array (
				'buffer' => true 
		) );
...
class JsBaseEngineTest extends CakeTestCase {
	...
	public function testRedirect() {
		$result = $this->JsEngine->redirect ( array (
				'controller' => 'posts',
				'action' => 'view',
				1 
		) );
	...
	public function testObject() {
		...
		$result = $this->JsEngine->object ( $object, array (
				'prefix' => 'PREFIX',
				'postfix' => 'POSTFIX' 
		) );
