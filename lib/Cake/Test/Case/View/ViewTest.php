...
class ViewTest extends CakeTestCase {
	...
	public function testGetTemplate() {
		...
		$this->Controller->params ['pass'] = array (
				'home' 
		);
	...
	public function testMissingView() {
		...
		$this->ThemeController->params ['pass'] = array (
				'home' 
		);
	...
	public function testUUIDGeneration() {
		$result = $this->View->uuid ( 'form', array (
				'controller' => 'posts',
				'action' => 'index' 
		) );
	...
	public function testElement() {
		...
		$result = $this->View->element ( 'plugin_element', array (), array (
				'plugin' => 'TestPlugin' 
		) );
	...
	public function testElementInexistent2() {
		$this->View->element ( 'TestPlugin.plugin_element', array (), array (
				'plugin' => 'test_plugin' 
		) );
	}
	...
	public function testElementCallbacks() {
		$this->getMock ( 'Helper', array (), array (
				$this->View 
		), 'ElementCallbackMockHtmlHelper' );
		$this->View->helpers = array (
				'ElementCallbackMockHtml' 
		);
		...
		$this->View->element ( 'test_element', array (), array (
				'callbacks' => true 
		) );
	...
	public function testElementParamsDontOverwriteHelpers() {
		...
		$result = $View->element ( 'type_check', array (
				'form' => 'string' 
		), array (
				'callbacks' => true 
		) );
		...
		$result = $View->element ( 'type_check', array (), array (
				'callbacks' => true 
		) );
		$this->assertEquals ( 'string', $result );
	}
	...
	public function testElementCache() {
		...
		$result = $View->element ( 'test_element', array (), array (
				'cache' => true 
		) );
	...
	public function testHelperCallbackTriggering() {
		$View = new View ( $this->PostsController );
		$View->helpers = array ();
		$View->Helpers = $this->getMock ( 'HelperCollection', array (
				'trigger' 
		), array (
				$View 
		) );
		
		$View->Helpers->expects ( $this->at ( 0 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.beforeRender' ), $this->attributeEqualTo ( '_subject', $View ) ) );
		$View->Helpers->expects ( $this->at ( 1 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.beforeRenderFile' ), $this->attributeEqualTo ( '_subject', $View ) ) );
		
		$View->Helpers->expects ( $this->at ( 2 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.afterRenderFile' ), $this->attributeEqualTo ( '_subject', $View ) ) );
		$View->Helpers->expects ( $this->at ( 3 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.afterRender' ), $this->attributeEqualTo ( '_subject', $View ) ) );
		
		$View->Helpers->expects ( $this->at ( 4 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.beforeLayout' ), $this->attributeEqualTo ( '_subject', $View ) ) );
		
		$View->Helpers->expects ( $this->at ( 5 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.beforeRenderFile' ), $this->attributeEqualTo ( '_subject', $View ) ) );
		
		$View->Helpers->expects ( $this->at ( 6 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.afterRenderFile' ), $this->attributeEqualTo ( '_subject', $View ) ) );
		
		$View->Helpers->expects ( $this->at ( 7 ) )->method ( 'trigger' )->with ( $this->logicalAnd ( $this->isInstanceOf ( 'CakeEvent' ), $this->attributeEqualTo ( '_name', 'View.afterLayout' ), $this->attributeEqualTo ( '_subject', $View ) ) );
	...
	public function testRender() {
		...
		$this->PostsController->cacheAction = array (
				'index' => 3600 
		);
	...
	public function testGetViewFileNameSubdirWithPluginAndViewPath() {
		...
		$expected = CAKE . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS . 'TestPlugin' . DS . 'View' . DS . 'Elements' . DS . 'sub_dir' . DS . 'sub_element.ctp';
	...
	public function testSet() {
		...
		$this->assertSame ( $View->viewVars, array (
				'somekey' => 'someValue' 
		) );
		$this->assertSame ( $View->getVars (), array (
				'somekey' 
		) );
