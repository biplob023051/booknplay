...
class HelperTestPostsTag extends Model {
	...
	public function schema($field = false) {
		$this->_schema = array (
				'helper_test_post_id' => array (
						'type' => 'integer',
						'null' => false,
						'default' => '',
						'length' => '8' 
				),
				'helper_test_tag_id' => array (
						'type' => 'integer',
						'null' => false,
						'default' => '',
						'length' => '8' 
...
class HelperTest extends CakeTestCase {
	...
	public function testSetEntityScoped() {
		...
		$expected = array (
				'ModelThatDoesntExist',
				'field_that_doesnt_exist' 
		);
		$this->assertEquals ( $expected, $this->Helper->entity () );
	}
	...
	public function testValueWithDefault() {
		...
		$result = $this->Helper->value ( array (
				'default' => 'something' 
		), 'zero' );
		$this->assertEquals ( array (
				'value' => 0 
	...
	/**
	 * Ensure HTML escaping of url params.
	 * So link addresses are valid and not exploited
	 *
	 * @return void
	 */
	...
	public function testAssetUrl() {
		...
		$result = $this->Helper->assetUrl ( 'foo.jpg', array (
				'pathPrefix' => 'img/' 
		) );
		...
		$result = $this->Helper->assetUrl ( 'style', array (
				'ext' => '.css' 
		) );
		$this->assertEquals ( 'style.css', $result );
		
		$result = $this->Helper->assetUrl ( 'dir/sub dir/my image', array (
				'ext' => '.jpg' 
		) );
	...
	public function testAssetUrlNoRewrite() {
		...
		$result = $this->Helper->assetUrl ( 'img/cake.icon.png', array (
				'fullBase' => true 
		) );
	...
	public function testAssetUrlPlugin() {
		...
		$result = $this->Helper->assetUrl ( 'TestPlugin.style', array (
				'ext' => '.css' 
		) );
	...
	public function testAssetUrlTimestampForce() {
		...
		$result = $this->Helper->assetUrl ( 'cake.generic.css', array (
				'pathPrefix' => CSS_URL 
		) );
		$this->assertRegExp ( '/' . preg_quote ( CSS_URL . 'cake.generic.css?', '/' ) . '[0-9]+/', $result );
	}
	...
	public function testClean() {
		...
		$result = $this->Helper->clean ( array (
				'<script>with something</script>',
				'<applet>something else</applet>' 
		) );
		$this->assertEquals ( array (
				'with something',
				'something else' 
