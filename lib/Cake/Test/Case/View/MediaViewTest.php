...
class MediaViewTest extends CakeTestCase {
	...
	public function testRender() {
		...
		$this->MediaView->response->expects ( $this->once () )->method ( 'disableCache' );
		
		$this->MediaView->response->expects ( $this->once () )->method ( 'file' )->with ( $vars ['path'] . $vars ['id'], array (
				'name' => null,
				'download' => null 
		) );
	...
	public function testRenderCachingAndName() {
		...
		$this->MediaView->response->expects ( $this->never () )->method ( 'disableCache' );
		
		$this->MediaView->response->expects ( $this->once () )->method ( 'cache' )->with ( $this->anything (), $vars ['cache'] );
	...
	public function testRenderUpperExtension() {
		...
		$this->MediaView->response->expects ( $this->any () )->method ( 'type' )->with ( 'jpg' )->will ( $this->returnArgument ( 0 ) );
		
		$this->MediaView->response->expects ( $this->at ( 0 ) )->method ( 'send' )->will ( $this->returnValue ( true ) );
