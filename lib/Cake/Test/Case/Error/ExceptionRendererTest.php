...
class ExceptionRendererTest extends CakeTestCase {
	...
	protected function _mockResponse($error) {
		$error->controller->response = $this->getMock ( 'CakeResponse', array (
				'_sendHeader' 
		) );
		return $error;
	}
	...
	public function testExceptionResponseHeader() {
		...
		$exception->responseHeader ( array (
				'Allow: POST, DELETE' 
		) );
		$ExceptionRenderer = new ExceptionRenderer ( $exception );
	...
	public static function testProvider() {
		return array (
				...
				array (
						new PrivateActionException ( array (
								'controller' => 'PostsController',
								'action' => '_secretSauce' 
						) ),
						array (
				...
				array (
						new MissingTableException ( array (
								'table' => 'articles',
								'class' => 'Article',
								'ds' => 'test' 
						) ),
				...
				array (
						new MissingDatabaseException ( array (
								'connection' => 'default' 
						) ),
						array (
				...
				array (
						new MissingViewException ( array (
								'file' => '/posts/about.ctp' 
						) ),
						array (
								"/posts\/about.ctp/" 
						),
				...
				array (
						new MissingLayoutException ( array (
				...
				array (
						new MissingConnectionException ( array (
								'class' => 'Mysql' 
						) ),
						array (
								'/<h2>Missing Database Connection<\/h2>/',
				...
				array (
						new MissingDatasourceConfigException ( array (
								'config' => 'default' 
						) ),
						array (
				...
				array (
						new MissingDatasourceException ( array (
								'class' => 'MyDatasource',
								'plugin' => 'MyPlugin' 
						) ),
						array (
				...
				array (
						new MissingHelperException ( array (
				...
				array (
						new MissingBehaviorException ( array (
				...
				array (
						new MissingComponentException ( array (
								'class' => 'SideboxComponent' 
						) ),
						array (
	...
	public function testMissingRenderSafe() {
		$exception = new MissingHelperException ( array (
				'class' => 'Fail' 
		) );
		$ExceptionRenderer = new ExceptionRenderer ( $exception );
		
		$ExceptionRenderer->controller = $this->getMock ( 'Controller', array (
				'render' 
		) );
		$ExceptionRenderer->controller->helpers = array (
				'Fail',
				'Boom' 
		);
		$ExceptionRenderer->controller->request = $this->getMock ( 'CakeRequest' );
		$ExceptionRenderer->controller->expects ( $this->at ( 0 ) )->method ( 'render' )->with ( 'missingHelper' )->will ( $this->throwException ( $exception ) );
		
		$response = $this->getMock ( 'CakeResponse' );
		$response->expects ( $this->once () )->method ( 'body' )->with ( $this->stringContains ( 'Helper class Fail' ) );
		...
		$this->assertEquals ( array (
				...
				'Html',
				'Session' 
		), $ExceptionRenderer->controller->helpers );
	}
	...
	public function testRenderExceptionInBeforeRender() {
		...
		$ExceptionRenderer->controller->request = $this->getMock ( 'CakeRequest' );
		$ExceptionRenderer->controller->expects ( $this->any () )->method ( 'beforeRender' )->will ( $this->throwException ( $exception ) );
		
		$response = $this->getMock ( 'CakeResponse' );
		$response->expects ( $this->once () )->method ( 'body' )->with ( $this->stringContains ( 'Not there, sorry' ) );
	...
	public function testMissingSubdirRenderSafe() {
		...
		$ExceptionRenderer->controller->expects ( $this->once () )->method ( 'render' )->with ( 'error400' )->will ( $this->throwException ( $exception ) );
		
		$response = $this->getMock ( 'CakeResponse' );
		$response->expects ( $this->once () )->method ( 'body' )->with ( $this->stringContains ( 'Not Found' ) );
		$response->expects ( $this->once () )->method ( 'type' )->with ( 'html' );
