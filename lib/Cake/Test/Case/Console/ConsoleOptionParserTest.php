...
class ConsoleOptionParserTest extends CakeTestCase {
	...
	public function testDescription() {
		...
		$result = $parser->description ( array (
				'A test',
				'something' 
		) );
	...
	public function testAddOptionZero() {
		...
		$this->assertEquals ( array (
				'count' => '0',
				'help' => false 
		), $result [0], 'Zero parameter did not parse out' );
	}
	...
	public function testMultipleOptions() {
		$parser = new ConsoleOptionParser ( 'test', false );
		$parser->addOption ( 'test' )->addOption ( 'connection' )->addOption ( 'table', array (
				'short' => 't',
				'default' => true 
		) );
	...
	public function testOptionWithBooleanParam() {
		$parser = new ConsoleOptionParser ( 'test', false );
		$parser->addOption ( 'no-commit', array (
				'boolean' => true 
		) )->addOption ( 'table', array (
				'short' => 't' 
		) );
	...
	public function testOptionWithValueStartingWithMinus() {
		$parser = new ConsoleOptionParser ( 'test', false );
		$parser->addOption ( 'name' )->addOption ( 'age' );
	...
	public function testPositionalArgument() {
		$parser = new ConsoleOptionParser ( 'test', false );
		$result = $parser->addArgument ( 'name', array (
				'help' => 'An argument' 
		) );
	...
	public function testAddArgumentOutOfOrder() {
		...
		$parser->addArgument ( 'name', array (
				'index' => 1,
				'help' => 'first argument' 
		) )->addArgument ( 'bag', array (
				'index' => 2,
				'help' => 'second argument' 
		) )->addArgument ( 'other', array (
				'index' => 0,
				'help' => 'Zeroth argument' 
		) );
	...
	public function testHelpSubcommandHelp() {
		...
		$subParser->addOption ( 'connection', array (
				'help' => 'Db connection.' 
		) );
...
TEXT;
	...
	public function testParsingWithSubParser() {
		$parser = new ConsoleOptionParser ( 'test', false );
		$parser->addOption ( 'primary' )->addArgument ( 'one', array (
				'required' => true,
				'choices' => array (
						'a',
						'b' 
				) 
		) )->addArgument ( 'two', array (
				'required' => true 
