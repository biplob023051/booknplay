...
class TestShellTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Shell = $this->getMock ( 'TestTestShell', array (
				'in',
				'out',
				'hr',
				'help',
		...
		$this->Shell->OptionParser = $this->getMock ( 'ConsoleOptionParser', array (), array (
				null,
				false 
		) );
	}
	...
	public function testAvailableCoreCategory() {
		...
		$this->Shell->expects ( $this->at ( 1 ) )->method ( 'out' )->with ( $this->stringContains ( '[1]' ) );
		$this->Shell->expects ( $this->at ( 2 ) )->method ( 'out' )->with ( $this->stringContains ( '[2]' ) );
		
		$this->Shell->expects ( $this->once () )->method ( 'in' )->with ( __d ( 'cake_console', 'What test case would you like to run?' ), null, 'q' )->will ( $this->returnValue ( '1' ) );
	...
	public function testRunnerOptions() {
		...
		$this->Shell->expects ( $this->once () )->method ( '_run' )->with ( array (
				'app' => false,
				'plugin' => null,
				'core' => true,
				'output' => 'text',
