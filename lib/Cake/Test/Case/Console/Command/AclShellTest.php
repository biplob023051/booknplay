...
class AclShellTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Task = $this->getMock ( 'AclShell', array (
				'in',
				'out',
				'hr',
				'createFile',
	...
	public function testViewWithModelForeignKeyOutput() {
		...
		$this->Task->expects ( $this->at ( 2 ) )->method ( 'out' )->with ( $this->stringContains ( '[1] ROOT' ) );
		
		$this->Task->expects ( $this->at ( 4 ) )->method ( 'out' )->with ( $this->stringContains ( '[3] Gandalf' ) );
		
		$this->Task->expects ( $this->at ( 6 ) )->method ( 'out' )->with ( $this->stringContains ( '[5] MyModel.2' ) );
	...
	/**
	 * test the method that splits model.foreign key.
	...
	public function testDelete() {
		...
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'out' )->with ( "<success>Aro deleted.</success>", 2 );
		$this->Task->delete ();
	...
	public function testGrant() {
		...
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'out' )->with ( $this->matchesRegularExpression ( '/granted/' ), true );
		$this->Task->grant ();
		$node = $this->Task->Acl->Aro->node ( array (
				'model' => 'AuthUser',
				'foreign_key' => 2 
		) );
	...
	public function testDeny() {
		...
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'out' )->with ( $this->stringContains ( 'Permission denied' ), true );
	...
	public function testCheck() {
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'out' )->with ( $this->matchesRegularExpression ( '/not allowed/' ), true );
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'out' )->with ( $this->matchesRegularExpression ( '/granted/' ), true );
		$this->Task->expects ( $this->at ( 2 ) )->method ( 'out' )->with ( $this->matchesRegularExpression ( '/is.*allowed/' ), true );
		$this->Task->expects ( $this->at ( 3 ) )->method ( 'out' )->with ( $this->matchesRegularExpression ( '/not.*allowed/' ), true );
	...
	public function testInherit() {
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'out' )->with ( $this->matchesRegularExpression ( '/Permission .*granted/' ), true );
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'out' )->with ( $this->matchesRegularExpression ( '/Permission .*inherited/' ), true );
	...
	public function testInitDb() {
		$this->Task->expects ( $this->once () )->method ( 'dispatchShell' )->with ( 'schema create DbAcl' );
