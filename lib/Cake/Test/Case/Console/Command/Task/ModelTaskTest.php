...
class ModelTaskTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Task = $this->getMock ( 'ModelTask', array (
				'in',
				'err',
				'createFile',
				'_stop',
	...
	/**
	 * Setup a mock that has out mocked.
	 * Normally this is not used as it makes $this->at() really tricky.
	 *
	 * @return void
	 */
	...
	protected function _setupOtherMocks() {
		...
		$this->Task->Fixture = $this->getMock ( 'FixtureTask', array (), array (
				$out,
				$out,
				$in 
		) );
		$this->Task->Test = $this->getMock ( 'FixtureTask', array (), array (
				$out,
				$out,
				$in 
		) );
	...
	public function testGetTableOddTableInteractive() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( 2 ) // bake_odd
 );
	...
	public function testInteractiveFieldValidation() {
		$this->Task->initValidations ();
		$this->Task->interactive = true;
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( '24', 'y', '18', 'n' ) );
	...
	public function testInteractiveFieldValidationWithBogusResponse() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( '999999', '24', 'n' ) );
		
		$this->Task->expects ( $this->at ( 10 ) )->method ( 'out' )->with ( $this->stringContains ( 'make a valid' ) );
	...
	public function testInteractiveFieldValidationWithRegexp() {
		$this->Task->initValidations ();
		$this->Task->interactive = true;
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( '/^[a-z]{0,9}$/', 'n' ) );
	...
	public function testFindPrimaryKey() {
		...
		$this->Task->expects ( $this->once () )->method ( 'in' )->with ( $anything, null, 'key' )->will ( $this->returnValue ( 'my_field' ) );
	...
	public function testFindDisplayName() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( 'y', 2 ) );
	...
	public function testConfirmAssociations() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( 'n', 'y', 'n', 'n', 'n' ) );
...
STRINGEND;
	...
	public function testBakeWithPlugin() {
		...
		// fake plugin path
		CakePlugin::load ( 'ControllerTest', array (
				'path' => APP . 'Plugin' . DS . 'ControllerTest' . DS 
		) );
		$path = APP . 'Plugin' . DS . 'ControllerTest' . DS . 'Model' . DS . 'BakeArticle.php';
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $path, $this->stringContains ( 'BakeArticle extends ControllerTestAppModel' ) );
	...
	public function testExecuteWithNamedModel() {
		...
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeArticle extends AppModel' ) );
	...
	public function testExecuteWithNamedModelVariations($name) {
		...
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeArticle extends AppModel' ) );
		$this->Task->execute ();
	}
	...
	public function testExecuteWithNamedModelHasManyCreated() {
		...
		$this->Task->expects ( $this->once () )->method ( '_checkUnitTest' )->will ( $this->returnValue ( 1 ) );
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( "'BakeComment' => array(" ) );
	...
	public function testExecuteIntoAll() {
		...
		$filename = '/my/path/BakeArticle.php';
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeArticle' ) );
		
		$filename = '/my/path/BakeArticlesBakeTag.php';
		$this->Task->expects ( $this->at ( 2 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeArticlesBakeTag' ) );
		
		$filename = '/my/path/BakeComment.php';
		$this->Task->expects ( $this->at ( 3 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeComment' ) );
		
		$filename = '/my/path/BakeComment.php';
		$this->Task->expects ( $this->at ( 3 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'public $primaryKey = \'otherid\';' ) );
		
		$filename = '/my/path/BakeTag.php';
		$this->Task->expects ( $this->at ( 4 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeTag' ) );
		
		$filename = '/my/path/BakeTag.php';
		$this->Task->expects ( $this->at ( 4 ) )->method ( 'createFile' )->with ( $filename, $this->logicalNot ( $this->stringContains ( 'public $primaryKey' ) ) );
		
		$filename = '/my/path/CategoryThread.php';
		$this->Task->expects ( $this->at ( 5 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class CategoryThread' ) );
	...
	public function testSkipTablesAndAll() {
		...
		$filename = '/my/path/CategoryThread.php';
		$this->Task->expects ( $this->at ( 4 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class CategoryThread' ) );
	...
	public function testExecuteIntoInteractive() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( $article, // article
'n', // no validation
'y', // associations
'y', // comment relation
'y', // user relation
...
'y' ) // looks good?
	...
	public function testExecuteWithNonExistantTableName() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( 'Foobar', // Or type in the name of the model
