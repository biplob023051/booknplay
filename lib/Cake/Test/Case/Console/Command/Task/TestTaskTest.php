...
class TestTaskTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Task = $this->getMock ( 'TestTask', array (
				'in',
				'err',
				'createFile',
				'_stop',
	...
	public function testFilePathGenerationModelRepeated() {
		...
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'createFile' )->with ( $file, $this->anything () );
		
		$this->Task->expects ( $this->at ( 3 ) )->method ( 'createFile' )->with ( $file, $this->anything () );
		
		$file = TESTS . 'Case' . DS . 'Controller' . DS . 'CommentsControllerTest.php';
		$this->Task->expects ( $this->at ( 5 ) )->method ( 'createFile' )->with ( $file, $this->anything () );
	...
	public function testGetUserFixtures() {
		...
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'in' )->will ( $this->returnValue ( 'app.pizza, app.topping, app.side_dish' ) );
	...
	/**
	 * test baking files.
	 * The conditionally run tests are known to fail in PHP4
	...
	public function testInteractiveWithPlugin() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( 5, // helper
1 ) // OtherHelper
 );
		
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $path, $this->anything () );
		
		$this->Task->stdout->expects ( $this->at ( 21 ) )->method ( 'write' )->with ( '1. OtherHelperHelper' );
	...
	public static function caseFileNameProvider() {
		return array (
				array (
						'Model',
						'Post',
						'Case' . DS . 'Model' . DS . 'PostTest.php' 
				),
				array (
						'Helper',
						'Form',
						'Case' . DS . 'View' . DS . 'Helper' . DS . 'FormHelperTest.php' 
				),
				array (
						'Controller',
						'Posts',
						'Case' . DS . 'Controller' . DS . 'PostsControllerTest.php' 
				),
				array (
						'Behavior',
						'Containable',
				...
				array (
						'Component',
						'Auth',
						'Case' . DS . 'Controller' . DS . 'Component' . DS . 'AuthComponentTest.php' 
				),
				array (
						'model',
						'Post',
	...
	public function testExecuteWithOneArg() {
		...
		$this->Task->expects ( $this->once () )->method ( 'isLoadableClass' )->will ( $this->returnValue ( true ) );
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $this->anything (), $this->stringContains ( 'class TestTaskTagTest extends CakeTestCase' ) );
		$this->Task->execute ();
	}
