...
class PluginTaskTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Task = $this->getMock ( 'PluginTask', array (
				'in',
				'err',
				'createFile',
				'_stop',
	...
	public function testExecuteWithNoArgs() {
		...
		$this->Task->expects ( $this->at ( 4 ) )->method ( 'createFile' )->with ( $file, new PHPUnit_Framework_Constraint_IsAnything () );
	...
	public function testFindPathNonExistant() {
		...
		$this->Task->expects ( $this->exactly ( $last ) )->method ( 'out' );
		
		$this->Task->expects ( $this->once () )->method ( 'in' )->will ( $this->returnValue ( $last ) );
