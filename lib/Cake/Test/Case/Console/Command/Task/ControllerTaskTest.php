...
class ControllerTaskTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Task = $this->getMock ( 'ControllerTask', array (
				'in',
				'out',
				'err',
				'hr',
		...
		$this->Task->Model = $this->getMock ( 'ModelTask', array (
				'in',
				'out',
				'err',
				'createFile',
		...
		$this->Task->Project = $this->getMock ( 'ProjectTask', array (
				'in',
				'out',
				'err',
				'createFile',
		...
		$this->Task->Test = $this->getMock ( 'TestTask', array (), array (
				$out,
				$out,
				$in 
		) );
	...
	public function testGetNameValidIndex() {
		...
		$this->Task->interactive = true;
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( 3, 1 ) );
	...
	public function testGetNameInvalidIndex() {
		$this->Task->interactive = true;
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( 50, 'q' ) );
	...
	public function testBakeWithPlugin() {
		...
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'createFile' )->with ( $path, new PHPUnit_Framework_Constraint_IsAnything () );
		$this->Task->expects ( $this->at ( 3 ) )->method ( 'createFile' )->with ( $path, $this->stringContains ( 'ArticlesController extends ControllerTestAppController' ) )->will ( $this->returnValue ( true ) );
	...
	/**
	 * test that bakeActions is creating the correct controller Code.
	 * (Using sessions)
	 *
	 * @return void
	 */
	...
	public function testInteractive() {
		...
		$this->Task->expects ( $this->any () )->method ( 'in' )->will ( $this->onConsecutiveCalls ( '1', 'y', // build interactive
...
'y' ) // looks good?
		...
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeArticlesController' ) );
		$this->Task->execute ();
	}
...
'y' ) // looks good?
		...
		$filename = '/my/path/BakeArticlesController.php';
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class BakeArticlesController' ) )->will ( $this->returnValue ( true ) );
	...
	public function testExecuteIntoAllAdmin() {
		...
		$filename = '/my/path/BakeArticlesController.php';
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'function admin_index' ) )->will ( $this->returnValue ( true ) );
	...
	public function testExecuteWithController() {
		...
		$filename = '/my/path/BakeArticlesController.php';
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $this->stringContains ( '$scaffold' ) );
	...
	public function testExecuteWithPublicParam() {
		...
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $expected );
		$this->Task->execute ();
	}
	...
	public function testExecuteWithControllerAndBoth() {
		...
		$filename = '/my/path/BakeArticlesController.php';
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'admin_index' ) );
		$this->Task->execute ();
	}
