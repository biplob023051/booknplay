...
class FixtureTaskTest extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Task = $this->getMock ( 'FixtureTask', array (
				'in',
				'err',
				'createFile',
				'_stop',
	...
	public function testImportRecordsFromDatabaseWithConditionsPoo() {
		...
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'in' )->will ( $this->returnValue ( 'WHERE 1=1' ) );
	...
	public function testImportRecordsNoEscaping() {
		...
		$Article->updateAll ( array (
				'body' => "'Body \"value\"'" 
		) );
		
		$this->Task->interactive = true;
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'in' )->will ( $this->returnValue ( 'WHERE 1=1 LIMIT 10' ) );
	...
	public function testExecuteWithNamedModel() {
		...
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class ArticleFixture' ) );
	...
	public function testExecuteIntoAll() {
		...
		$this->Task->Model->expects ( $this->any () )->method ( 'listAll' )->will ( $this->returnValue ( array (
				'articles',
				'comments' 
		) ) );
		...
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class CommentFixture' ) );
	...
	public function testAllWithCountAndRecordsFlags() {
		...
		$filename = '/my/path/ArticleFixture.php';
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( "'title' => 'Third Article'" ) );
		
		$filename = '/my/path/CommentFixture.php';
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( "'comment' => 'First Comment for First Article'" ) );
	...
	public function testExecuteInteractive() {
		...
		$this->Task->Model->expects ( $this->any () )->method ( 'getTable' )->with ( 'Article' )->will ( $this->returnValue ( 'articles' ) );
		
		$filename = '/my/path/ArticleFixture.php';
		$this->Task->expects ( $this->once () )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class ArticleFixture' ) );
	...
	public function testBake() {
		...
		$result = $this->Task->bake ( 'Article', 'comments', array (
				'records' => true 
		) );
	...
	public function testGenerateFixtureFile() {
		...
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'ArticleFixture' ) );
		
		$this->Task->expects ( $this->at ( 1 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( '<?php' ) );
	...
	public function testGeneratePluginFixtureFile() {
		...
		// fake plugin path
		CakePlugin::load ( 'TestFixture', array (
				'path' => APP . 'Plugin' . DS . 'TestFixture' . DS 
		) );
		$this->Task->expects ( $this->at ( 0 ) )->method ( 'createFile' )->with ( $filename, $this->stringContains ( 'class Article' ) );
