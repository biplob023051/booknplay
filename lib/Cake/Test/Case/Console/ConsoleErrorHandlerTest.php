...
class ConsoleErrorHandlerTest extends CakeTestCase {
	...
	public function setUp() {
		parent::setUp ();
		$this->Error = $this->getMock ( 'ConsoleErrorHandler', array (
				'_stop' 
		) );
	...
	public function testHandleFatalError() {
		...
		$this->Error->expects ( $this->once () )->method ( '_stop' )->with ( 1 );
	...
	public function testCakeErrors() {
		...
		ConsoleErrorHandler::$stderr->expects ( $this->once () )->method ( 'write' )->with ( $this->stringContains ( 'Missing action' ) );
		
		$this->Error->expects ( $this->once () )->method ( '_stop' )->with ( 404 );
	...
	public function testNonCakeExceptions() {
		...
		ConsoleErrorHandler::$stderr->expects ( $this->once () )->method ( 'write' )->with ( $this->stringContains ( 'Too many parameters.' ) );
	...
	public function testError500Exception() {
		...
		$this->Error->expects ( $this->once () )->method ( '_stop' )->with ( 500 );
