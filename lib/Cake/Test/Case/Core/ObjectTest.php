...
class ObjectTest extends CakeTestCase {
	...
	public function testLog() {
		...
		$this->assertTrue ( $this->object->log ( array (
				'Test' => 'warning 2' 
		) ) );
	...
	public function testMethodDispatching() {
		...
		$this->object->dispatchMethod ( 'oneParamMethod', array (
				'Hello' 
		) );
		$expected [] = array (
				'oneParamMethod' => array (
		...
		$this->object->dispatchMethod ( 'twoParamMethod', array (
				true,
				false 
		) );
		$expected [] = array (
		...
		$this->object->dispatchMethod ( 'threeParamMethod', array (
				true,
				false,
				null 
		) );
		...
		$this->object->dispatchMethod ( 'fourParamMethod', array (
				1,
				2,
				3,
				4 
		...
		$this->object->dispatchMethod ( 'fiveParamMethod', array (
				1,
				2,
				3,
				4,
		...
		$this->object->dispatchMethod ( 'crazyMethod', array (
				1,
				2,
				3,
				4,
		...
		$this->object->dispatchMethod ( 'methodWithOptionalParam', array (
				'Hello' 
		) );
		$expected [] = array (
				'methodWithOptionalParam' => array (
	...
	public function testRequestAction() {
		...
		$result = $this->object->requestAction ( '/tests_apps/index', array (
				'return' 
		) );
	...
	public function testRequestActionPlugins() {
		...
		$result = $this->object->requestAction ( '/test_plugin/tests/index', array (
				'return' 
		) );
		$expected = 'test plugin index';
		$this->assertEquals ( $expected, $result );
		
		$result = $this->object->requestAction ( '/test_plugin/tests/index/some_param', array (
				'return' 
		) );
		$expected = 'test plugin index';
		$this->assertEquals ( $expected, $result );
		
		$result = $this->object->requestAction ( array (
				'controller' => 'tests',
				'action' => 'index',
				'plugin' => 'test_plugin' 
		), array (
	...
	public function testRequestActionArray() {
		...
		$result = $this->object->requestAction ( array (
				'controller' => 'request_action',
				'action' => 'paginate_request_action' 
		) );
		$this->assertTrue ( $result );
	...
	public function testRequestActionRemoveReturnParam() {
		$result = $this->object->requestAction ( '/request_action/param_check', array (
				'return' 
		) );
	...
	public function testRequestActionGetParameters() {
		$result = $this->object->requestAction ( '/request_action/params_pass?get=value&limit=5' );
	...
	public function testRequestActionPostWithData() {
		...
		$result = $this->object->requestAction ( '/request_action/post_pass', array (
				'data' => $data 
		) );
		$this->assertEquals ( $data, $result );
	}
