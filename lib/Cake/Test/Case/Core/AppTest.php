...
class AppTest extends CakeTestCase {
	...
	public function testCore() {
		...
		$this->assertEquals ( array (
				CAKE . 'Controller' . DS . 'Component' . DS 
		), str_replace ( '/', DS, $component ) );
		
		$auth = App::core ( 'Controller/Component/Auth' );
		$this->assertEquals ( array (
				CAKE . 'Controller' . DS . 'Component' . DS . 'Auth' . DS 
		), str_replace ( '/', DS, $auth ) );
		
		$datasource = App::core ( 'Model/Datasource' );
		$this->assertEquals ( array (
				CAKE . 'Model' . DS . 'Datasource' . DS 
		), str_replace ( '/', DS, $datasource ) );
	}
	...
	/**
	 * Make sure that .
	 * svn and friends are excluded from App::objects('plugin')
	 */
	...
	public function testClassLoading() {
		...
		$file = App::import ( 'Model', array (
				'NonExistingPlugin.NonExistingModel' 
		), false );
		$this->assertFalse ( $file );
	...
	public function testLoadingWithSearch() {
		$file = App::import ( 'File', 'NewName', false, array (
				CAKE . 'Config' . DS 
		), 'config.php' );
		$this->assertTrue ( $file );
		
		$file = App::import ( 'File', 'AnotherNewName', false, array (
				CAKE 
		), 'config.php' );
		$this->assertFalse ( $file );
	}
	...
	public function testLoadingVendor() {
		App::build ( array (
				...
				'vendors' => array (
						CAKE . 'Test' . DS . 'test_app' . DS . 'Vendor' . DS 
				) 
		), App::RESET );
		CakePlugin::load ( array (
				'TestPlugin',
		...
		ob_start ();
		$result = App::import ( 'Vendor', 'css/TestAsset', array (
				'ext' => 'css' 
		) );
		$text = ob_get_clean ();
		$this->assertTrue ( $result );
		...
		ob_start ();
		$result = App::import ( 'Vendor', 'SomeNameInSubfolder', array (
				'file' => 'somename/some.name.php' 
		) );
		$text = ob_get_clean ();
		$this->assertTrue ( $result );
		...
		ob_start ();
		$result = App::import ( 'Vendor', 'TestHello', array (
				'file' => 'Test' . DS . 'hello.php' 
		) );
		$text = ob_get_clean ();
		$this->assertTrue ( $result );
		...
		ob_start ();
		$result = App::import ( 'Vendor', 'MyTest', array (
				'file' => 'Test' . DS . 'MyTest.php' 
		) );
		$text = ob_get_clean ();
		$this->assertTrue ( $result );
