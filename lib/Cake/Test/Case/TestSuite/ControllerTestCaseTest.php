...
class ControllerTestCaseTest extends CakeTestCase {
	...
	public function testGenerate() {
		...
		$Posts->RequestHandler->expects ( $this->once () )->method ( 'isPut' )->will ( $this->returnValue ( true ) );
		$this->assertTrue ( $Posts->RequestHandler->isPut () );
		
		$Posts->Auth->Session->expects ( $this->any () )->method ( 'write' )->will ( $this->returnValue ( 'written!' ) );
	...
	public function testGenerateWithPlugin() {
		...
		$Tests = $this->Case->generate ( 'ControllerTestCaseTest', array (
				'models' => array (
						'TestPlugin.TestPluginComment' => array (
								'save' 
						) 
				) 
		) );
		$this->assertInstanceOf ( 'TestPluginComment', $Tests->TestPluginComment );
		$Tests->TestPluginComment->expects ( $this->at ( 0 ) )->method ( 'save' )->will ( $this->returnValue ( true ) );
		$Tests->TestPluginComment->expects ( $this->at ( 1 ) )->method ( 'save' )->will ( $this->returnValue ( false ) );
	...
	public function testUseRoutes() {
		...
		$result = $this->Case->testAction ( '/tests_apps/index.json', array (
				'return' => 'contents' 
		) );
	...
	public function testSkipRoutes() {
		...
		$this->Case->testAction ( '/tests_apps/missing_action.json', array (
				'return' => 'view' 
		) );
	}
	...
	public function testTestActionGetData() {
		...
		$this->assertEquals ( array (
				'var1',
				'var2' 
		), array_keys ( $result ['params'] ['named'] ) );
