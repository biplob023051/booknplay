...
class CakeTestFixtureTest extends CakeTestCase {
	...
	public function setUp() {
		$methods = array_diff ( get_class_methods ( 'DboSource' ), array (
				'enabled' 
		) );
		$methods [] = 'connect';
	...
	public function testImport() {
		...
		ConnectionManager::create ( 'new_test_suite', array_merge ( $testSuiteConfig, array (
				'prefix' => 'new_' . $testSuiteConfig ['prefix'] 
		) ) );
	 ...
	 * Make sure to try with postgres as its
	...
	public function testInsert() {
		$Fixture = new CakeTestFixtureTestFixture ();
		$this->criticDb->expects ( $this->atLeastOnce () )->method ( 'insertMulti' )->will ( $this->returnCallback ( array (
				$this,
				'insertCallback' 
		) ) );
	...
	public function testInsertStrings() {
		...
		$this->assertEquals ( array (
				...
				'email',
				'age' 
		), array_values ( $this->insertMulti ['fields'] ) );
		$expected = array (
				array (
						'Mark Doe',
						'mark.doe@email.com',
