...
class SetTest extends CakeTestCase {
	...
	public function testEnum() {
		...
		$result = Set::enum ( 1, array (
				'one',
				'two' 
		) );
		$this->assertEquals ( 'two', $result );
		$result = Set::enum ( 2, array (
				'one',
				'two' 
		) );
		$this->assertNull ( $result );
		
		$result = Set::enum ( 'first', array (
				'first' => 'one',
				'second' => 'two' 
		) );
		$this->assertEquals ( 'one', $result );
		$result = Set::enum ( 'third', array (
				'first' => 'one',
				'second' => 'two' 
		) );
		$this->assertNull ( $result );
		
		$result = Set::enum ( 'no', array (
				'no' => 0,
				'yes' => 1 
		) );
		$this->assertEquals ( 0, $result );
		$result = Set::enum ( 'not sure', array (
				'no' => 0,
				'yes' => 1 
		) );
		$this->assertNull ( $result );
	...
	public function testMatches() {
		...
		$this->assertTrue ( Set::matches ( array (
				'id=2' 
		), $a [1] ['Article'] ) );
		$this->assertFalse ( Set::matches ( array (
				'id>2' 
		), $a [1] ['Article'] ) );
		$this->assertTrue ( Set::matches ( array (
				'id>=2' 
		...
		$this->assertEquals ( array (
				'comment',
				'published' 
		), Set::extract ( '/Article/keep/Comment/fields', $r ) );
		$this->assertEquals ( array (
				'user' 
		), Set::extract ( '/Article/keep/User/fields', $r ) );
	}
	...
	public function testInsert() {
		...
		$result = Set::insert ( $a, 'files', array (
				'name' => 'files' 
		) );
		$expected = array (
				'pages' => array (
	...
	public function testFormatting() {
		...
		$result = Set::format ( $data, '{1}, {0}', array (
				'{n}.Person.first_name',
				'{n}.Person.last_name' 
		) );
		$expected = array (
	...
	public function testFormattingNullValues() {
		...
		$result = Set::format ( $data, '%s', array (
				'{n}.Person.something' 
		) );
		$expected = array (
				'42',
		...
		$expected = array (
				array (
						"IndexedPage" => array (
								...
								'post_vars' => array (),
								'cookies' => array (
										'PHPSESSID' => "dde9896ad24595998161ffaf9e0dbe2d" 
								),
								'redirect' => '',
								'created' => "1195055503",
								'updated' => "1195055503" 
	...
	public function testPushDiff() {
		...
		$array2 = array (
				0 => array (
						'ModelOne' => array (
								'id' => 1001,
								'field_one' => 's2.0.m1.f1',
								'field_two' => 's2.0.m1.f2' 
						) 
				),
				1 => array (
						'ModelTwo' => array (
								'id' => 1002,
								'field_one' => 's2.1.m2.f2',
								'field_two' => 's2.1.m2.f2' 
						) 
				) 
		);
	...
	public function testApply() {
		...
		$result = Set::apply ( '/Movie/title', $data, 'ucfirst', array (
				'type' => 'map' 
		) );
		$expected = array (
				'Movie 3',
		...
		$result = Set::apply ( '/Movie/title', $data, 'strtoupper', array (
				'type' => 'map' 
		) );
		$expected = array (
				'MOVIE 3',
		...
		$result = Set::apply ( '/Movie/rating', $data, array (
				'SetTest',
				'method' 
		), array (
				'type' => 'reduce' 
		...
		$result = Set::apply ( '/Movie/rating', $data, 'strtoupper', array (
	...
	/**
	 * test Set nest with a normal model result set.
	 * For kicks rely on Set nest detecting the key names
	 * automatically
	 *
	 * @return void
	 */
