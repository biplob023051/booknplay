...
class DebuggerTest extends CakeTestCase {
	...
	public function testOutput() {
		...
		$this->assertTags ( $result [0], array (
				...
				'a' => array (
						...
						'onclick' => "preg:/document\.getElementById\('cakeErr[a-z0-9]+\-trace'\)\.style\.display = " . "\(document\.getElementById\('cakeErr[a-z0-9]+\-trace'\)\.style\.display == 'none'" . " \? '' \: 'none'\);/" 
				),
				'b' => array (),
				'Notice',
				'/b',
	...
	public function testChangeOutputFormats() {
		...
		Debugger::output ( 'js', array (
				'traceLine' => '{:reference} - <a href="txmt://open?url=file://{:file}' . '&line={:line}">{:path}</a>, line {:line}' 
		) );
		$result = Debugger::trace ();
		$this->assertRegExp ( '/' . preg_quote ( 'txmt://open?url=file://', '/' ) . '(\/|[A-Z]:\\\\)' . '/', $result );
		
		Debugger::output ( 'xml', array (
				'error' => '<error><code>{:code}</code><file>{:file}</file><line>{:line}</line>' . '{:description}</error>',
	...
	public function testAddFormatCallback() {
		...
		Debugger::addFormat ( 'callback', array (
				'callback' => array (
						$this,
						'customFormat' 
				) 
