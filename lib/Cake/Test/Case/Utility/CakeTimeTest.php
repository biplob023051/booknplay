...
class CakeTimeTest extends CakeTestCase {
	...
	public function testTimeAgoInWordsEnd($input, $expected, $end) {
		$result = $this->Time->timeAgoInWords ( $input, array (
				'end' => $end 
		) );
		$this->assertEquals ( $expected, $result );
	}
	...
	public function testTimeAgoInWordsAccuracy() {
		$result = $this->Time->timeAgoInWords ( strtotime ( '+8 years +4 months +2 weeks +3 days' ), array (
				'accuracy' => array (
						'year' => 'year' 
				),
				'end' => '+10 years' 
		...
		$result = $this->Time->timeAgoInWords ( strtotime ( '+1 years +5 weeks' ), array (
				'accuracy' => array (
						'year' => 'year' 
				),
				'end' => '+10 years' 
	...
	public function testTimeAgoInWordsWithFormat() {
		...
		$result = $this->Time->timeAgoInWords ( strtotime ( '+2 weeks +2 days' ), 'Y-m-d' );
		$this->assertRegExp ( '/^2 weeks, [1|2] day(s)?$/', $result );
		
		$result = $this->Time->timeAgoInWords ( strtotime ( '+2 months +2 days' ), array (
				'end' => '1 month',
				'format' => 'Y-m-d' 
		) );
	...
	public function testTimeAgoInWordsNegativeValues() {
		...
		$result = $this->Time->timeAgoInWords ( strtotime ( '-2 years -5 months -2 days' ), array (
				'end' => '3 years' 
		) );
		...
		$result = $this->Time->timeAgoInWords ( strtotime ( '-1 month -1 week -6 days' ), array (
				'end' => '1 year',
				'accuracy' => array (
						'month' => 'month' 
				) 
		...
		$result = $this->Time->timeAgoInWords ( $timestamp, array (
				'accuracy' => array (
						'year' => 'year' 
				) 
		) );
		...
		$result = $this->Time->timeAgoInWords ( strtotime ( '-13 months -5 days' ), array (
				'end' => '2 years' 
		) );
