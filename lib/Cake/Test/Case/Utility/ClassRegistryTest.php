...
class ClassRegistryTest extends CakeTestCase {
	...
	public function testAddModel() {
		...
		$NewTag = ClassRegistry::init ( array (
				'class' => 'RegisterArticleTag',
				'alias' => 'NewTag' 
		) );
		$this->assertTrue ( is_a ( $Tag, 'RegisterArticleTag' ) );
		
		$NewTagCopy = ClassRegistry::init ( array (
				'class' => 'RegisterArticleTag',
				'alias' => 'NewTag' 
		) );
		...
		$UserCopy = ClassRegistry::init ( array (
				'class' => 'RegisterUser',
				'alias' => 'User',
				'table' => false 
		) );
		...
		$Category = ClassRegistry::init ( array (
				'class' => 'RegisterCategory' 
		) );
		$this->assertTrue ( is_a ( $Category, 'RegisterCategory' ) );
		
		$ParentCategory = ClassRegistry::init ( array (
				'class' => 'RegisterCategory',
	...
	public function testPluginAppModel() {
		...
		// Faking a plugin
		CakePlugin::load ( 'RegistryPlugin', array (
				'path' => '/fake/path' 
		) );
	...
	/**
	 * Test that you cannot init() an abstract class.
