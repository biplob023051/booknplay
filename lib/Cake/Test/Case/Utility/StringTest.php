...
class StringTest extends CakeTestCase {
	...
	public function testInsert() {
		...
		$result = String::insert ( ':I.am: :not.yet: passing.', array (
				'I.am' => 'We are' 
		), array (
				'before' => ':',
				'after' => ':',
		...
		$result = String::insert ( '?-pended result', array (
				'Pre' 
		) );
	...
	public function testCleanInsert() {
		...
		$result = String::cleanInsert ( '<p class=":missing" id=":missing">Text here</p>', array (
				'clean' => 'html',
				'before' => ':',
				'after' => '' 
		) );
	...
	/**
	 * Tests that non-insertable variables (i.e.
	 * arrays) are skipped when used as values in
	 * String::insert().
	 *
	 * @return void
	 */
	public function testAutoIgnoreBadInsertData() {
		...
		$result = String::insert ( '(:foo > :bar || :fale!)', $data, array (
				'clean' => 'text' 
		) );
...
TEXT;
	...
	public function testTruncate() {
		...
		$this->assertSame ( $this->Text->truncate ( $text2, 10, array (
				'exact' => false 
		) ), '...' );
		...
		$this->assertSame ( $this->Text->truncate ( $text2, 10, array (
				'html' => true 
		) ), 'Heiz&ouml;lr&uuml;c' . $elipsis );
		$this->assertSame ( $this->Text->truncate ( $text2, 10, array (
				'exact' => false,
				'html' => true 
		) ), $elipsis );
		$this->assertSame ( $this->Text->truncate ( $text3, 20, array (
				'html' => true 
		) ), '<b>&copy; 2005-2007, Cake S' . $elipsis . '</b>' );
		$this->assertSame ( $this->Text->truncate ( $text4, 15, array (
				'html' => true 
		) ), '<img src="mypic.jpg"> This image ta' . $elipsis );
		$this->assertSame ( $this->Text->truncate ( $text4, 45, array (
				'html' => true 
		) ), '<img src="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But the' . $elipsis . '</b>' );
		$this->assertSame ( $this->Text->truncate ( $text4, 90, array (
				'html' => true 
		) ), '<img src="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But the following image tag should be conform <img src="mypic.jpg" alt="Me, myself and I" /></b><br />Great,' . $elipsis );
		$this->assertSame ( $this->Text->truncate ( $text5, 6, array (
				'ellipsis' => '',
				'html' => true 
		) ), '0<b>1<i>2<span class="myclass">3</span>4<u>5</u></i></b>' );
		$this->assertSame ( $this->Text->truncate ( $text5, 20, array (
				'ellipsis' => '',
				'html' => true 
		) ), $text5 );
...
<p><span style="font-size: medium;"><a>... </a></span></p>';
	...
	public function testTail() {
		...
		$result = $this->Text->tail ( $text1, 13, array (
				'exact' => false 
		) );
		...
		$result = $this->Text->tail ( $text2, 10, array (
				'exact' => false 
		) );
		$this->assertEquals ( '...', $result );
		...
		$result = $this->Text->tail ( $text5, 6, array (
				'ellipsis' => '' 
		) );
	...
	public function testHighlightHtml() {
		...
		$this->assertEquals ( $this->Text->highlight ( $text3, array (
				'strong',
				'what' 
		), $options ), $text3 );
		
		$expected = '<b>What</b> a <b>strong</b> mouse: <img src="what-a-strong-mouse.png" alt="What a strong mouse!" />';
		$this->assertEquals ( $this->Text->highlight ( $text4, array (
				'strong',
				'what' 
		), $options ), $expected );
	}
	...
	public function testHighlightCaseInsensitivity() {
		...
		$result = $this->Text->highlight ( $text, 'test', array (
				'format' => '<b>\1</b>' 
		) );
		$this->assertEquals ( $expected, $result );
		
		$result = $this->Text->highlight ( $text, array (
				'test' 
		), array (
				'format' => '<b>\1</b>' 
		) );
