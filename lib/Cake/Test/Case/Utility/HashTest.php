...
class HashTest extends CakeTestCase {
	...
	public function testContains() {
		...
		$this->assertTrue ( Hash::contains ( $data, array (
				'apple' 
		) ) );
		$this->assertFalse ( Hash::contains ( $data, array (
				'data' 
		) ) );
	...
	public function testExtractBasic() {
		...
		$result = Hash::extract ( array (
				false 
		), '{n}.Something.another_thing' );
	...
	public function testInsertSimple() {
		...
		$result = Hash::insert ( $a, 'files', array (
				'name' => 'files' 
		) );
		$expected = array (
				'pages' => array (
	...
	public function testInsertOverwriteStringValue() {
		...
		$result = Hash::insert ( $data, 'Some.string.value', array (
				'values' 
		) );
		$expected = array (
				'Some' => array (
	...
	public function testCombineWithFormatting() {
		...
		$result = Hash::combine ( $a, '{n}.User.id', array (
				'%1$s: %2$s',
				'{n}.User.Data.user',
				'{n}.User.Data.name' 
		), '{n}.User.group_id' );
	...
	public function testMap() {
		...
		$result = Hash::map ( $data, '{n}.Article.id', array (
				$this,
				'mapCallback' 
		) );
		$expected = array (
	...
	public function testReduce() {
		...
		$result = Hash::reduce ( $data, '{n}.Article.id', array (
				$this,
				'reduceCallback' 
		) );
		$this->assertEquals ( 15, $result );
	...
	/**
	 * test Hash nest with a normal model result set.
	 * For kicks rely on Hash nest detecting the key names
	 * automatically
	 *
	 * @return void
	 */
