...
class CakeNumberTest extends CakeTestCase {
	...
	public function testFormatDelta() {
		...
		$result = $this->Number->formatDelta ( $value, array (
				'before' => '',
				'after' => '' 
		) );
		$expected = '+100,100,100.00';
	...
	public function testCurrency() {
		...
		$result = $this->Number->currency ( 1000.45, null, array (
				'after' => 'øre',
				'before' => 'Kr. ',
				'decimals' => ',',
				'thousands' => '.' 
		...
		$result = $this->Number->currency ( 0.5, null, array (
				'after' => 'øre' 
		) );
		$expected = '50øre';
		$this->assertEquals ( $expected, $result );
		
		$result = $this->Number->currency ( 1, null, array (
				'wholeSymbol' => '$ ' 
		) );
		$expected = '$ 1.00';
		$this->assertEquals ( $expected, $result );
		...
		$result = $this->Number->currency ( 0.2, null, array (
				'wholeSymbol' => ' $',
				'wholePosition' => 'after',
		...
		$result = $this->Number->currency ( 311, 'USD', array (
				'wholePosition' => 'after' 
		) );
		$expected = '311.00$';
		$this->assertEquals ( $expected, $result );
		...
		$result = $this->Number->currency ( 12, null, array (
				'wholeSymbol' => ' dollars',
				'wholePosition' => 'after',
				'fractionSymbol' => ' cents',
		...
		$result = $this->Number->currency ( 0.12, null, array (
				'wholeSymbol' => ' dollars',
				'wholePosition' => 'after',
				'fractionSymbol' => ' cents',
				'fractionPosition' => 'after' 
	...
	public function testCurrencyAddFormat() {
		$this->Number->addFormat ( 'NOK', array (
				'before' => 'Kr. ' 
		) );
		...
		$this->Number->addFormat ( 'Other', array (
				'before' => '$$ ',
				'after' => 'c!' 
		) );
		...
		$this->Number->addFormat ( 'Other2', array (
				'before' => '$ ',
				'after' => false 
		) );
	...
	public function testCurrencyPositive() {
		...
		$result = $this->Number->currency ( $value, 'USD', array (
				'before' => '#' 
		) );
		$expected = '#100,100,100.00';
		$this->assertEquals ( $expected, $result );
	...
	public function testCurrencyNegative() {
		...
		$result = $this->Number->currency ( $value, 'EUR', array (
				'negative' => '-' 
		) );
		$expected = '-&#8364;100.100.100,00';
		$this->assertEquals ( $expected, $result );
		
		$result = $this->Number->currency ( $value, 'GBP', array (
				'negative' => '-' 
		) );
		$expected = '-&#163;100,100,100.00';
		$this->assertEquals ( $expected, $result );
	...
	public function testCurrencyOptions() {
		...
		$result = $this->Number->currency ( $value, null, array (
				'before' => 'GBP' 
		) );
		...
		$result = $this->Number->currency ( '1234567.8912345', null, array (
				'before' => 'GBP',
				'places' => 3 
		) );
		...
		$result = $this->Number->currency ( '650.120001', null, array (
				'before' => 'GBP',
				'places' => 4 
		) );
		...
		$result = $this->Number->currency ( '0.35', 'USD', array (
				'after' => false 
		) );
		$expected = '$0.35';
		$this->assertEquals ( $expected, $result );
