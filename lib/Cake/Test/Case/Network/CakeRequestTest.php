...
class CakeRequestTest extends CakeTestCase {
	...
	public function testAddParams() {
		...
		$result = $request->addParams ( array (
				'plugin' => null,
				'action' => 'index' 
		) );
	...
	public function testPutParsing() {
		...
		$request = $this->getMock ( 'TestCakeRequest', array (
				'_readInput' 
		) );
		$request->expects ( $this->at ( 0 ) )->method ( '_readInput' )->will ( $this->returnValue ( 'data[Article][]=title' ) );
		$request->reConstruct ();
		...
		$request->expects ( $this->at ( 0 ) )->method ( '_readInput' )->will ( $this->returnValue ( 'one=1&two=three' ) );
		$request->reConstruct ();
		...
		$request->expects ( $this->at ( 0 ) )->method ( '_readInput' )->will ( $this->returnValue ( 'data[Article][title]=Testing&action=update' ) );
		$request->reConstruct ();
		$expected = array (
				'Article' => array (
						'title' => 'Testing' 
		...
		$request->expects ( $this->at ( 0 ) )->method ( '_readInput' )->will ( $this->returnValue ( 'data[Article][]=title&Tag[Tag][]=1&Tag[Tag][]=2' ) );
		$request->reConstruct ();
		$this->assertEquals ( $data ['data'], $request->data );
		...
		$request->expects ( $this->at ( 0 ) )->method ( '_readInput' )->will ( $this->returnValue ( 'data[Article][title]=some%20title&Tag[Tag][]=1&Tag[Tag][]=2' ) );
		$request->reConstruct ();
		$this->assertEquals ( $data ['data'], $request->data );
		...
		$request->expects ( $this->at ( 0 ) )->method ( '_readInput' )->will ( $this->returnValue ( 'a[]=1&a[]=2&b[]=1&b[]=2' ) );
		$request->reConstruct ();
		$this->assertEquals ( $data, $request->data );
	}
	...
	public function testPutParsingJSON() {
		...
		$request->expects ( $this->at ( 0 ) )->method ( '_readInput' )->will ( $this->returnValue ( '{"Article":["title"]}' ) );
		$request->reConstruct ();
		...
		$_FILES = array (
				'data' => array (
						...
						'type' => array (
								...
								'Post' => array (
										'attachment' => 'application/x-javascript' 
								) 
						),
						'tmp_name' => array (
								'File' => array (
								...
								'Post' => array (
										'attachment' => '/private/var/tmp/phpEwlrIo' 
								) 
						),
						'error' => array (
								'File' => array (
	...
	public function testAddDetector() {
		$request = new CakeRequest ( 'some/path' );
		$request->addDetector ( 'compare', array (
				'env' => 'TEST_VAR',
				'value' => 'something' 
		) );
		...
		$request->addDetector ( 'compareCamelCase', array (
				'env' => 'TEST_VAR',
				'value' => 'foo' 
		) );
		...
		$request->addDetector ( 'banana', array (
				'env' => 'TEST_VAR',
				'pattern' => '/^ban.*$/' 
		) );
		...
		$request->addDetector ( 'mobile', array (
				'options' => array (
						'Imagination' 
				) 
		) );
		...
		$request->addDetector ( 'callme', array (
				'env' => 'TEST_VAR',
				'callback' => array (
						$this,
						'detectCallback' 
		...
		$request->addDetector ( 'index', array (
				'param' => 'action',
				'value' => 'index' 
		) );
	...
	/**
	 * Test that a request with a .
	 * in the main GET parameter is filtered out.
	...
	public function testInput() {
		...
		$request->expects ( $this->once () )->method ( '_readInput' )->will ( $this->returnValue ( 'I came from stdin' ) );
	...
	public function testInputDecode() {
		...
		$request->expects ( $this->once () )->method ( '_readInput' )->will ( $this->returnValue ( '{"name":"value"}' ) );
...
XML;
		...
		$request->expects ( $this->once () )->method ( '_readInput' )->will ( $this->returnValue ( $xml ) );
		
		$result = $request->input ( 'Xml::build', array (
				'return' => 'domdocument' 
		) );
		$this->assertInstanceOf ( 'DOMDocument', $result );
		$this->assertEquals ( 'Test', $result->getElementsByTagName ( 'title' )->item ( 0 )->childNodes->item ( 0 )->wholeText );
	}
	...
	public function testOnlyAllow() {
		...
		$this->assertTrue ( $request->onlyAllow ( array (
				'put' 
		) ) );
