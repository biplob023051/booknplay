...
class Controller extends Object implements CakeEventListener {
	 ...
	 * Defaults to Cake's conventional ".ctp".
	 *
	 * @var string
	 */
	public $ext = '.ctp';
	 ...
	 * This is a list of all the methods reachable
	 ...
	 *        	Request object for this controller. Can be null for testing,
	 ...
	 * The properties that get set are
	 *
	...
	protected function _mergeUses($merge) {
		...
		$this->uses = array_merge ( $this->uses, array_diff ( $merge ['uses'], $this->uses ) );
	}
	...
	public function implementedEvents() {
		return array (
				...
				'Controller.beforeRedirect' => array (
	...
	public function redirect($url, $status = null, $exit = true) {
		...
		$event = new CakeEvent ( 'Controller.beforeRedirect', $this, array (
				$url,
				$status,
				$exit 
		) );
		
		list ( $event->break, $event->breakOn, $event->collectReturn ) = array (
				true,
				false,
				true 
		);
	...
	/**
	 * Validates models passed by parameters.
	 * Example:
	 *
	 ...
	 *        	A string containing an SQL comparison operator, or an array matching operators
	 *        	to fields
	 * @param string $bool
	 *        	SQL boolean operator: AND, OR, XOR, etc.
	 * @param boolean $exclusive
	 *        	If true, and $op is an array, fields not included in $op will not be
	 ...
	 * @param boolean $exit
	 *        	If true, exit() will be called after the redirect
	 * @return mixed false to stop redirection event,
