...
class RequestHandlerComponent extends Component {
	...
	/**
	 * Constructor.
	 * Parses the accepted content types accepted by the client using HTTP_ACCEPT
	 *
	...
	public function startup(Controller $controller) {
		...
		$isRecognized = (! in_array ( $this->ext, array (
				'html',
				'htm' 
		...
		} elseif (empty ( $this->ext ) || in_array ( $this->ext, array (
				'html',
				'htm' 
		) )) {
			$this->respondAs ( 'html', array (
		...
		foreach ( $this->_inputTypeMap as $type => $handler ) {
			if ($this->requestedWith ( $type )) {
				$input = call_user_func_array ( array (
						$controller->request,
						'input' 
				), $handler );
	...
	public function beforeRedirect(Controller $controller, $url, $status = null, $exit = true) {
		...
		$this->response->body ( $this->requestAction ( $url, array (
				'return',
				'bare' => false 
		) ) );
		$this->response->send ();
	 ...
	 * headers, and the caching response headers.
	 * If it was not modified, then the
	 ...
	 *        	The Content-type or array of Content-types assigned to the name,
	 ...
	 *
	 * @param string|array $type
	 *        	Can be null (or no parameter), a string type name, or an
	 *        	array of types
	...
	/**
	 * Determines the content type of the data the client has sent (i.e.
	 * in a POST request)
	 *
	 * @param string|array $type
	 ...
	 *
	 * @param string|array $type
	 *        	An optional array of 'friendly' content-type names, i.e.
	 *        	'html', 'xml', 'js', etc.
	 ...
	 * This wraps several methods
	 * available on CakeResponse. It also allows you to use Content-Type aliases.
	 *
	 * @param string|array $type
	 *        	Friendly type name, i.e. 'html' or 'xml', or a full content-type,
	 *        	like 'application/x-shockwave'.
	 * @param array $options
	 *        	If $type is a friendly type name that is associated with
	...
	/**
	 * Add a new mapped input type.
	 * Mapped input types are automatically
	 * converted by RequestHandlerComponent during the startup() callback.
	 *
	 * @param string $type
	 *        	The type alias being converted, ie. json
	 * @param array $handler
	 *        	The handler array for the type. The first index should
