...
class PhpAcl extends Object implements AclInterface {
	...
	/**
	 * Options:
	 * - policy: determines behavior of the check method.
	 * Deny policy needs explicit allow rules, allow policy needs explicit deny rules
	 ...
	 * Checks to see if the ARO (access request object) has access to the
	...
	public function access($aro, $aco, $action, $type = 'deny') {
		...
		foreach ( $aco as $i => $node ) {
			...
			if ($i < $depth - 1) {
				...
				$tree [$node] [$type] = array_merge ( is_array ( $aro ) ? $aro : array (
						$aro 
				), $tree [$node] [$type] );
			}
		}
...
class PhpAro {
	 ...
	 * index.
	 * The resulting array will contain a prioritized list of (list of) roles ordered from
	 * the most distant AROs to the requested one itself.
	 *
	 * @param string|array $aro
	 *        	An ARO identifier
	...
	/**
	 * adds one or more aliases to the internal map.
