...
class Scaffold {
	...
	protected function _scaffoldView(CakeRequest $request) {
		if ($this->controller->beforeScaffold ( 'view' )) {
			...
			$this->controller->set ( Inflector::variable ( $this->controller->modelClass ), $this->request->data );
	...
	protected function _scaffoldForm($action = 'edit') {
		$this->controller->viewVars ['scaffoldFields'] = array_merge ( $this->controller->viewVars ['scaffoldFields'], array_keys ( $this->ScaffoldModel->hasAndBelongsToMany ) );
	 ...
	 * Either uses Sessions or flash messages depending
	...
	protected function _scaffold(CakeRequest $request) {
		...
		if (isset ( $db )) {
			...
			throw new MissingDatabaseException ( array (
					'connection' => $this->ScaffoldModel->useDbConfig 
			) );
		}
	}
