...
class ModelValidator implements ArrayAccess, IteratorAggregate, Countable {
	...
	public function validateAssociated(&$data, $options = array()) {
		...
		foreach ( $data as $association => &$values ) {
			...
			if (isset ( $associations [$association] )) {
				if (in_array ( $associations [$association], array (
						'belongsTo',
						'hasOne' 
				) )) {
					if ($options ['deep']) {
	 ...
	 * On the current model. This method will
	...
	public function getMethods() {
		...
		if (empty ( $this->_modelMethods )) {
			foreach ( get_class_methods ( $this->_model ) as $method ) {
				$this->_modelMethods [strtolower ( $method )] = array (
						$this->_model,
						$method 
				);
			}
	 ...
	 * If `Model::$validate` is not set or empty, this method returns false.
	 * True otherwise.
	 *
	...
	protected function _triggerBeforeValidate($options = array()) {
		$model = $this->getModel ();
		$event = new CakeEvent ( 'Model.beforeValidate', $model, array (
				$options 
		) );
	 ...
	 * @param array|CakeValidationSet $rules
	 ...
	 * If second argument is an array or instance of
	 ...
	 * @param string|array|CakeValidationSet $name
	 *        	name of the rule to be added or list of rules for the field
	 * @param array|CakeValidationRule $rule
