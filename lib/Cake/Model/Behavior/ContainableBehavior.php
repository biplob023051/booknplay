 ...
 * used for a find call.
 * Most useful for limiting the amount of associations and
 * data returned.
 *
 * @package Cake.Model.Behavior
...
class ContainableBehavior extends ModelBehavior {
	...
	public function beforeFind(Model $Model, $query) {
		...
		$autoFields = ($this->settings [$Model->alias] ['autoFields'] && ! in_array ( $Model->findQueryType, array (
				'list',
				'count' 
		) ) && ! empty ( $query ['fields'] ));
	...
	public function containments(Model $Model, $contain, $containments = array(), $throwErrors = null) {
		...
		foreach ( ( array ) $contain as $name => $children ) {
			...
			foreach ( $keys as $i => $key ) {
				...
				if (! $optionKey && is_string ( $key ) && preg_match ( '/^[a-z(]/', $key ) && (! isset ( $Model->{$key} ) || ! is_object ( $Model->{$key} ))) {
					...
					$children [$option] = is_array ( $val ) ? $val : array (
							$val 
					);
					$newChildren = null;
		...
		if (! isset ( $containments ['models'] [$Model->alias] )) {
			$containments ['models'] [$Model->alias] = array (
					'keep' => array (),
					'instance' => &$Model 
			);
		}
