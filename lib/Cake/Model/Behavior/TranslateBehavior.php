...
class TranslateBehavior extends ModelBehavior {
	...
	public function setup(Model $Model, $config = array()) {
		...
		if (! $db->connected) {
			trigger_error ( __d ( 'cake_dev', 'Datasource %s for TranslateBehavior of model %s is not connected', $Model->useDbConfig, $Model->alias ), E_USER_ERROR );
			return false;
		}
	...
	public function afterValidate(Model $Model) {
		$Model->data [$Model->alias] = array_merge ( $Model->data [$Model->alias], $this->runtime [$Model->alias] ['beforeSave'] );
		return true;
	}
			...
			foreach ( $value as $_locale => $_value ) {
				...
				if (array_key_exists ( $_locale, $translations )) {
					$RuntimeModel->save ( array (
							$RuntimeModel->alias => array_merge ( $conditions, array (
	 ...
	 * @param boolean $reset
	 *        	Leave true to have the fields only modified for the next operation.
	...
	public function bindTranslation(Model $Model, $fields, $reset = true) {
		...
		foreach ( $fields as $key => $value ) {
			...
			if ($association === 'name') {
				throw new CakeException ( __d ( 'cake_dev', 'You cannot bind a translation named "name".' ) );
			}
			...
			if (is_null ( $association )) {
				...
				foreach ( array (
					...
					if (isset ( $Model->{$type} [$association] ) || isset ( $Model->__backAssociation [$type] [$association] )) {
						trigger_error ( __d ( 'cake_dev', 'Association %s is already bound to model %s', $association, $Model->alias ), E_USER_ERROR );
						return false;
					}
				}
	...
	protected function _removeField(Model $Model, $field) {
		if (array_key_exists ( $field, $this->settings [$Model->alias] )) {
			...
			$this->settings [$Model->alias] = array_merge ( array_diff ( $this->settings [$Model->alias], array (
					$field 
			) ) );
		}
		...
		if (array_key_exists ( $field, $this->runtime [$Model->alias] ['fields'] )) {
			...
			$this->runtime [$Model->alias] ['fields'] = array_merge ( array_diff ( $this->runtime [$Model->alias] ['fields'], array (
					$field 
			) ) );
		}
	}
	 ...
	 *        	instance of model
	 * @param string|array $fields
	 *        	string with field, or array(field1, field2=>AssocName, field3), or null for
