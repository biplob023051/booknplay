...
class TreeBehavior extends ModelBehavior {
	...
	public function beforeDelete(Model $Model, $cascade = true) {
		...
		$data = $Model->find ( 'first', array (
				'conditions' => array (
						$Model->escapeField ( $Model->primaryKey ) => $Model->id 
				),
				'fields' => array (
						$Model->escapeField ( $left ),
						$Model->escapeField ( $right ) 
				),
				'recursive' => - 1 
		) );
		if ($data) {
		...
		if (! $Model->id || ! $Model->exists ()) {
			if (array_key_exists ( $parent, $Model->data [$Model->alias] ) && $Model->data [$Model->alias] [$parent]) {
				$parentNode = $Model->find ( 'first', array (
						'conditions' => array (
								$scope,
								$Model->escapeField () => $Model->data [$Model->alias] [$parent] 
						),
						'fields' => array (
								$Model->primaryKey,
								$right 
	 ...
	 * @param Model $Model
	 *        	Model instance
	 * @param integer|string|boolean $id
	 ...
	 * @param string $mode
	 *        	parent or tree
	 * @param string|integer $missingParentAction
	...
	public function recover(Model $Model, $mode = 'parent', $missingParentAction = null) {
		...
		if ($mode === 'parent') {
			...
			if ($missingParents) {
				if ($missingParentAction === 'return') {
					...
					$Model->deleteAll ( array (
							$Model->escapeField ( $Model->primaryKey ) => array_flip ( $missingParents ) 
					), false );
				} else {
					$Model->updateAll ( array (
							$Model->escapeField ( $parent ) => $missingParentAction 
					), array (
							$Model->escapeField ( $Model->primaryKey ) => array_flip ( $missingParents ) 
					) );
				}
			...
			foreach ( $Model->find ( 'all', array (
				...
				$Model->updateAll ( array (
						$parent => $db->value ( $parentId, $parent ) 
				), array (
						$Model->escapeField () => $array [$Model->alias] [$Model->primaryKey] 
				) );
			}
		}
		return true;
	...
	protected function _getMax(Model $Model, $scope, $right, $recursive = -1, $created = false) {
		...
		list ( $edge ) = array_values ( $Model->find ( 'first', array (
				'conditions' => $scope,
				'fields' => $db->calculate ( $Model, 'max', array (
						$name,
						$right 
				) ),
				'recursive' => $recursive,
	...
	protected function _getMin(Model $Model, $scope, $left, $recursive = -1) {
		...
		list ( $edge ) = array_values ( $Model->find ( 'first', array (
				'conditions' => $scope,
				'fields' => $db->calculate ( $Model, 'min', array (
						$name,
						$left 
				) ),
				'recursive' => $recursive,
	...
	protected function _sync(Model $Model, $shift, $dir = '+', $conditions = array(), $created = false, $field = 'both') {
		...
		if (is_string ( $conditions )) {
			$conditions = array (
					$Model->escapeField ( $field ) . " {$conditions}" 
			);
		}
		...
		$Model->updateAll ( array (
				$Model->escapeField ( $field ) => $Model->escapeField ( $field ) . ' ' . $dir . ' ' . $shift 
		), $conditions );
