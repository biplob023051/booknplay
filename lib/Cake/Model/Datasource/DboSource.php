...
class DboSource extends DataSource {
	 ...
	 * Cached results for both DboSource::name() and
	...
	/**
	 * Flag to support nested transactions.
	 * If it is set to false, you will be able to use
	 ...
	 * (rows affected, timing, any errors, number of rows in resultset).
	 * The query is also logged.
	 ...
	 * If no previous operation exists,
	 * this returns false.
	 *
	 * @param mixed $source        	
	...
	public function query() {
		...
		if (isset ( $args [1] ) && $args [1] === true) {
			...
			return $this->fetchAll ( $args [0], $args [1], array (
					'cache' => $cache 
			) );
		}
	}
	 ...
	 *        	SQL statement
	 * @param array $params
	 *        	parameters to be bound as values for the SQL statement
	 * @param array $options
	 *        	additional options for the query.
	...
	/**
	 * Cache a value into the methodCaches.
	 * Will respect the value of DboSource::$cacheMethods.
	 ...
	 * If in a non-CLI environment the sql_log element
	...
	public function showLog($sorted = false) {
		...
		if (PHP_SAPI !== 'cli') {
			...
			$View->set ( 'logs', array (
					$this->configKeyName => $log 
			) );
			echo $View->element ( 'sql_dump', array (
					'_forced_from_dbo_' => true 
			) );
		} else {
	 ...
	 * @param array $fields
	 *        	An array of field names to insert. If null, $model->data will be
	 *        	used to generate field names.
	 * @param array $values
	 *        	An array of values with keys matching the fields. If null, $model->data will
	...
	protected function _filterResults(&$results, Model $model, $filtered = array()) {
		...
		$keys = array_diff ( array_keys ( $current ), $filtered, array (
				$model->alias 
		) );
		$filtering = array ();
		foreach ( $keys as $className ) {
			...
			foreach ( $results as $key => &$result ) {
				$data = $linkedModel->afterFind ( array (
						array (
				...
				if ($type !== 'hasAndBelongsToMany') {
					...
					if ($q !== false) {
						...
						if (($type === 'belongsTo' || $type === 'hasOne') && isset ( $row [$linkModel->alias], $joined [$model->alias] ) && in_array ( $linkModel->alias, $joined [$model->alias] )) {
							...
							if (! empty ( $joinedData )) {
								$fetch [0] = array (
										$linkModel->alias => $row [$linkModel->alias] 
								);
							}
						} else {
							$fetch = $this->fetchAll ( $q, $model->cacheQueries );
	...
	protected function _mergeHasMany(&$resultSet, $merge, $association, $model, $linkModel) {
		...
		foreach ( $resultSet as &$result ) {
			...
			$result = Hash::mergeDiff ( $result, array (
					$association => $merged 
			) );
		}
	}
			...
			if (isset ( $merge [$association] )) {
				...
				if (! isset ( $data [$association] )) {
					...
					if (is_array ( $merge [0] [$association] )) {
						...
						$data [$association] = array_merge ( $data [$association], array (
								$association => array () 
						) );
					}
				}
	...
	public function generateAssociationQuery(Model $model, $linkModel, $type, $association, $assocData, &$queryData, $external, &$resultSet) {
		...
		if (empty ( $queryData ['fields'] )) {
			...
			$assocFields = $this->fields ( $model, $modelAlias, array (
					"{$modelAlias}.{$model->primaryKey}" 
			) );
		...
		if ($external || (in_array ( $type, array (
				'hasOne',
				'belongsTo' 
		) ) && $assocData ['fields'] !== false)) {
		...
		switch ($type) {
			case 'hasOne' :
			case 'belongsTo' :
				$conditions = $this->_mergeConditions ( $assocData ['conditions'], $this->getConstraint ( $type, $model, $linkModel, $association, array_merge ( $assocData, compact ( 'external', 'self' ) ) ) );
			...
			case 'hasMany' :
				...
				if (! empty ( $assocData ['foreignKey'] )) {
					$assocData ['fields'] = array_merge ( $assocData ['fields'], $this->fields ( $linkModel, $association, array (
							"{$association}.{$assocData['foreignKey']}" 
					) ) );
				}
				$query = array (
	...
	/**
	 * Builds and generates a JOIN statement from an array.
	 * Handles final clean-up before conversion.
	 *
	 * @param array $join
	...
	/**
	 * Builds and generates an SQL statement from an array.
	 * Handles final clean-up before conversion.
	 *
	 * @param array $query
	...
	/**
	 * Gets a list of record IDs for the given conditions.
	 * Used for multi-record updates and deletes
		...
		foreach ( $joins as $assoc ) {
			if (isset ( $model->{$assoc} ) && $model->useDbConfig == $model->{$assoc}->useDbConfig && $model->{$assoc}->getDataSource ()) {
				...
				$join [] = $this->buildJoinStatement ( array (
						...
						'alias' => $assoc,
						'type' => isset ( $assocData ['type'] ) ? $assocData ['type'] : 'LEFT',
						'conditions' => trim ( $this->conditions ( $this->_mergeConditions ( $assocData ['conditions'], $this->getConstraint ( $assocData ['association'], $model, $model->{$assoc}, $assoc, $assocData ) ), true, false, $model ) ) 
				) );
			}
		}
		return $join;
	...
	/**
	 * Returns an SQL calculation, i.e.
	 * COUNT() or MAX()
	 *
	 * @param Model $model        	
	 * @param string $func
	...
	public function defaultConditions(Model $model, $conditions, $useAlias = true) {
		...
		return array (
				"{$alias}.{$model->primaryKey}" => $model->getID () 
		);
	}
	
	/**
	 * Returns a key formatted like a string Model.fieldname(i.e.
	 * Post.title, or Country.name)
	 *
	 * @param Model $model        	
	 * @param string $key        	
	 * @param string $assoc        	
					...
					if (isset ( $field [1] )) {
						if (strpos ( $field [1], '.' ) === false) {
							...
							if (! Hash::numeric ( $field [0] )) {
								$field [0] = implode ( '.', array_map ( array (
										&$this,
										'name' 
								), $field [0] ) );
	...
	protected function _parseKey($model, $key, $value) {
		...
		if (strtolower ( $operator ) === 'not') {
			$data = $this->conditionKeysToString ( array (
					$operator => array (
							$key => $value 
					) 
			), true, $model );
	...
	protected function _quoteFields($conditions) {
		...
		$conditions = preg_replace_callback ( '/(?:[\'\"][^\'\"\\\]*(?:\\\.[^\'\"\\\]*)*[\'\"])|([a-z0-9_][a-z0-9\\-_]*\\.[a-z0-9_][a-z0-9_\\-]*)/i', array (
				&$this,
				'_quoteMatchedField' 
		), $conditions );
		if ($conditions !== null) {
	 ...
	 * @param array $values
	 *        	The array of values to insert. The values should
	...
	/**
	 * Reset a sequence based on the MAX() value of $column.
	 * Useful
	 ...
	 *
	 * @param array $column
	 *        	An array structured like the following: array('name' => 'value', 'type' => 'value'[, options]),
