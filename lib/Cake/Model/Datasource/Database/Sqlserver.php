...
class Sqlserver extends DboSource {
	...
	public function connect() {
		...
		try {
			$this->_connection = new PDO ( "sqlsrv:server={$config['host']};Database={$config['database']}", $config ['login'], $config ['password'], $flags );
			$this->connected = true;
	...
	public function describe($model) {
		...
		while ( $column = $cols->fetch ( PDO::FETCH_OBJ ) ) {
			...
			if ($fields [$field] ['default'] !== null) {
				$fields [$field] ['default'] = preg_replace ( "/^[(]{1,2}'?([^')]*)?'?[)]{1,2}$/", "$1", $fields [$field] ['default'] );
			...
			if (in_array ( $fields [$field] ['type'], array (
					'date',
					'time',
					'datetime',
					'timestamp' 
	 ...
	 *
	 * @param mixed $real
	 *        	Either the string value of the fields type.
	...
	public function insertMulti($table, $fields, $values) {
		...
		$hasPrimaryKey = $primaryKey && ((is_array ( $fields ) && in_array ( $primaryKey, $fields ) || (is_string ( $fields ) && strpos ( $fields, $this->startQuote . $primaryKey . $this->endQuote ) !== false)));
	...
	public function buildColumn($column) {
		...
		} elseif (array_keys ( $column ) == array (
				'type',
				'name' 
		)) {
			$result .= ' NULL';
