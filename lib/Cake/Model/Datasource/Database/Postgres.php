...
class Postgres extends DboSource {
	...
	/**
	 * Base driver configuration settings.
	...
	public function connect() {
		...
		try {
			$this->_connection = new PDO ( "pgsql:host={$config['host']};port={$config['port']};dbname={$config['database']}", $config ['login'], $config ['password'], $flags );
	...
	public function describe($model) {
		...
		if ($fields === null) {
			$cols = $this->_execute ( "SELECT DISTINCT table_schema AS schema, column_name AS name, data_type AS type, is_nullable AS null,
					column_default AS default, ordinal_position AS position, character_maximum_length AS char_length,
					character_octet_length AS oct_length FROM information_schema.columns
				WHERE table_name = ? AND table_schema = ?  ORDER BY position", array (
					$table,
					$this->config ['schema'] 
			) );
	 ...
	 * @param boolean $reset
	 *        	true for resetting the sequence, false to leave it as is.
	 ...
	 *
	 * @param array $column
	 *        	An array structured like the following:
	...
	public function buildColumn($column) {
		...
		$out = preg_replace ( '/integer\([0-9]+\)/', 'integer', $out );
		$out = preg_replace ( '/bigint\([0-9]+\)/', 'bigint', $out );
		...
		if (strpos ( $out, 'DEFAULT DEFAULT' )) {
			...
			} elseif (in_array ( $column ['type'], array (
					'integer',
					'float' 
			) )) {
	...
	public function buildIndex($indexes, $table = null) {
		...
		foreach ( $indexes as $name => $value ) {
			if ($name === 'PRIMARY') {
				...
				if (is_array ( $value ['column'] )) {
					$value ['column'] = implode ( ', ', array_map ( array (
							&$this,
							'name' 
					), $value ['column'] ) );
				} else {
