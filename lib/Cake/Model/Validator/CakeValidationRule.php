 ...
 * Represents a validation method, error message and
...
class CakeValidationRule {
	...
	public function isRequired() {
		if (in_array ( $this->required, array (
				'create',
				'update' 
		), true )) {
	 ...
	 *        	Field name
	 * @param array $data
	 *        	Data to check rule against
	 * @return boolean
	 */
	public function checkRequired($field, &$data) {
		return ((! array_key_exists ( $field, $data ) && $this->isRequired () === true) || (array_key_exists ( $field, $data ) && (empty ( $data [$field] ) && ! is_numeric ( $data [$field] )) && $this->allowEmpty === false));
	}
	...
	public function process($field, &$data, &$methods) {
		...
		if (isset ( $methods [$rule] )) {
			...
			$this->_ruleParams [0] = array (
					$field => $this->_ruleParams [0] 
			);
			...
			$this->_valid = call_user_func_array ( array (
					'Validation',
					$this->_rule 
			), $this->_ruleParams );
	...
	protected function _parseRule($field, &$data) {
		if (is_array ( $this->rule )) {
			...
			$this->_ruleParams = array_merge ( array (
					$data [$field] 
			), array_values ( array_slice ( $this->rule, 1 ) ) );
		} else {
			$this->_rule = $this->rule;
			$this->_ruleParams = array (
					$data [$field] 
			);
		}
	}
