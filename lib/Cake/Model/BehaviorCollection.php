...
class BehaviorCollection extends ObjectCollection implements CakeEventListener {
	 ...
	 * You can use use `$config['enabled'] = false`
	...
	public function load($behavior, $config = array()) {
		...
		foreach ( $this->_loaded [$alias]->mapMethods as $method => $methodAlias ) {
			$this->_mappedMethods [$method] = array (
					$alias,
					$methodAlias 
			);
		}
	 ...
	 * Will call either normal methods or mapped methods.
	 *
	...
	/**
	 * Gets the method list for attached behaviors, i.e.
	 * all public, non-callback methods.
	...
	/**
	 * Check to see if a behavior in this collection implements the provided method.
	 * Will
