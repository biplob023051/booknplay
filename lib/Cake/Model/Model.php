...
class Model extends Object implements CakeEventListener {
	...
	/**
	 * Holds physical schema/database name for this model.
	 * Automatically set during Model creation.
	 *
	 * @var string
	 */
	 ...
	 * It must be an array with the field name as key and using
	...
	/**
	 * List of table names included in the model description.
	...
	/**
	 * Whether or not to cache queries for this model.
	 * This enables in-memory
	...
	public function __get($name) {
		if ($name === 'displayField') {
			return $this->displayField = $this->hasField ( array (
					'title',
					'name',
					$this->primaryKey 
			) );
	 ...
	 *        	Class name
	 * @param string $plugin
	 *        	name of the plugin where $className is located
	...
	/**
	 * Sets a custom table for your model class.
	 * Used by your controller to select a database table.
	 *
	 ...
	 *
	 * @param string|array|SimpleXmlElement|DomNode $one
	...
	public function deconstruct($field, $data) {
		...
		if (isset ( $data ['hour'] ) && isset ( $data ['meridian'] ) && ! empty ( $data ['hour'] ) && $data ['hour'] != 12 && $data ['meridian'] === 'pm') {
	...
	/**
	 * Check that a method is callable on a model.
	 * This will check both the model's own methods, its
	 ...
	 *        	Optional data array to assign to the model after it is created. If null or false,
	...
	public function save($data = null, $validate = true, $fieldList = array()) {
		...
		if (empty ( $this->data ) && ! $this->hasField ( array (
				'created',
				'updated',
				'modified' 
		) )) {
		...
		if ($options ['callbacks'] === true || $options ['callbacks'] === 'before') {
			...
			list ( $event->break, $event->breakOn ) = array (
					true,
					array (
							false,
							null 
	...
	protected function _isUUIDField($field) {
		...
		return $field ['length'] == 36 && in_array ( $field ['type'], array (
				'string',
				'binary' 
		) );
	}
	 ...
	 *        	True if a new record was created, otherwise only associations with
			...
			if (! empty ( $assoc ['counterCache'] )) {
				if (! is_array ( $assoc ['counterCache'] )) {
					if (isset ( $assoc ['counterScope'] )) {
						$assoc ['counterCache'] = array (
								$assoc ['counterCache'] => $assoc ['counterScope'] 
						);
					} else {
						$assoc ['counterCache'] = array (
								$assoc ['counterCache'] => array () 
	 ...
	 *
	 * @param array $data
	 *        	Record data to save. This can be either a numerically-indexed array (for saving multiple
	...
	public function saveMany($data = null, $options = array()) {
		...
		foreach ( $data as $key => $record ) {
			...
			if ($validates) {
				if ($options ['deep']) {
					$saved = $this->saveAssociated ( $record, array_merge ( $options, array (
							'atomic' => false 
					) ) );
				} else {
	...
	public function saveAssociated($data = null, $options = array()) {
		...
		foreach ( $data as $association => $values ) {
			...
			if (isset ( $associations [$association] ) && $associations [$association] === 'belongsTo' && $notEmpty) {
				...
				if ($validates) {
					if ($options ['deep']) {
						$saved = $this->{$association}->saveAssociated ( $values, array_merge ( $options, array (
								'atomic' => false 
						) ) );
					} else {
						$saved = $this->{$association}->save ( $values, array_merge ( $options, array (
								'atomic' => false 
						) ) );
					}
			...
			if (isset ( $associations [$association] ) && $notEmpty) {
				...
				switch ($type) {
					...
					case 'hasMany' :
						...
						$options = $this->{$association}->_addToWhiteList ( $key, $options );
						$_return = $this->{$association}->saveMany ( $values, array_merge ( $options, array (
								'atomic' => false 
						) ) );
	 ...
	 *
	 * @param array $fields
	 *        	Set of fields and values, indexed by fields.
	...
	public function delete($id = null, $cascade = true) {
		...
		if (! $this->getDataSource ()->delete ( $this, array (
				$this->alias . '.' . $this->primaryKey => $id 
		) )) {
			return false;
		}
	...
	protected function _deleteDependent($id, $cascade) {
		...
		foreach ( array_merge ( $this->hasMany, $this->hasOne ) as $assoc => $data ) {
			...
			if ($data ['foreignKey'] === false && $data ['conditions'] && in_array ( $this->name, $model->getAssociated ( 'belongsTo' ) )) {
				$model->recursive = 0;
				$conditions = array (
						$this->escapeField ( null, $this->name ) => $id 
				);
			} else {
				$model->recursive = - 1;
				$conditions = array (
						$model->escapeField ( $data ['foreignKey'] ) => $id 
				);
				if ($data ['conditions']) {
	...
	protected function _deleteLinks($id) {
		foreach ( $this->hasAndBelongsToMany as $data ) {
			...
			$records = $this->{$joinModel}->find ( 'all', array (
					'conditions' => array (
							$this->{$joinModel}->escapeField ( $data ['foreignKey'] ) => $id 
					),
	...
	public function buildQuery($type = 'first', $query = array()) {
		...
		if ($query ['callbacks'] === true || $query ['callbacks'] === 'before') {
			$event = new CakeEvent ( 'Model.beforeFind', $this, array (
					$query 
			) );
			list ( $event->break, $event->breakOn, $event->modParams ) = array (
					true,
					array (
							false,
							null 
	...
	/**
	 * Handles the before/after filter logic for find('count') operations.
	 * Only called by Model::find().
	 *
	 * @param string $state
	 *        	Either "before" or "after"
	...
	/**
	 * Handles the before/after filter logic for find('list') operations.
	 * Only called by Model::find().
	 *
	 * @param string $state
	 *        	Either "before" or "after"
	...
	protected function _filterResults($results, $primary = true) {
		$event = new CakeEvent ( 'Model.afterFind', $this, array (
				$results,
				$primary 
		) );
		$event->modParams = 0;
	 ...
	 * Custom SQL queries should be performed with this method.
	 *
	 * @param string $sql
	 *        	SQL statement
	 * @param boolean|array $params
	 *        	Either a boolean to control query caching or an array of parameters
	 *        	for use with prepared statement placeholders.
	 * @param boolean $cache
	 *        	If $params is provided, a boolean flag for enabling/disabled
	 *        	query caching.
	 ...
	 * Will validate hasAndBelongsToMany associations
	 ...
	 * @param mixed $value
	 *        	Name of validation rule that was not failed, or validation message to
	 ...
	 * Escaping is done according to the
	...
	/**
	 * Gets the name and fields to be used by a join model.
	 * This allows specifying join fields
	 * in the association definition.
	 *
	 * @param string|array $assoc
	...
	public function joinModel($assoc, $keys = array()) {
		...
		if (is_array ( $assoc )) {
			...
			return array (
					$with,
					array_unique ( array_merge ( $assoc [$with], $keys ) ) 
			);
		}
		trigger_error ( __d ( 'cake_dev', 'Invalid join model settings in %s. The association parameter has the wrong type, expecting a string or array, but was passed type: %s', $this->alias, gettype ( $assoc ) ), E_USER_WARNING );
	}
	
	/**
	 * Called before each find operation.
	 * Return false if you want to halt the find
	 ...
	 * Can be used to modify any results returned by find().
	 ...
	 *
	 * @param string $type
	 *        	If null this deletes cached views if Cache.check is true
	 ...
	 *
	 * @param
	 *        	ModelValidator Model validator instance.
