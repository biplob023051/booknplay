...
class ConsoleShell extends AppShell {
	...
	public function startup() {
		...
		if (! $this->_loadRoutes ()) {
			$message = __d ( 'cake_console', 'There was an error loading the routes config. Please check that the file exists and contains no errors.' );
			$this->err ( $message );
		}
	}
	public function getOptionParser() {
		...
		return parent::getOptionParser ()->description ( $description )->epilog ( $epilog );
	}
	/**
			...
			switch ($command) {
				...
				case preg_match ( "/^(\w+) bind (\w+) (\w+)/", $command, $tmp ) :
					...
					if ($this->_isValidModel ( $modelA ) && $this->_isValidModel ( $modelB ) && in_array ( $association, $this->associations )) {
						...
						$this->out ( __d ( 'cake_console', "Created %s association between %s and %s", $association, $modelA, $modelB ) );
					} else {
				...
				case preg_match ( "/^(\w+) unbind (\w+) (\w+)/", $command, $tmp ) :
					...
					if ($this->_isValidModel ( $modelA ) && $this->_isValidModel ( $modelB ) && in_array ( $association, $this->associations ) && $validCurrentAssociation) {
						$this->{$modelA}->unbindModel ( array (
								$association => array (
										$modelB 
								) 
						) );
						$this->out ( __d ( 'cake_console', "Removed %s association between %s and %s", $association, $modelA, $modelB ) );
					} else {
