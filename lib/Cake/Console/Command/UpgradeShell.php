...
class UpgradeShell extends AppShell {
	...
	public function tests() {
		...
		if (! empty ( $this->params ['plugin'] )) {
			$this->_paths = array (
					App::pluginPath ( $this->params ['plugin'] ) . 'tests' . DS 
			);
		}
		$patterns = array (
				array (
	...
	public function locations() {
		...
		$sourceDirs = array (
				...
				'tests',
				'Test' => array (
						'regex' => '@class (\S*Test) extends CakeTestCase@' 
				),
				'views',
				'View',
				'vendors/shells' 
	...
	public function helpers() {
		...
		if (! empty ( $this->params ['plugin'] )) {
			$this->_paths = array (
					App::pluginPath ( $this->params ['plugin'] ) . 'views' . DS 
			);
		}
		...
		foreach ( $plugins as $plugin ) {
			...
			$pluginHelpers = array_merge ( $pluginHelpers, App::objects ( 'helper', App::pluginPath ( $plugin ) . DS . 'views' . DS . 'helpers' . DS, false ) );
		}
	...
	public function i18n() {
		...
		if (! empty ( $this->params ['plugin'] )) {
			$this->_paths = array (
					App::pluginPath ( $this->params ['plugin'] ) 
			);
		}
	...
	public function request() {
		...
		$controllers = array_diff ( App::path ( 'controllers' ), App::core ( 'controllers' ), array (
				APP 
		) );
	...
	protected function _findFiles($extensions = '') {
		...
		foreach ( $this->_paths as $path ) {
			...
			$Iterator = new RegexIterator ( new RecursiveIteratorIterator ( new RecursiveDirectoryIterator ( $path ) ), '/^.+\.(' . $extensions . ')$/i', RegexIterator::MATCH );
