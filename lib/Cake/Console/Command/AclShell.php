 ...
 * This console is known to have issues with zend.ze1_compatibility_mode
...
class AclShell extends AppShell {
	...
	public function startup() {
		...
		if ($this->command) {
			...
			if (! in_array ( $this->command, array (
					'initdb' 
			) )) {
	...
	public function getPath() {
		...
		if (empty ( $nodes )) {
			$this->error ( __d ( 'cake_console', "Supplied Node '%s' not found", $this->args [1] ), __d ( 'cake_console', 'No tree returned.' ) );
		}
	...
	public function view() {
		...
		if (isset ( $this->args [1] )) {
			...
			$topNode = $this->Acl->{$class}->find ( 'first', array (
					'conditions' => array (
							$class . '.id' => $this->_getNodeId ( $class, $identity ) 
					) 
			) );
	...
	public function getOptionParser() {
		...
		$parser->description ( __d ( 'cake_console', 'A console tool for managing the DbAcl' ) )->addSubcommand ( 'create', array (
				...
				'parser' => array (
						...
						'arguments' => array (
								'aro' => array (
										'help' => __d ( 'cake_console', 'ARO to check.' ),
										'required' => true 
								),
								'aco' => array (
										'help' => __d ( 'cake_console', 'ACO to check.' ),
										'required' => true 
								),
								'action' => array (
										'help' => __d ( 'cake_console', 'Action to check' ),
										'default' => 'all' 
								) 
						) 
				) 
				...
				'parser' => array (
						...
						'arguments' => array (
								...
								'aco' => array (
										'help' => __d ( 'cake_console', 'ACO to grant access to.' ),
										'required' => true 
								),
								'action' => array (
										'help' => __d ( 'cake_console', 'Action to grant' ),
										'default' => 'all' 
								) 
						) 
				) 
				...
				'parser' => array (
						...
						'arguments' => array (
								'aro' => array (
										'help' => __d ( 'cake_console', 'ARO to deny.' ),
										'required' => true 
								),
								'aco' => array (
										'help' => __d ( 'cake_console', 'ACO to deny.' ),
										'required' => true 
								),
								'action' => array (
										'help' => __d ( 'cake_console', 'Action to deny' ),
										'default' => 'all' 
								) 
						) 
				) 
				...
				'parser' => array (
						...
						'arguments' => array (
								...
								'action' => array (
										'help' => __d ( 'cake_console', 'Action to inherit' ),
										'default' => 'all' 
								) 
						) 
				) 
				...
				'parser' => array (
						...
						'arguments' => array (
								'type' => $type,
								'node' => array (
										'help' => __d ( 'cake_console', 'The optional node to view the subtree of.' ) 
								) 
						) 
				) 
	...
	/**
	 * Get the node for a given identifier.
	 * $identifier can either be a string alias
	...
	protected function _getParams() {
		...
		$action = '*';
		if (isset ( $this->args [2] ) && ! in_array ( $this->args [2], array (
				'',
				'all' 
		) )) {
			$action = $this->args [2];
