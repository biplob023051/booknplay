...
class ModelTask extends BakeTask {
	...
	protected function _interactive() {
		...
		if ($knownToExist) {
			$displayField = $tempModel->hasField ( array (
					'name',
					'title' 
			) );
			if (! $displayField) {
			...
			$wannaDoValidation = $this->in ( $prompt, array (
					'y',
					'n' 
			), 'y' );
			...
			$wannaDoAssoc = $this->in ( $prompt, array (
					'y',
					'n' 
			), 'y' );
	...
	public function doMoreAssociations(Model $model, $associations) {
		...
		$wannaDoMoreAssoc = $this->in ( $prompt, array (
				'y',
				'n' 
		), 'n' );
	...
	protected function _generatePossibleKeys() {
		$possible = array ();
		foreach ( $this->_tables as $otherTable ) {
			$tempOtherModel = new Model ( array (
					'table' => $otherTable,
					'ds' => $this->connection 
			) );
	...
	public function getOptionParser() {
		...
		return $parser->description ( __d ( 'cake_console', 'Bake models.' ) )->addArgument ( 'name', array (
