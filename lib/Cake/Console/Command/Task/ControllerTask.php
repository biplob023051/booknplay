...
class ControllerTask extends BakeTask {
	...
	/**
	 * Bake All the controllers at once.
	 * Will only bake controllers for models that exist.
	 *
	 * @return void
	 */
	public function all() {
		...
		ClassRegistry::config ( 'Model', array (
				'ds' => $this->connection 
		) );
	...
	protected function _interactive() {
		...
		if (strtolower ( $doItInteractive ) === 'y') {
			$this->interactive = true;
			$useDynamicScaffold = $this->in ( __d ( 'cake_console', "Would you like to use dynamic scaffolding?" ), array (
					'y',
					'n' 
			), 'n' );
			...
			if (strtolower ( $useDynamicScaffold ) === 'y') {
				...
				$wannaUseSession = $this->in ( __d ( 'cake_console', "Would you like to use Session flash messages?" ), array (
						'y',
						'n' 
				), 'y' );
			}
		...
		if ($this->interactive === true) {
			...
			$looksGood = $this->in ( __d ( 'cake_console', 'Look okay?' ), array (
					'y',
					'n' 
			), 'y' );
	...
	protected function _askAboutMethods() {
		$wannaBakeCrud = $this->in ( __d ( 'cake_console', "Would you like to create some basic class methods \n(index(), add(), view(), edit())?" ), array (
				'y',
				'n' 
		), 'n' );
		$wannaBakeAdminCrud = $this->in ( __d ( 'cake_console', "Would you like to create the basic class methods for admin routing?" ), array (
				'y',
				'n' 
		), 'n' );
		return array (
	...
	public function bakeActions($controllerName, $admin = null, $wannaUseSession = true) {
		...
		$this->Template->set ( compact ( 'plugin', 'admin', 'controllerPath', 'pluralName', 'singularName', 'singularHumanName', 'pluralHumanName', 'modelObj', 'wannaUseSession', 'currentModelName', 'displayField', 'primaryKey' ) );
	...
	public function doComponents() {
		return $this->_doPropertyChoices ( __d ( 'cake_console', "Would you like this controller to use any components?" ), __d ( 'cake_console', "Please provide a comma separated list of the component names you'd like to use.\nExample: 'Acl, Security, RequestHandler'" ) );
	}
	...
	public function getOptionParser() {
		...
		return $parser->description ( __d ( 'cake_console', 'Bake a controller for a model. Using options you can bake public, admin or both.' ) )->addArgument ( 'name', array (
