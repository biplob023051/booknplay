...
class ViewTask extends BakeTask {
	...
	/**
	 * An array of action names that don't require templates.
	 * These
	 * actions will not emit errors when doing bakeActions()
	 *
	 * @var array
	 */
	public $noTemplateActions = array (
			'delete' 
	);
	...
	protected function _methodsToBake() {
		$methods = array_diff ( array_map ( 'strtolower', get_class_methods ( $this->controllerName . 'Controller' ) ), array_map ( 'strtolower', get_class_methods ( 'AppController' ) ) );
	...
	protected function _interactive() {
		...
		$interactive = $this->in ( $prompt, array (
				'y',
				'n' 
		), 'n' );
		...
		$wannaDoScaffold = $this->in ( $prompt, array (
				'y',
				'n' 
		), 'y' );
	...
	public function getOptionParser() {
		...
		return $parser->description ( __d ( 'cake_console', 'Bake views for a controller, using built-in or custom templates.' ) )->addArgument ( 'controller', array (
