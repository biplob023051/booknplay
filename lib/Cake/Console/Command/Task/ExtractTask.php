...
class ExtractTask extends AppShell {
	...
	protected function _getPaths() {
		$defaultPath = APP;
		while ( true ) {
			$currentPaths = count ( $this->_paths ) > 0 ? $this->_paths : array (
					'None' 
			);
			$message = __d ( 'cake_console', "Current paths: %s\nWhat is the path you would like to extract?\n[Q]uit [D]one", implode ( ', ', $currentPaths ) );
	...
	public function execute() {
		...
		if (isset ( $this->params ['merge'] )) {
			...
			$this->out ();
			$response = $this->in ( __d ( 'cake_console', 'Would you like to merge all domains strings into the default.pot file?' ), array (
					'y',
					'n' 
			), 'n' );
	...
	public function getOptionParser() {
		...
		return $parser->description ( __d ( 'cake_console', 'CakePHP Language String Extraction:' ) )->addOption ( 'app', array (
	...
	protected function _extractTokens() {
		foreach ( $this->_files as $file ) {
			...
			unset ( $allTokens );
			$this->_parse ( '__', array (
					'singular' 
			) );
			$this->_parse ( '__n', array (
					'singular',
					'plural' 
			) );
			$this->_parse ( '__d', array (
					'domain',
					'singular' 
			) );
			$this->_parse ( '__c', array (
					'singular' 
			) );
			$this->_parse ( '__dc', array (
					'domain',
					'singular' 
			) );
			$this->_parse ( '__dn', array (
					'domain',
					'singular',
					'plural' 
			) );
			$this->_parse ( '__dcn', array (
					'domain',
					'singular',
					'plural' 
			) );
	...
	protected function _writeFiles() {
		...
		foreach ( $this->_storage as $domain => $sentences ) {
			...
			while ( $overwriteAll === false && $File->exists () && strtoupper ( $response ) !== 'Y' ) {
				$this->out ();
				$response = $this->in ( __d ( 'cake_console', 'Error: %s already exists in this location. Overwrite? [Y]es, [N]o, [A]ll', $filename ), array (
						'y',
						'n',
						'a' 
				), 'y' );
	 ...
	 * APP constant, i.e.
	 * this task is extracting strings from same application.
	 *
	 * @return boolean
	 */
	protected function _isExtractingApp() {
		return $this->_paths === array (
				APP 
		);
	}
