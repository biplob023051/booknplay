...
class Shell extends Object {
	...
	public function __construct($stdout = null, $stderr = null, $stdin = null) {
		if (! $this->name) {
			$this->name = Inflector::camelize ( str_replace ( array (
					'Shell',
					'Task' 
			), '', get_class ( $this ) ) );
		}
	...
	protected function _loadModels() {
		...
		$uses = is_array ( $this->uses ) ? $this->uses : array (
				$this->uses 
		);
	...
	/**
	 * Dispatch a command to another Shell.
	 * Similar to Object::requestAction()
	 ...
	 *        	The command name to run on this shell. If this argument is empty,
	...
	public function createFile($path, $contents) {
		...
		if (is_file ( $path ) && $this->interactive === true) {
			...
			$key = $this->in ( __d ( 'cake_console', 'Do you want to overwrite?' ), array (
					'y',
					'n',
					'q' 
			), 'n' );
	...
	protected function _checkUnitTest() {
		if (class_exists ( 'PHPUnit_Framework_TestCase' )) {
			...
			return true;
		} elseif (App::import ( 'Vendor', 'phpunit', array (
				'file' => 'PHPUnit' . DS . 'Autoload.php' 
		) )) {
			return true;
		}
		
		$prompt = __d ( 'cake_console', 'PHPUnit is not installed. Do you want to bake unit test files anyway?' );
		$unitTest = $this->in ( $prompt, array (
				'y',
				'n' 
		), 'y' );
	...
	/**
	 * Find the correct path for a plugin.
	 * Scans $pluginPaths for the plugin you want.
	 *
