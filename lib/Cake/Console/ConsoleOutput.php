...
class ConsoleOutput {
	 ...
	 * Manipulated with ConsoleOutput::outputAs();
	 *
	 * @var integer
	 */
	...
	public function styleText($text) {
		...
		return preg_replace_callback ( '/<(?P<tag>[a-z0-9-_]+)>(?P<text>.*?)<\/(\1)>/ims', array (
				$this,
				'_replaceTags' 
		), $text );
	}
	...
	/**
	 * Get/Set the output type to use.
	 * The output type how formatting tags are treated.
	 *
	 * @param integer $type
