 ...
 * for GetOpt compatible option definition.
...
class ConsoleOptionParser {
	 ...
	 *        	Whether you want the verbose and quiet options set. Setting
	 ...
	 *        	The text to set, or null if you want to read. If an array the
	...
	/**
	 * Get or set an epilog to the parser.
	 * The epilog is added to the end of
	 ...
	 * Options allow you to define optional or required
	 ...
	 *        	The long name you want to the value to be parsed out as when options are parsed.
	 ...
	 * An exception will be raised when parse() encounters an invalid value.
	 *
	 * @param ConsoleInputArgument|string $name
	 ...
	 * Take an array of argument definitions.
	 ...
	 * Takes an array of option definitions.
	 ...
	 * it will be used.
	 *
	 * @param ConsoleInputSubcommand|string $name
	 ...
	 * @param string $command
	 *        	The subcommand to use. If this parameter is a subcommand, that has a parser,
	...
	public function parse($argv, $command = null) {
		...
		foreach ( $this->_args as $i => $arg ) {
			if ($arg->isRequired () && ! isset ( $args [$i] ) && empty ( $params ['help'] )) {
				throw new ConsoleException ( __d ( 'cake_console', 'Missing required arguments. %s is required.', $arg->name () ) );
			}
		}
	 ...
	 *        	If present and a valid subcommand that has a linked parser.
