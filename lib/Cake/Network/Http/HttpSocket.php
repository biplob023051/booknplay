...
class HttpSocket extends CakeSocket {
	...
	/**
	 * Set the resource to receive the request content.
	 * This resource must support fwrite.
	 *
	 * @param resource|boolean $resource
	 ...
	 * HttpSocket::get() and HttpSocket::post() wrap this
	...
	public function request($request = array()) {
		...
		$this->request = Hash::merge ( $this->request, array_diff_key ( $this->config ['request'], array (
				'cookies' => true 
		) ), $request );
		...
		if (is_array ( $this->request ['header'] )) {
			...
			if (($scheme === 'http' && $port != 80) || ($scheme === 'https' && $port != 443) || ($port != 80 && $port != 443)) {
				$Host .= ':' . $port;
			}
	...
	/**
	 * Normalizes URLs into a $uriTemplate.
	 ...
	 *
	 * @param
	 *        	string|array Either a string or array of URL options to create an URL with.
	...
	protected function _setAuth() {
		...
		call_user_func_array ( "$authClass::authentication", array (
				$this,
				&$this->_auth [$method] 
		) );
	}
	...
	/**
	 * Configure the socket's context.
	 * Adds in configuration
	...
	/**
	 * This function can be thought of as a reverse to PHP5's http_build_query().
	 * It takes a given query string and turns it into an array and
	...
	/**
	 * Builds a request line according to HTTP/1.1 specs.
	 * Activate quirks mode to work outside specs.
	 *
	 * @param array $request
	...
	public function buildCookies($cookies) {
		...
		return $this->_buildHeader ( array (
