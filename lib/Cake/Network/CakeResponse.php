...
class CakeResponse {
	 ...
	 * This can be an 'extension' that will be transformed using the $_mimetypes array
	...
	/**
	 * File range.
	 * Used for requesting ranges of files.
	 *
	 * @var array
	 */
	...
	protected function _setCookies() {
		foreach ( $this->_cookies as $name => $c ) {
			setcookie ( $name, $c ['value'], $c ['expire'], $c ['path'], $c ['domain'], $c ['secure'], $c ['httpOnly'] );
		}
	}
	...
	protected function _setContentType() {
		if (in_array ( $this->_status, array (
				304,
				204 
		) )) {
			return;
	 ...
	 * It can be either a file extension
	 ...
	 *        	If set to true, the Cache-Control header will be set as public
	 ...
	 *
	 * @param integer $seconds
	 *        	if null, the method will return the current
	 ...
	 * values will be imploded into a comma separated string.
	 * If no
	 * parameters are passed, then an array with the current Vary header
	 * value is returned
	 *
	 * @param string|array $cacheVariances
	 *        	a single Vary string or a array
	 ...
	 * can be cached by a HTTP client.
	 * A bad way of generating Etags is
	 ...
	 *
	 * @param string $hash
	 *        	the unique has that identifies this response
	 * @param boolean $weak
	 *        	whether the response is semantically the same as
	...
	public function compress() {
		$compressionEnabled = ini_get ( "zlib.output_compression" ) !== '1' && extension_loaded ( "zlib" ) && (strpos ( env ( 'HTTP_ACCEPT_ENCODING' ), 'gzip' ) !== false);
	...
	/**
	 * Sets the protocol to be used when sending the response.
	 * Defaults to HTTP/1.1
	 * If called with no arguments, it will return the current configured protocol
	 *
	 * @param
	 *        	string protocol to be used for sending response
	 ...
	 * headers headers.
	 * If the response is detected to be not modified, it
	...
	/**
	 * String conversion.
	 * Fetches the response body as a string.
	 ...
	 *
	 * @param array $options
	 *        	Either null to get all cookies, string for a specific cookie
