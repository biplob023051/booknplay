...
class CakeRequest implements ArrayAccess {
	...
	/**
	 * Array of POST data.
	 * Will contain form data as well as uploaded files.
	...
	/**
	 * Copy of php://input.
	 * Since this stream can only be read once in most SAPI's
	...
	protected function _processPost() {
		if ($_POST) {
			$this->data = $_POST;
		} elseif (($this->is ( 'put' ) || $this->is ( 'delete' )) && strpos ( env ( 'CONTENT_TYPE' ), 'application/x-www-form-urlencoded' ) === 0) {
	...
	/**
	 * Get the request uri.
	 * Looks in PATH_INFO first, as this is the exact value we need prepared
	 ...
	 *
	 * @param boolean $safe
	 *        	Use safe = false when you think the user might manipulate their HTTP_CLIENT_IP
	 ...
	 * Uses the built in detection rules
	 ...
	 *
	 * @param integer $tldLength
	 *        	Number of segments your tld contains. For example: `example.com` contains 1 tld.
	 ...
	 *        	A decoding callback that will convert the string data to another
