...
class PaginatorHelper extends AppHelper {
	 ...
	 * Defaults to JsHelper. You should make sure
	 ...
	 * Sets up the helper that is used for creating 'AJAX' links.
	 *
	 ...
	 *        	Default options for pagination links. If a string is supplied - it
	...
	public function options($options = array()) {
		...
		if (! empty ( $options [$model] )) {
			...
			$this->request->params ['paging'] [$model] = array_merge ( $this->request->params ['paging'] [$model], $options [$model] );
			unset ( $options [$model] );
		}
	 ...
	 * Sets named parameters for the sort and direction. Handles
	 ...
	 * @param string $title
	 *        	Title for the link. If $title is null $key will be used
	...
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check} ( $model )) {
			...
			if ($tag === false) {
				return $this->link ( $title, $url, compact ( 'escape', 'model', 'class' ) + $options );
			}
	...
	public function numbers($options = array()) {
		...
		$params = ( array ) $this->params ( $options ['model'] ) + array (
				'page' => 1 
		);
		unset ( $options ['model'] );
		...
		if ($modulus && $params ['pageCount'] > $modulus) {
			...
			if ($first && $start > 1) {
				...
				if ($offset < $start - 1) {
					...
					$out .= $this->first ( $offset, compact ( 'tag', 'separator', 'class', 'ellipsis' ) + array (
							'after' => $separator 
					) );
				}
			}
			...
			for($i = $start; $i < $params ['page']; $i ++) {
				$out .= $this->Html->tag ( $tag, $this->link ( $i, array (
						'page' => $i 
			...
			if ($currentTag) {
				$out .= $this->Html->tag ( $tag, $this->Html->tag ( $currentTag, $params ['page'] ), array (
						'class' => $currentClass 
				) );
			} else {
				$out .= $this->Html->tag ( $tag, $params ['page'], array (
						'class' => $currentClass 
				) );
			}
			...
			if ($last && $end < $params ['pageCount']) {
				...
				if ($offset <= $last && $params ['pageCount'] - $end > $offset) {
					...
					$out .= $this->last ( $offset, compact ( 'tag', 'separator', 'class', 'ellipsis' ) + array (
							'before' => $separator 
					) );
				}
			}
			...
			for($i = 1; $i <= $params ['pageCount']; $i ++) {
				if ($i == $params ['page']) {
					...
					if ($currentTag) {
						$out .= $this->Html->tag ( $tag, $this->Html->tag ( $currentTag, $i ), array (
								'class' => $currentClass 
						) );
					} else {
						$out .= $this->Html->tag ( $tag, $i, array (
								'class' => $currentClass 
						) );
					}
				} else {
	 ...
	 *
	 * @param string|integer $first
	 *        	if string use as label for the link. If numeric, the number of page links
	...
	public function first($first = '<< first', $options = array()) {
		...
		if (is_int ( $first ) && $params ['page'] >= $first) {
			...
			$out = $this->Html->tag ( $tag, $this->link ( $first, array (
					'page' => 1 
			), $options ), compact ( 'class' ) ) . $after;
		}
		return $out;
	}
	...
	public function last($last = 'last >>', $options = array()) {
		...
		if (is_int ( $last ) && $params ['page'] <= $lower) {
			...
			$out = $before . $this->Html->tag ( $tag, $this->link ( $last, array (
