...
class TextHelper extends AppHelper {
	...
	public function autoLinkUrls($text, $options = array()) {
		...
		$text = preg_replace_callback ( '#(?<!href="|">)(?<!\b[[:punct:]])(?<!http://|https://|ftp://|nntp://)www.[^\n\%\ <]+[^<\n\%\,\.\ <](?<!\))#i', array (
				&$this,
				'_insertPlaceHolder' 
		), $text );
		if ($options ['escape']) {
	...
	/**
	 * Saves the placeholder for a string, for later use.
	 * This gets around double
	...
	public function autoLinkEmails($text, $options = array()) {
		...
		$atom = '[a-z0-9!#$%&\'*+\/=?^_`{|}~-]';
		$text = preg_replace_callback ( '/(' . $atom . '+(?:\.' . $atom . '+)*@[a-z0-9-]+(?:\.[a-z0-9-]+)+)/i', array (
				&$this,
				'_insertPlaceholder' 
		), $text );
		if ($options ['escape']) {
	...
	public function autoLink($text, $options = array()) {
		...
		return $this->autoLinkEmails ( $text, array_merge ( $options, array (
				'escape' => false 
		) ) );
	}
