...
class CacheHelper extends AppHelper {
	...
	/**
	 * Parse a file + output.
	 * Matches nocache tags between the current output and the current file
	...
	protected function _parseContent($file, $out) {
		$out = preg_replace_callback ( '/<!--nocache-->/', array (
				$this,
				'_replaceSection' 
		), $out );
	...
	public function cache($file, $out) {
		...
		if ($cacheTime && $cacheTime > 0) {
			...
			try {
				...
				$message = __d ( 'cake_dev', 'Unable to write view cache file: "%s" for "%s"', $e->getMessage (), $this->request->here );
	...
	/**
	 * Strip cake:nocache tags from a string.
	 * Since View::render()
