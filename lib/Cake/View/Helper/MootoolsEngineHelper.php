...
class MootoolsEngineHelper extends JsBaseEngineHelper {
	...
	public function domReady($functionBody) {
		...
		return $this->event ( 'domready', $functionBody, array (
				'stop' => false 
		) );
	}
	...
	public function effect($name, $options = array()) {
		$speed = null;
		if (isset ( $options ['speed'] ) && in_array ( $options ['speed'], array (
				'fast',
				'slow' 
		) )) {
	...
	public function drop($options = array()) {
		if (empty ( $options ['drag'] )) {
			trigger_error ( __d ( 'cake_dev', 'MootoolsEngine::drop() requires a "drag" option to properly function' ), E_USER_WARNING );
			return false;
		}
		...
		$safe = array_merge ( array_keys ( $this->_callbackArguments ['drop'] ), array (
				'droppables' 
		) );
