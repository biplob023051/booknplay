...
class FormHelper extends AppHelper {
		...
		if ($key === 'fields') {
			if (! isset ( $this->fieldset [$model] ['fields'] )) {
				...
				foreach ( $object->hasAndBelongsToMany as $alias => $assocData ) {
					$this->fieldset [$object->alias] ['fields'] [$alias] = array (
							'type' => 'multiple' 
					);
				}
			}
			...
			return isset ( $object->hasAndBelongsToMany [$field] ) ? array (
					'type' => 'multiple' 
			) : null;
		}
	...
	public function error($field, $text = null, $options = array()) {
		...
		if (is_array ( $error )) {
			if (count ( $error ) > 1) {
				...
				$error = call_user_func_array ( array (
						$this->Html,
						'nestedList' 
				), $listParams );
			} else {
	...
	public function inputs($fields = null, $blacklist = null, $options = array()) {
		...
		foreach ( $fields as $name => $options ) {
			...
			$entity = explode ( '.', $name );
			$blacklisted = (is_array ( $blacklist ) && (in_array ( $name, $blacklist ) || in_array ( end ( $entity ), $blacklist )));
			if ($blacklisted) {
				continue;
			}
	...
	protected function _getInput($args) {
		...
		switch ($type) {
			...
			case 'textarea' :
				return $this->textarea ( $fieldName, $options + array (
						'cols' => '30',
						'rows' => '6' 
				) );
			case 'url' :
	...
	protected function _maxLength($options) {
		...
		$autoLength = (! array_key_exists ( 'maxlength', $options ) && isset ( $fieldDef ['length'] ) && is_scalar ( $fieldDef ['length'] ) && $options ['type'] !== 'select');
		if ($autoLength && in_array ( $options ['type'], array (
				'text',
				'email',
				'tel',
				'url',
	...
	protected function _inputLabel($fieldName, $label, $options) {
		...
		if ($options ['type'] === 'date' || $options ['type'] === 'datetime') {
			$firstInput = 'M';
			if (array_key_exists ( 'dateFormat', $options ) && ($options ['dateFormat'] === null || $options ['dateFormat'] === 'NONE')) {
				$firstInput = 'H';
	...
	public function checkbox($fieldName, $options = array()) {
		...
		$options = $this->_initInputField ( $fieldName, $options ) + array (
				'hiddenField' => true 
		);
	...
	public function radio($fieldName, $options = array(), $attributes = array()) {
		...
		foreach ( $options as $optValue => $optTitle ) {
			...
			$out [] = $this->Html->useTag ( 'radio', $attributes ['name'], $tagName, array_diff_key ( $allOptions, array (
					'name' => null,
					'type' => null,
					'id' => null 
			) ), $optTitle );
	...
	public function __call($method, $params) {
		...
		return $this->Html->useTag ( 'input', $options ['name'], array_diff_key ( $options, array (
				'name' => null 
		) ) );
	}
	...
	public function postLink($title, $url = null, $options = array(), $confirmMessage = false) {
		...
		if ($confirmMessage) {
			$confirmMessage = str_replace ( array (
					"'",
					'"' 
			), array (
					"\'",
	 ...
	 *        	New default values
	 * @param
	 *        	boolean Merge with current defaults
