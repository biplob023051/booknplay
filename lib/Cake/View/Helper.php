...
class Helper extends Object {
	 ...
	 * sub inputs for a parent input.
	 * This is used for date/time
	 * inputs primarily.
	 *
	 * @var array
	 */
	...
	/**
	 * Lazy loads helpers.
	 * Provides access to deprecated request properties as well.
	 *
	 * @param string $name
	...
	public function __get($name) {
		if (isset ( $this->_helperMap [$name] ) && ! isset ( $this->{$name} )) {
			$settings = array_merge ( ( array ) $this->_helperMap [$name] ['settings'], array (
					'enabled' => false 
			) );
	 ...
	 *        	Either a relative string url like `/products/view/23` or
	 ...
	 * Depending on options passed provides full url with domain name.
	 * Also calls Helper::assetTimestamp() to add timestamp to local files
	 *
	 * @param
	 *        	string|array Path string or url array
	 * @param array $options
	 ...
	 * Removes a number of well known XSS attacks
	...
	public function domId($options = null, $id = 'id') {
		...
		$dom = $model . implode ( '', array_map ( array (
				'Inflector',
				'camelize' 
		), $entity ) );
	 ...
	 * beforeLayout is called before the layout is rendered.
	 *
	 * Overridden in subclasses.
	 *
	 ...
	 * afterLayout is called after the layout has rendered.
	 *
	 * Overridden in subclasses.
	 *
	 * @param string $layoutFile
	...
	protected function _clean() {
		...
		$this->_cleaned = str_replace ( array (
				"&amp;",
				"&lt;",
				"&gt;" 
		), array (
