 ...
 * View interacts with Helpers and view variables passed
...
class View extends Object {
	...
	/**
	 * Turns on or off Cake's conventional mode of applying layout files.
	 * On by default.
	...
	/**
	 * File extension.
	 * Defaults to Cake's template ".ctp".
	 *
	 * @var string
	 */
	public $ext = '.ctp';
	 ...
	 * This is often used for extension based routing.
	...
	/**
	 * The Cache configuration View will use to store cached elements.
	 * Changing this will change
	 ...
	 * Used for resolving parent files.
	 *
	 * @var string
	 */
	...
	/**
	 * Currently rendering an element.
	 * Used for finding parent fragments
	 * for elements.
	 *
	 * @var string
	 */
	 ...
	 * Usually the manager is shared with
	...
	public function render($view = null, $layout = null) {
		...
		if ($view !== false && $viewFileName = $this->_getViewFileName ( $view )) {
			...
			$this->getEventManager ()->dispatch ( new CakeEvent ( 'View.beforeRender', $this, array (
					$viewFileName 
			) ) );
			$this->Blocks->set ( 'content', $this->_render ( $viewFileName ) );
			$this->getEventManager ()->dispatch ( new CakeEvent ( 'View.afterRender', $this, array (
					$viewFileName 
			) ) );
		}
	...
	/**
	 * Renders a layout.
	 * Returns output from _render(). Returns false on error.
	...
	/**
	 * Render cached view.
	 * Works in concert with CacheHelper and Dispatcher to
	...
	/**
	 * Provides view or element extension/inheritance.
	 * Views can extends a
			...
			switch ($this->_currentType) {
				case self::TYPE_ELEMENT :
					...
					if (! $parent) {
						...
						throw new LogicException ( __d ( 'cake_dev', 'You cannot extend an element which does not exist (%s).', $defaultPath . $name . $this->ext ) );
					}
					break;
	 ...
	 * Provides access to attributes that were deprecated.
	 *
	 * @param string $name
	...
	protected function _render($viewFile, $data = array()) {
		...
		$beforeEvent = new CakeEvent ( 'View.beforeRenderFile', $this, array (
				$viewFile 
		) );
		...
		$afterEvent = new CakeEvent ( 'View.afterRenderFile', $this, array (
				$viewFile,
				$content 
		) );
	...
	/**
	 * Loads a helper.
	 * Delegates to the `HelperCollection::load()` to load the helper
	 *
	...
	protected function _getViewFileName($name = null) {
		...
		throw new MissingViewException ( array (
				'file' => $defaultPath . $name . $this->ext 
		) );
	}
