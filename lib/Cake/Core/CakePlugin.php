...
class CakePlugin {
	...
	public static function load($plugin, $config = array()) {
		...
		if (empty ( $config ['path'] )) {
			foreach ( App::path ( 'plugins' ) as $path ) {
				if (is_dir ( $path . $plugin )) {
					self::$_plugins [$plugin] = $config + array (
							'path' => $path . $plugin . DS 
					);
					break;
				}
		...
		if (empty ( self::$_plugins [$plugin] ['path'] )) {
			throw new MissingPluginException ( array (
					'plugin' => $plugin 
			) );
		}
	 ...
	 * It is possible to set specific defaults for each plugins in the options array.
	 * Examples:
	 *
	 * {{{
	...
	public static function bootstrap($plugin) {
		...
		if (is_callable ( $config ['bootstrap'] )) {
			return call_user_func_array ( $config ['bootstrap'], array (
					$plugin,
					$config 
			) );
		}
		...
		if ($config ['bootstrap'] === true) {
			return self::_includeFile ( $path . 'Config' . DS . 'bootstrap.php', $config ['ignoreMissing'] );
		}
		
		$bootstrap = ( array ) $config ['bootstrap'];
		foreach ( $bootstrap as $file ) {
			self::_includeFile ( $path . 'Config' . DS . $file . '.php', $config ['ignoreMissing'] );
		}
	 ...
	 *
	 * @param string $plugin
	 *        	name of the plugin, if null will operate on all plugins having enabled the
	...
	public static function routes($plugin = null) {
		...
		return ( bool ) self::_includeFile ( self::path ( $plugin ) . 'Config' . DS . 'routes.php', $config ['ignoreMissing'] );
	}
