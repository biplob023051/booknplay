...
class App {
	...
	/**
	 * Indicates the the Application is in the bootstrapping process.
	 * Used to better cache
	...
	/**
	 * Get all the currently loaded paths from App.
	...
	/**
	 * Sets up each package location on the file system.
	 * You can configure multiple search paths
	 ...
	 *        	App::RESET will set paths, App::APPEND with append paths, App::PREPEND will prepend paths (default)
	...
	/**
	 * Gets the path that a plugin is on.
	 * Searches through the defined plugin paths.
	 *
	 * Usage:
	 *
	...
	/**
	 * Finds the path that a theme is on.
	 * Searches through the defined theme paths.
	 *
	 * Usage:
	 *
	 ...
	 * This package location will be used
	...
	/**
	 * Method to handle the automatic class loading.
	 * It will look for each class' package
	...
	public static function load($className) {
		...
		list ( $plugin, $package ) = count ( $parts ) > 1 ? $parts : array (
				null,
				current ( $parts ) 
		);
	 ...
	 * Calling App::import() will
	 * not construct any classes contained in the files. It will only find and require() the file.
	 *
	 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#including-files-with-app-import
	 * @param string|array $type
	 *        	The type of Class if passed as a string, or all params can be passed as
	 *        	an single array to $type,
	 * @param string $name
	 *        	Name of the Class or a unique name for the file
	 * @param boolean|array $parent
	 *        	boolean true if Class Parent should be searched, accepts key => value
	 ...
	 *        	Return the loaded file, the file must have a return
	...
	public static function import($type = null, $name = null, $parent = true, $search = array(), $file = null, $return = false) {
		...
		if (is_array ( $name )) {
			foreach ( $name as $class ) {
				if (! App::import ( compact ( 'type', 'parent', 'search', 'file', 'return' ) + array (
						'name' => $class 
				) )) {
					return false;
				}
		...
		$specialPackage = in_array ( $originalType, array (
				'file',
				'vendor' 
		) );
	...
	protected static function _loadClass($name, $plugin, $type, $originalType, $parent) {
		...
		if ($parent && isset ( self::$types [$originalType] ['extends'] )) {
			...
			if ($plugin && in_array ( $originalType, array (
					'controller',
					'model' 
			) )) {
