...
class Router {
	...
	/**
	 * List of valid extensions to parse from an URL.
	 ...
	 * of this file.
	 * This is a cheat as get_class_vars() returns the value of static vars even if they
	 * have changed.
	 *
	 * @var array
	 */
	...
	protected static function _validateRouteClass($routeClass) {
		if ($routeClass !== 'CakeRoute' && (! class_exists ( $routeClass ) || ! is_subclass_of ( $routeClass, 'CakeRoute' ))) {
	 ...
	 *        	An array describing the default route parameters. These parameters will be used by default
	 *        	and can supply routing parameters that are not dynamic. See above.
	 * @param array $options
	 *        	An array matching the named elements in the route to regular expressions which that
	...
	/**
	 * Specifies what named parameters CakePHP should be parsing out of incoming URLs.
	 * By default
	 ...
	 *
	 * @param array $named
	 *        	A list of named parameters. Key value pairs are accepted where values are
	...
	/**
	 * Parses given URL string.
	 * Returns 'routing' parameters for that url.
	 *
	 * @param string $url
	 *        	URL to be parsed
	 ...
	 *
	 * @param CakeRequest|array $request
	...
	/**
	 * Pops a request off of the request stack.
	 * Used when doing requestAction
	 *
	...
	public static function getPaths($current = false) {
		...
		return array (
				'base' => self::$_requests [0]->base 
		);
	}
	
	/**
	 * Reloads default Router settings.
	 * Resets all class variables and
	 ...
	 *
	 * @param integer $which
	 *        	A zero-based array index representing the route to move. For example,
	 ...
	 *
	 * @param array $url
	 *        	An URL that didn't match any routes
	 ...
	 *        	Query string Either a string of already compiled query string arguments or
	 ...
	 *
	 * @param CakeRequest|array $params
	 *        	The params array or CakeRequest object that needs to be reversed.
	 * @param boolean $full
	 *        	Set to true to include the full URL including the protocol when reversing
	 *        	the URL.
	 ...
	 * @param boolean $merge
	 *        	Default true will merge extensions. Set to false to override current extensions
	 * @return array
	 */
