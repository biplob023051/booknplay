...
class CakeRoute {
		...
		foreach ( $args as $param ) {
			...
			if ((! isset ( $this->options ['named'] ) || ! empty ( $this->options ['named'] )) && $separatorIsPresent) {
				...
				if ($passIt) {
					...
					$named = array_merge_recursive ( $named, array (
							$key => $val 
					) );
				}
			} else {
	...
	/**
	 * Apply persistent parameters to an URL array.
	 * Persistent parameters are a special
	...
	public function match($url) {
		...
		foreach ( $url as $key => $value ) {
			...
			if (($greedyNamed || isset ( $allowedNamedParams [$key] )) && ($value !== false && $value !== null) && (! in_array ( $key, $prefixes ))) {
				$named [$key] = $value;
				continue;
			}
