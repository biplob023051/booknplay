...
class CakeNumber {
	...
	/**
	 * Currencies supported by the helper.
	 * You can add additional currency formats
	...
	/**
	 * If native number_format() should be used.
	 * If >= PHP5.4
	 *
	 * @var boolean
	 */
	...
	public static function fromReadableSize($size, $default = false) {
		...
		$l = - 2;
		$i = array_search ( substr ( $size, - 2 ), array (
				'KB',
				'MB',
				'GB',
				'TB',
		...
		if ($i === false) {
			$l = - 1;
			$i = array_search ( substr ( $size, - 1 ), array (
					'K',
					'M',
					'G',
					'T',
	 ...
	 *        	if int then places, if string then before, if (,.-) then use it
	 ...
	 *        	Shortcut to default options. Valid values are
	...
	/**
	 * Add a currency format to the Number helper.
	 * Makes reusing
