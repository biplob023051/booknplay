...
/**
 * Validation Class.
 * Used for validation of model data
 *
...
class Validation {
	 ...
	 *        	'all' may be passed as a sting, defaults to fast which checks format of most major credit cards
	 ...
	 *
	 * @param string|array $check1
	 *        	if string is passed for a string must also be passed for $check2
	...
	public static function date($check, $format = 'ymd', $regex = null) {
		...
		$format = (is_array ( $format )) ? array_values ( $format ) : array (
				$format 
		);
	...
	public static function ip($check, $type = 'both') {
		...
		return ( boolean ) filter_var ( $check, FILTER_VALIDATE_IP, array (
				'flags' => $flags 
		) );
	}
	...
	public static function url($check, $strict = false) {
		...
		$regex = '/^(?:(?:https?|ftps?|sftp|file|news|gopher):\/\/)' . (! empty ( $strict ) ? '' : '?') . '(?:' . self::$_pattern ['IPv4'] . '|\[' . self::$_pattern ['IPv6'] . '\]|' . self::$_pattern ['hostname'] . ')(?::[1-9][0-9]{0,4})?' . '(?:\/?|\/' . $validChars . '*)?' . '(?:\?' . $validChars . '*)?' . '(?:#' . $validChars . '*)?$/iu';
		return self::_check ( $check, $regex );
	}
