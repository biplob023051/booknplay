...
class Set {
	...
	/**
	 * Maps the given value as an object.
	 ...
	 * If $path does not contain a '/' the call
	...
	public static function extract($path, $data = null, $options = array()) {
		...
		do {
			...
			foreach ( $contexts as $key => $context ) {
				...
				} elseif (is_array ( $context ['item'] ) && array_key_exists ( $token, $context ['item'] ) && ! (strval ( $key ) === strval ( $token ) && count ( $tokens ) === 1 && $tokens [0] === '.')) {
			...
			if ($conditions) {
				foreach ( $conditions as $condition ) {
					...
					foreach ( $matches as $i => $match ) {
						if (Set::matches ( array (
								$condition 
						), $match ['item'], $i + 1, $length )) {
							$filtered [$i] = $match;
						}
					}
					$matches = $filtered;
	 ...
	 * and "{[a-z]+}" (i.e.
	 * any string literal enclosed in brackets besides {n} and {s}) is interpreted as
	 ...
	 * If $all is set to false (which is the default) it will
	 ...
	 * $path2 as path to get the values.
	 * If $path2 is not specified, all values will be initialized
	 ...
	 * each array element's key, i.e.
	 * array(array('Foo' => array('Bar' => 'Far'))) becomes
	 * array('0.Foo.Bar' => 'Far').
	 *
	 * @param array $data
	 *        	Array to flatten
	...
	public static function sort($data, $path, $dir) {
		...
		list ( $keys, $values ) = array (
				Set::extract ( $result, '{n}.id' ),
				Set::extract ( $result, '{n}.value' ) 
		);
	 ...
	 *        	Callback method to be applied to extracted data.
	...
	public static function nest($data, $options = array()) {
		...
		foreach ( $data as $result ) {
			...
			if (isset ( $idMap [$id] [$options ['children']] )) {
				...
				$idMap [$id] = array_merge ( $result, array (
