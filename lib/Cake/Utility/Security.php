...
class Security {
	 ...
	 * @param mixed $salt
	 *        	If true, automatically appends the application's salt
	...
	public static function setCost($cost) {
		if ($cost < 4 || $cost > 31) {
			trigger_error ( __d ( 'cake_dev', 'Invalid value, cost must be between %s and %s', array (
					4,
					31 
			) ), E_USER_WARNING );
			return null;
	 ...
	 * To use blowfish hashing see ``Security::hash()``
	 *
	...
	protected static function _crypt($password, $salt = false) {
		...
		if ($salt === true || strpos ( $salt, '$2a$' ) !== 0 || strlen ( $salt ) < 29) {
			trigger_error ( __d ( 'cake_dev', 'Invalid salt: %s for %s Please visit http://www.php.net/crypt and read the appropriate section for building %s salts.', array (
					$salt,
					'blowfish',
					'blowfish' 
			) ), E_USER_WARNING );
