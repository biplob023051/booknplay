...
abstract class ObjectCollection {
	 ...
	 *
	 *
	 * @param string $callback|CakeEvent
	...
	public function trigger($callback, $params = array(), $options = array()) {
		...
		foreach ( $list as $name ) {
			...
			if ($options ['break'] && ($result === $options ['breakOn'] || (is_array ( $options ['breakOn'] ) && in_array ( $result, $options ['breakOn'], true )))) {
				return $result;
			} elseif ($options ['modParams'] !== false && ! in_array ( $result, array (
					true,
					false,
					null 
			), true )) {
	 ...
	 * @param
	 *        	boolean Prioritize enabled list after enabling object(s)
	 * @return void
	 */
	 ...
	 * @param
	 *        	integer|null Integer priority to set or null for default
	 * @return void
	 */
