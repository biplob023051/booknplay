...
class String {
	...
	/**
	 * Replaces variable placeholders inside a $str with any given $data.
	 * Each key in the $data array
	 ...
	 * $options.
	 * The default method used is text but html is also available. The goal of this function
	 ...
	 * You can specify any expression in highlighter that
	...
	public static function excerpt($text, $phrase, $radius = 100, $ellipsis = '...') {
		if (empty ( $text ) || empty ( $phrase )) {
			return self::truncate ( $text, $radius * 2, array (
					'ellipsis' => $ellipsis 
			) );
		}
