...
class CakeTime {
	 ...
	 *
	 * @param string $format
	 *        	Format with specifiers for strftime function.
		...
		switch ($specifier [1]) {
			...
			case 'r' :
				...
				if ($complete !== 't_fmt_ampm') {
					return str_replace ( '%p', self::_translateSpecifier ( array (
							'%p',
							'p' 
					) ), $complete );
				}
	 ...
	 *
	 * @param integer|string|DateTime $dateString
	 ...
	 *
	 * @param integer|string|DateTime $begin
	 *        	UNIX timestamp, strtotime() valid string or DateTime object
	 ...
	 * Wrapper for PHP function strtotime().
	 * 
	 * @param integer|string|DateTime $dateString
	 *        	UNIX timestamp, strtotime() valid string or DateTime object
	 * @param string|DateTimeZone $timezone
	 ...
	 *
	 * @param integer|string|DateTime $dateTime
	 ...
	 *
	 * @param string|integer $timeInterval
	 *        	the numeric value with space then time type.
	...
	public static function gmt($dateString = null) {
		...
		return gmmktime ( intval ( date ( 'G', $time ) ), intval ( date ( 'i', $time ) ), intval ( date ( 's', $time ) ), intval ( date ( 'n', $time ) ), intval ( date ( 'j', $time ) ), intval ( date ( 'Y', $time ) ) );
	}
	 ...
	 * @param integer|string|DateTime $format
