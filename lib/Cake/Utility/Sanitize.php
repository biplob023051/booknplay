...
class Sanitize {
	...
	public static function stripAll($str) {
		return Sanitize::stripScripts ( Sanitize::stripImages ( Sanitize::stripWhitespace ( $str ) ) );
	}
	
	/**
	 * Strips the specified tags from output.
	 * First parameter is string from
	...
	public static function clean($data, $options = array()) {
		...
		if ($options ['encode']) {
			$data = Sanitize::html ( $data, array (
					'remove' => $options ['remove_html'] 
			) );
		}
		if ($options ['dollar']) {
