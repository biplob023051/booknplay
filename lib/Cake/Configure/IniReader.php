...
class IniReader implements ConfigReaderInterface {
	...
	/**
	 * Build and construct a new ini file parser.
	 * The parser can be used to read
	 * ini files that are on the filesystem.
	 *
	 * @param string $path
	 *        	Path to load ini config files from. Defaults to APP . 'Config' . DS
	 * @param string $section
	 *        	Only get one section, leave null to parse and fetch
	 ...
	 *
	 * @param string $key
	 *        	The identifier to read from. If the key has a . it will be treated
		...
		if (! empty ( $this->_section ) && isset ( $contents [$this->_section] )) {
			...
			foreach ( $contents as $section => $attribs ) {
				if (is_array ( $attribs )) {
					...
					$parse = $this->_parseNestedValues ( array (
							$attribs 
					) );
	 ...
	 *
	 * @param string $key
	 *        	The identifier to write to. If the key has a . it will be treated
	 *        	as a plugin prefix.
	 * @param array $data
