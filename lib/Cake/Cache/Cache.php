...
class Cache {
	...
	/**
	 * Drops a cache engine.
	 * Deletes the cache configuration information
	...
	/**
	 * Temporarily change the settings on a cache config.
	 * The settings will persist for the next write
	 ...
	 * Will automatically use the currently
	...
	public static function write($key, $value, $config = 'default') {
		...
		if ($success === false && $value !== '') {
			trigger_error ( __d ( 'cake_dev', "%s cache was unable to write '%s' to %s cache", $config, $key, self::$_engines [$config]->settings ['engine'] ), E_USER_WARNING );
		}
		return $success;
	}
