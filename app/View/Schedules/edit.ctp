	...
	<div class="row-fluid">
		<div class="block">
			<div class="categoriesform">
				<p class="block-heading"><?php echo __('Edit Schedule'); ?></p>
				<div id="chart-container" class="block-body collapse in">
					...
					<?php echo $this->EBForm->create('Schedule',array('class'=>'form-horizontal','type'=>'file')); ?>					<div class="control-group">
							<?php
								...
								echo $this->EBForm->input('date',array('type'=>'text','class'=>'datepicker'));
							...
							<table cellpadding="5" cellspacing="5" border="0">
				            	<?php 
									$prices_slot = explode(',', $this->request->data['Schedule']['prices']);
									
					            	for($j=1;$j<=Configure::read ( 'slots_per_day' );$j++){
										
										$price_value = !empty($prices_slot[$j-1]) ? $prices_slot[$j-1] : 100;
				            	?>
								<tr>
								<td>
								...
								<input type="number" id="<?php echo 's'.$j;?>" name="data[prices][<?php echo 's'.$j;?>]" placeholder="<?php echo 's'.$j;?>" value = '<?php echo $price_value;?>' class='single_slot' maxlength="5" style="display: inline-block;width: 38px;height: 18px;margin: -1px 5px 0 0;vertical-align: middle;border: 1px solid #ccc;border-bottom: double #ccc;-webkit-border-radius: 2px;-moz-border-radius: 2px;border-radius: 2px;text-align: center;"/>
								</td>
								</tr>
				                <?php }?>
								</table>
