 <?php
	class ImageComponent extends Component {
		...
		/**
		 * Determines image type, calculates scaled image size, and returns resized image.
		 * If no width or height is
		 * specified for the new image, the dimensions of the original image will be used, resulting in a copy
		 * of the original image.
		 *
		 * @param string $original
		 *        	absolute path to original image file
		 * @param string $new_filename
		 *        	absolute path to new image file to be created
		 * @param integer $new_width
		 *        	(optional) width to scale new image (default 0)
		 * @param integer $new_height
		 *        	(optional) height to scale image (default 0)
		 * @param integer $quality
		 *        	quality of new image (default 100, resizePng will recalculate this value)
		 *        	
		 * @access public
		 *        
		...
		private function __resizePng($original, $new_filename, $scaled_width, $scaled_height, $width, $height, $quality) {
			 ...
			 * so the higher the value (0-9), the lower the quality.
			 * this is pretty much
