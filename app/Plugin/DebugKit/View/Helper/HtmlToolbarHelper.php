...
class HtmlToolbarHelper extends ToolbarHelper {
	 ...
	 *
	 * @param mixed $values
	 *        	Array to make pretty.
	 * @param int $openDepth
	 *        	Depth to add open class
	...
	public function table($rows, $headers = array()) {
		...
		$out .= $this->Html->tableCells ( $rows, array (
				'class' => 'odd' 
		), array (
				'class' => 'even' 
		), false, false );
	...
	public function send() {
		...
		$toolbar = $view->element ( 'debug_toolbar', array (
				'disableTimer' => true 
		), array (
				'plugin' => 'DebugKit' 
		) );
	 ...
	 *
	 * @param string $sql
	 *        	SQL query string you want an explain link for.
	 * @param
	 *        	$connection
	...
	public function explainLink($sql, $connection) {
		...
		$form = $this->Form->create ( 'log', array (
				'url' => $url,
				'id' => "logForm{$uid}" 
		) );
		$form .= $this->Form->hidden ( 'log.ds', array (
				'id' => "logDs{$uid}",
				'value' => $connection 
		) );
		$form .= $this->Form->hidden ( 'log.sql', array (
				'id' => "logSql{$uid}",
				'value' => $sql 
		) );
		$form .= $this->Form->hidden ( 'log.hash', array (
				'id' => "logHash{$uid}",
				'value' => $hash 
		) );
