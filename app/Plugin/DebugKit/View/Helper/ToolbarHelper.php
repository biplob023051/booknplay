...
class ToolbarHelper extends AppHelper {
	...
	/**
	 * settings property to be overloaded.
	 * Subclasses should specify a format
	 *
	 * @var array
	 */
	 ...
	 *
	 * @param string $name
	 *        	Name of the panel you are replacing.
	 * @param string $content
	 *        	Content to write to the panel.
	 ...
	 *
	 * @param string $name
	 *        	Name of the panel you want cached data for
	 * @param int $index        	
	 ...
	 *        	Connection name to get logs for.
	 * @param array $options
	 *        	Options for the query log retrieval.
	...
	public function getQueryLogs($connection, $options = array()) {
		...
		foreach ( $log ['log'] as $i => $query ) {
			$isSlow = ($query ['took'] > 0 && $query ['numRows'] / $query ['took'] != 1 && $query ['numRows'] / $query ['took'] <= $options ['threshold']);
			$query ['actions'] = '';
			$isHtml = ($this->getName () == 'HtmlToolbar');
			if ($isSlow && $isHtml) {
				$query ['actions'] = sprintf ( '<span class="slow-query">%s</span>', __d ( 'debug_kit', 'maybe slow' ) );
			} elseif ($isSlow) {
				$query ['actions'] = '*';
			}
