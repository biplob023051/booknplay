...
class ToolbarHelperTestCase extends CakeTestCase {
	...
	public function setUp() {
		...
		$this->Toolbar->MockBackend = $this->getMock ( 'Helper', array (
				'testMethod' 
		), array (
				$this->View 
		) );
	...
	public function testCacheWrite() {
		$result = $this->Toolbar->writeCache ( 'test', array (
				'stuff',
				'to',
				'cache' 
		) );
	 ...
	 * top most level of the history stack.
	 * As this is where the current request is stored.
	 *
	 * @return void
	 *
	 */
	public function testOnlyWritingToFirstElement() {
		...
		$this->Toolbar->writeCache ( 'test', array (
				'new',
				'values' 
		) );
	...
	public function testGetQueryLogs() {
		...
		$result = $this->Toolbar->getQueryLogs ( $model->useDbConfig, array (
				'cache' => false 
		) );
