...
class DebugKitToolbarComponentTestCase extends CakeTestCase {
	...
	public function testLoadPanels() {
		...
		$this->Controller->Toolbar->loadPanels ( array (
				'session',
				'request' 
		) );
	...
	public function testLoadPluginPanels() {
		...
		App::build ( array (
				'Plugin' => array (
						$debugKitPath . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS 
				) 
		) );
		...
		$this->_loadController ();
		$this->Controller->Toolbar->loadPanels ( array (
				'DebugkitTestPlugin.PluginTest' 
		) );
		$this->assertInstanceOf ( 'PluginTestPanel', $this->Controller->Toolbar->panels ['plugintest'] );
	}
	...
	public function testLibPanels() {
		...
		App::build ( array (
				'Lib' => array (
						$debugKitPath . 'Test' . DS . 'test_app' . DS . 'Lib' . DS 
				) 
		) );
	...
	public function testCacheConfigGeneration() {
		$this->_loadController ();
		$this->Controller->Components->trigger ( 'startup', array (
				$this->Controller 
		) );
	...
	public function testStateSaving() {
		...
		$this->Controller->Components->trigger ( 'beforeRender', array (
				$this->Controller 
		) );
	...
	public function testNoRequestActionInterference() {
		...
		$result = $this->Controller->requestAction ( '/debug_kit_test/request_action_return', array (
				'return' 
		) );
		$this->assertEquals ( $result, 'I am some value from requestAction.' );
		
		$result = $this->Controller->requestAction ( '/debug_kit_test/request_action_render', array (
				'return' 
		) );
