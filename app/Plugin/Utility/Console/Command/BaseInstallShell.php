...
abstract class BaseInstallShell extends AppShell {
	...
	public function main() {
		$this->stdout->styles ( 'success', array (
				'text' => 'green' 
		) );
	...
	public function checkDbConfig() {
		...
		$answer = strtoupper ( $this->in ( 'Is this correct?', array (
				'Y',
				'N' 
		) ) );
	...
	public function createTables() {
		$answer = strtoupper ( $this->in ( 'Existing tables will be deleted, continue?', array (
				'Y',
				'N' 
		) ) );
	...
	public function executeSchema($path, $track = true) {
		...
		$contents = String::insert ( $contents, array (
		...
		switch ($field) {
			case 'username' :
				...
				if (! $username) {
					...
					$result = $model->find ( 'count', array (
							'conditions' => array (
									$model->alias . '.' . $this->userFields ['username'] => $username 
							) 
					) );
			...
			case 'email' :
				...
				if (! $email) {
					...
					$result = $model->find ( 'count', array (
							'conditions' => array (
									$model->alias . '.' . $this->userFields ['email'] => $email 
							) 
					) );
