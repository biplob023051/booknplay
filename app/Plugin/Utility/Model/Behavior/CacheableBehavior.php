...
class CacheableBehavior extends ModelBehavior {
	...
	public function afterSave(Model $model, $created = true) {
		...
		if ($id && $method && (($created && $events ['onCreate']) || (! $created && $events ['onUpdate']))) {
			...
			if (method_exists ( $model, $method )) {
				...
				$this->writeCache ( $model, $cacheKey, array (
						$model->read ( null, $id ) 
				) );
			}
		}
	 ...
	 * The first index should be the method name, the other indices should be unique values.
	 *
	 * @param Model $model        	
	 * @param array|string $keys        	
	...
	/**
	 * Return the expiration time for cache.
	 * Either used the passed value, or the settings default.
	 *
	 * @param Model $model        	
	 * @param mixed $expires        	
	 * @return int|string
	 ...
	 * Be sure to parse the cache key and validate the config and expires.
	 *
	 * @param Model $model        	
	 * @param array|string $keys        	
	 * @param mixed $value        	
	...
	/**
	 * Global function to reset specific cache keys within each model.
	 * By default, reset the getById and getList method keys.
