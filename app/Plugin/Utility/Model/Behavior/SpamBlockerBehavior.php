	...
	public function beforeSave(Model $model) {
		...
		if ($data) {
			...
			$comments = $model->find ( 'all', array (
					...
					'conditions' => array (
							$columnMap ['email'] => $data [$columnMap ['email']] 
					),
					'recursive' => - 1,
					'contain' => false 
			) );
			...
			if ($comments) {
				foreach ( $comments as $comment ) {
					if (in_array ( $comment [$model->alias] [$columnMap ['status']], array (
							$statusMap ['spam'],
							$statusMap ['deleted'] 
					) )) {
						-- $points;
	...
	public function notify(Model $model, $data, $status, $points = 0) {
		...
		if ($settings ['model'] && $settings ['link'] && $settings ['email']) {
			...
			$result = $Article->find ( 'first', array (
					'conditions' => array (
							$columnMap ['id'] => $data [$columnMap ['foreignKey']] 
					),
					'recursive' => - 1,
					'contain' => false 
			) );
			...
			// Send email
			$Email = new CakeEmail ();
			$Email->to ( $settings ['email'] )->from ( array (
					$email => $author 
			) )->subject ( 'Comment Approval: ' . $title )->helpers ( array (
					'Html',
					'Time' 
			) )->viewVars ( array (
					'settings' => $settings,
			...
			if (Configure::read ( 'debug' )) {
				$Email->transport ( 'Debug' )->config ( array (
						'log' => true 
				) );
			}
