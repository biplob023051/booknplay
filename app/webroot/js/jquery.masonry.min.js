...
jQuery(window).load(function() {
	jQuery(this).bindPortfolioMasonry();
});
jQuery.fn.bindPortfolioMasonry = function(){
	var container = jQuery('.portfolio-item-wrapper');
	container.children().removeClass('inline');
	...
	jQuery(window).resize(function(){
		var temp_width =  container.children().filter(':first').width() + 20;
		container.masonry('reload');		
	});	
	jQuery('ul.portfolio-filter a').click(function(e){
		
		// code for inline style
		container.find('#inlineinfo').parents('.portfolio-item').css({'height':'auto'});
		container.find('#inlineinfo').parents('.portfolio-item').find('.portfolio-item-context').css({'border-bottom':'1px solid #ececec'});
		container.find('#inlineinfo').remove();
		container.children().removeClass('inline');
		...
		if( select_filter == "All" || jQuery(this).parent().index() == 0 ){		
			...
			container.children().not('.' + select_filter).each(function(){
				if( !jQuery(this).hasClass('hide') ){
			...
			container.children('.' + select_filter).each(function(){
				if( jQuery(this).hasClass('hide') ){
