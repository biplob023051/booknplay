...
function bl_init(){
	...
	$('.single_slot').click(function(){
		...
		if($(this).hasClass('available')){
			...
			//if($('#selected_slots').text() == '')
				...
				}).appendTo($(this).closest('.book_in_bl'));
	...
	function update_price(no, current_ground_id){
		if (typeof(no)==='undefined') no = 0;
		num = 2;
		$.post( BASE_URL+"grounds/booking_price_for_ground", $( "#booking_form_id_"+current_ground_id+", #search_ground_form" ).serialize()+ '&s_number=' + no, function( price_data ) {
			console.log('price_data:'+price_data.base);
			var base = no*base_price;
			var service = no*service_charge;
			var total = base+service;
			$('#base_price').text(price_data.base);
			$('#service_charge').text(price_data.service);
			$('#total_price').text(price_data.total);
		}, 'json');
	}
...
$(document).ready(function(){
	 $('.view_slot').click(function(){
		   ...
		   $('.ajax_bl_parent').hide();
		   $('.happy_hours').hide();
		   $.get(BASE_URL+"grounds/booking_layout/"+$(this).val()+"/"+$('#GroundDate').val()+"/1", function(data, status){
			    $view_slot.html('X');
			    $bookingListEntry.find('.ajax_bl_parent').show();
				$bookingListEntry.find(".ajax_bl").html(data);
				$bookingListEntry.find('.happy_hours').show("slow");
				bl_init();
		   });
	       return true;
	 });
